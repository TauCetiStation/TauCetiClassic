...
export const ReligiousTool = (props, context) => {
  ...
  const { sects, encyclopedia } = data;

  encyclopedia.ASPECTS.map(
    (aspect) => (ASPECT2COLOR[aspect.name] = aspect.color)
  );
      ...
      <Window.Content fontSize="14px">
        <Tabs textAlign="center">
          <Stack direction="raw" width="100%">
            <Stack.Item grow>
              <Tabs.Tab fluid selected={tab === 1} onClick={() => setTab(1)}>
                Religion <Icon name="place-of-worship" />
              </Tabs.Tab>
            </Stack.Item>
            {!sects && (
              <Stack.Item grow>
                <Tabs.Tab fluid selected={tab === 2} onClick={() => setTab(2)}>
                  Rites <Icon name="pray" />
                </Tabs.Tab>
              </Stack.Item>
            )}
            <Stack.Item grow>
              <Tabs.Tab fluid selected={tab === 3} onClick={() => setTab(3)}>
                Encyclopedia <Icon name="book-open" />
              </Tabs.Tab>
            </Stack.Item>
          </Stack>
        </Tabs>
        <Stack.Item>{GetTab(tab, sects)}</Stack.Item>
      </Window.Content>
    </Window>
  );
};
...
const GetInfoItem = (title, list) => {
  let listItems = null;
  if (!list.length) {
    listItems = <Box color="gray">Nothing.</Box>;
  } else {
  ...
  return (
    <Stack.Item width="100%" height={22}>
      <Section title={title} fill={1}>
        <Box textAlign="left" ml={!list.length ? 0 : 3}>
          <ui>{listItems}</ui>
        </Box>
      </Section>
    </Stack.Item>
...
const ReligionTab = (props, context) => {
  ...
  return (
    <Section title={name} textAlign="center">
      <Box>{deities}</Box>
      <Box>
        <Stack mt={2}>
          <Stack.Item width="100%" height={22}>
            <Section title="Resources" fill={1}>
          ...
          <Stack.Item width="100%" height={22}>
            <Section title="Aspects" fill={1}>
              <Box textAlign="left">{GetAspectBox('', aspects)}</Box>
            </Section>
          </Stack.Item>
    ...
    <Stack>
      ...
      <Section height={51} fill scrollable width="100%">
        <Stack.Item>
          {cat === 'RITES' && <ERitesTab />}
          {cat === 'SECTS' && <ESectsTab />}
          {cat === 'ASPECTS' && <EAspectsTab />}
          {cat === 'GOD SPELLS' && <ESpellsTab />}
          {cat === 'HOLY REAGENTS' && <EReagentsTab />}
          {cat === 'FAITH REACTIONS' && <EReactionsTab />}
        </Stack.Item>
      </Section>
    </Stack>
  );
...
const ERitesTab = (props, context) => {
  ...
  return (
    <Box>
      ...
      />
      {items.map((rite) => (
        <Section key={rite.name} title={rite.name}>
          <BlockQuote>
            <Box>{rite.desc.replace(/<[/]?i>/g, '')}</Box>
            <br />
            <Box>
              <b>Length:</b> {rite.ritual_length / 10} seconds.
            </Box>
            <Box color={rite.can_talismaned ? 'green' : 'red'}>
...
const ESectsTab = (props, context) => {
  ...
  return (
    <Box>
      ...
      />
      {items.map((sect) => (
        <Section key={sect.name} title={sect.name}>
          <BlockQuote>
            <Box>{sect.desc}</Box>
            <br />
...
const ESpellsTab = (props, context) => {
  ...
  return (
    <Box>
      ...
      />
      {items.map((spell) => (
        <Section key={spell.name} title={spell.name}>
          <BlockQuote>
            {GetAspectBox('Needed Aspects:', spell.needed_aspects)}
            {GetCostsBox(spell.favor_cost)}
            <Box>Cooldown: {spell.charge_max / 10} seconds</Box>
          </BlockQuote>
        </Section>
      ))}
    </Box>
...
const EReagentsTab = (props, context) => {
  ...
  const items =
    (searchText.length > 0 &&
      // Flatten all categories and apply search to it
      HOLY_REAGENTS.filter(testSearch)) ||
    // If none of that results in a list, return an empty list
    HOLY_REAGENTS;
  ...
  return (
    <Box>
      ...
      />
      {items.map((reagent) => (
        <Section key={reagent.name} title={reagent.name}>
          <BlockQuote>
...
const EReactionsTab = (props, context) => {
  ...
  const testSearch = createSearch(
    searchText,
    (reaction) => reaction.convertable_id + ' to ' + reaction.result_id
  );

  const items =
    (searchText.length > 0 &&
      // Flatten all categories and apply search to it
      FAITH_REACTIONS.filter(testSearch)) ||
    // If none of that results in a list, return an empty list
    FAITH_REACTIONS;
...
const RiteTab = (props, context) => {
  const { act, data } = useBackend(context);
  const { rites, favor, piety, can_talismaning, holds_religious_tool } = data;
                ...
                <>
                  <Button
                    ...
                    disabled={
                      ...
                      favor < rite.favor_cost ||
                      piety < rite.piety_cost
                    }
                    icon="arrow-right"
                    onClick={() =>
                      act('perform_rite', {
                  ...
                  <Button
                    ...
                    tooltip={
                      ...
                      ' favor' +
                      (rite.piety_cost > 0
                        ? ' ' + rite.piety_cost + ' piety'
                        : '')
                    }
                    disabled={
                      !can_talismaning ||
                      !rite.can_talismaned ||
                      favor < rite.favor_cost * 2 ||
                      piety < rite.piety_cost * 2
                    }
                    icon="scroll"
                    onClick={() =>
