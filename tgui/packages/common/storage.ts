...
const listenForHubStorage = async (): Promise<HubStorageBackend> => {
...
// Helper wrapper to keep the enablement state of hubstorage
const doWithHubStorageBackend = async (
  f: (hub: HubStorageBackend) => Promise<void>,
) => {
  if (!testHubStorage()) {
    Byond.winset(null, 'browser-options', '+byondstorage');
    const hub: HubStorageBackend = await listenForHubStorage();
    await f(hub);
    Byond.winset(null, 'browser-options', '-byondstorage');
  } else {
    await f(new HubStorageBackend());
  }
};
...
class StorageProxy implements StorageBackend {
  ...
  constructor() {
    this.backendPromise = (async () => {
      // TG uses the byondstorage flag to check if iframe is not available
      // We have byondstorage enabled by default, so we try the iframe for every window
      ...
      if (Byond.storageCdn) {
        ...
        if (await iframe.ready()) {
          ...
          await doWithHubStorageBackend(async (hub) => {
      ...
      // IFrame didn't work out for us, let's check if we need to enable byondstorage
      if (!testHubStorage()) {
        Byond.winset(null, 'browser-options', '+byondstorage');

        return await listenForHubStorage();
      }
