...
```dm
/obj/machinery/my_machine/tgui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
  . = ..()
  if(.)
    return
...
/obj/copypasta/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
  if(..())
    return
  switch(action)
    if("copypasta")
