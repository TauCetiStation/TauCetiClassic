...
match_helper() {
    ...
    while [[ $s =~ $regex ]]; do
        ...
        run_test_fail "change istype to $define. Use this pattern for your VSCode: ^(?!#define)(.*)$istype_pattern -> \$1$istype(\$2)" "grep -RPnr --include='*.dm' '^(?!#define).*$istype_pattern' code/"
    done
}
