...
/datum/announcement/centcomm/gang/cops_closely/play(gang_names)
	message = "Нам поступила информация, что местная космическая полиция уже приближаются к [station_name()]" + \
	"и прибудут примерно через 5 минут. Напоминаем еще раз, у нас отсутствует юридическое право контролировать" + \
