...
/datum/component/gamemode/syndicate/proc/give_codewords()
	...
	switch(code_words)
		...
		if(1) // half
			to_chat(traitor_mob, "Воспользуйтесь кодовыми сломавами в полученом вами порядке для индентификации других агентов в обычном разговоре. Будьте бдительны, все вокруг вас потенциальные враги.")
		if(2)
...
/datum/component/gamemode/syndicate/proc/equip_traitor()
	...
	for(var/datum/objective/target/dehead/D in R.objectives.GetObjectives())
		...
		if (!where)
			to_chat(traitor_mob, "Синдикат к несчастью не может предоставить вам банку с биогелем для хранения голов.")
		else
