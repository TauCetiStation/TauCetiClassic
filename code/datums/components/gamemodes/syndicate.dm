...
/datum/component/gamemode/syndicate/proc/give_uplink()
	...
	if (istype(R, /obj/item/device/radio))
		...
		to_chat(traitor_mob, "Портативное телепортационное реле, сокращённо - Аплинк, было установлено в ваш [R.name] [loc]. Просто введите код выданный вам ранее \"[pda_pass]\", зайдите в настройки вашего КПК, а именно изменения вашего рингтона, вместо \"beep\" введите тот самый код для получения доступа к скрытому функционалу.")
...
/datum/component/gamemode/syndicate/proc/give_codewords()
	...
	switch(code_words)
		...
		if(1) // half
			to_chat(traitor_mob, "Воспользуйтесь кодовыми словами в полученном вами порядке для индентификации других агентов в обычном разговоре. Будьте бдительны, все вокруг вас потенциальные враги.")
		if(2)
...
/datum/component/gamemode/syndicate/proc/equip_traitor()
	...
	if (traitor_mob.mind?.assigned_role == "Clown")
		to_chat(traitor_mob, "Ваши специальные тренировки позволили вам преодолеть клоунскую неуклюжесть, что позволит вам без затруднений и вреда для себя применять любое вооружение.")
