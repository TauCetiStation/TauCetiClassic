...
/datum/component/anti_magic/Initialize(
	...
	if(isitem(movable))
		...
		RegisterSignals(movable, list(COMSIG_ITEM_ATTACK, COMSIG_ITEM_ATTACK_OBJ), PROC_REF(on_attack))
		compatible = TRUE
...
/datum/component/anti_magic/proc/on_equip(atom/movable/source, mob/equipper, slot)
	...
	for(var/obj/effect/proc_holder/spell/magic_spell in equipper.actions)
		if(!magic_spell.spell_requirements)
			continue
