...
/datum/component/overlay_lighting/Initialize(_range, _power, _color, is_directional)
	if(!ismovable(parent))
	...
	visible_mask.plane = LIGHTING_VISUAL_PLANE
	visible_mask.appearance_flags = RESET_COLOR | RESET_ALPHA | RESET_TRANSFORM | KEEP_APART | TILE_BOUND
	visible_mask.alpha = 0
	if(is_directional)
		directional = TRUE
		cone = image('icons/effects/light_overlays/light_cone.dmi', icon_state = "light")
		cone.plane = LIGHTING_VISUAL_PLANE
		cone.appearance_flags = RESET_COLOR | RESET_ALPHA | RESET_TRANSFORM | KEEP_APART | TILE_BOUND
		cone.alpha = 110
...
/datum/component/overlay_lighting/RegisterWithParent()
	. = ..()
	if(directional)
		RegisterSignal(parent, COMSIG_ATOM_CHANGE_DIR, .proc/on_parent_dir_change)
...
/datum/component/overlay_lighting/UnregisterFromParent()
	...
	if(directional)
		UnregisterSignal(parent, COMSIG_ATOM_CHANGE_DIR)
...
/datum/component/overlay_lighting/proc/set_holder(atom/movable/new_holder)
	...
	if(current_holder)
		if(current_holder != parent && current_holder != parent_attached_to)
			UnregisterSignal(current_holder, list(COMSIG_PARENT_QDELETING, COMSIG_MOVABLE_MOVED))
			if(directional)
				UnregisterSignal(current_holder, COMSIG_ATOM_CHANGE_DIR)
		if(overlay_lighting_flags & LIGHTING_ON)
			remove_dynamic_lumi()
	...
	if(new_holder != parent && new_holder != parent_attached_to)
		...
		if(directional)
			RegisterSignal(new_holder, COMSIG_ATOM_CHANGE_DIR, .proc/on_holder_dir_change)
	if(overlay_lighting_flags & LIGHTING_ON)
		add_dynamic_lumi()
...
/datum/component/overlay_lighting/proc/on_parent_moved(atom/movable/source, atom/oldLoc, dir)
	SIGNAL_HANDLER
	var/atom/movable/movable_parent = parent
...
/datum/component/overlay_lighting/proc/on_parent_attached_to_qdel(atom/movable/source, force)
	...
	if(directional)
		UnregisterSignal(parent_attached_to, COMSIG_ATOM_CHANGE_DIR)
	if(parent_attached_to == current_holder)
		set_holder(null)
	set_parent_attached_to(null)
...
///Called when current_holder changes loc.
/datum/component/overlay_lighting/proc/on_holder_dir_change(atom/movable/source, newdir)
	SIGNAL_HANDLER
	set_direction(newdir)

///Called when parent changes loc.
/datum/component/overlay_lighting/proc/on_parent_dir_change(atom/movable/source, newdir)
	SIGNAL_HANDLER
	set_direction(newdir)
