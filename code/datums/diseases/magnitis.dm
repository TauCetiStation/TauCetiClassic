...
/datum/disease/magnitis/stage_act()
	...
	switch(stage)
		if(2)
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>Вы чувствуете легкий импульс, проходящий через Ваше тело</span>")
			if(prob(2))
		...
		if(3)
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>Вы чувствуете, как сильный электрический разряд проходит через Ваше тело.</span>")
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>Вам хочется подурачиться.</span>")
			if(prob(4))
		...
		if(4)
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>Вы чувствуете мощнейший электрический разряд, проходящий через Ваше тело.</span>")
			if(prob(2))
				to_chat(affected_mob, "<span class='warning'>Вы размышляете о природе чудес.</span>")
			if(prob(8))
