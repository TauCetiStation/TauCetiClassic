...
/datum/disease/robotic_transformation/stage_act()
	...
	switch(stage)
		if(2)
			...
			if (prob(9))
				to_chat(affected_mob, "<span class='warning'>Буп...бииип...</span>")
		if(3)
			if (prob(8))
			...
			if (prob(4))
				to_chat(affected_mob, "<span class='warning'>Вы чувтсвуете острую боль в голове.</span>")
				affected_mob.Paralyse(2)
			if (prob(4))
				to_chat(affected_mob, "<span class='warning'>Вы чувствуете как внутри вас...что-то движется.</span>")
		if(4)
			if (prob(10))
				to_chat(affected_mob, "<span class='warning'>Ваша оболочка становится эластичной.</span>")
		...
		if(5)
			...
			if(affected_mob.notransform)
				return
			to_chat(affected_mob, "<span class='warning'>Ваша кожа вот-вот отвалится...</span>")
