...
/datum/wires/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	if(!handle_fumbling(L, holder, SKILL_TASK_AVERAGE, SKILL_ENGINEERING, required_skill_proficiency, message_self = "<span class='notice'>You fumble around figuring out the wiring.</span>"))
		return
	switch(action)
		if("cut")
			if(I && iswirecutter(I))
