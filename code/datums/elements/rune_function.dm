...
/datum/element/rune_function/New()
	. = ..()
	if(items_to_create.len == 0)
		init_subtypes(/datum/building_agent/tool/maelstrom, items_to_create)
	if(!uplink_items_image || uplink_items_image.len == 0)
		gen_images()
...
/datum/element/rune_function/proc/on_click(datum/source, mob/user, params)
	...
	if(istype(source, /obj/effect/decal/cleanable/crayon/maelstrom))
		...
		if(istype(power_holder.power, /datum/rune/maelstrom/portal_beacon))
			INVOKE_ASYNC(src, PROC_REF(open_uplink), user, power_holder)
