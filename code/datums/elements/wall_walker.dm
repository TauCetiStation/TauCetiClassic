...
/datum/element/wall_walker/proc/try_pass_wall(mob/living/passing_mob, turf/bumped_wall)
	...
	if(stored_pulling) //force whatever you're pulling to come with you
		stored_pulling.set_dir(get_dir(stored_pulling.loc, passing_mob.loc))
	...
	if(stored_pulling) //don't drop them because we went into a wall
		stored_pulling.set_dir(get_dir(stored_pulling.loc, passing_mob.loc))
		passing_mob.start_pulling(stored_pulling)

/// If the wall is of the proper type, pull whatever you're pulling into it
/datum/element/wall_walker/proc/exit_wall(mob/living/passing_mob, turf/exited_wall)
