/// This animal will not be able to attack a given typecache, and will receive
/// a message when it tries to.
/datum/element/prevent_attacking_of_types
	...
	var/alert_message = "Nuh-uh"

/datum/element/prevent_attacking_of_types/Attach(datum/target, typecache, alert_message)
	. = ..()
	...
	RegisterSignal(target, COMSIG_MOB_PRE_ATTACKINGTARGET, PROC_REF(on_pre_attacking_target))

/datum/element/prevent_attacking_of_types/Detach(datum/source, ...)
	UnregisterSignal(source, COMSIG_MOB_PRE_ATTACKINGTARGET)
	return ..()
