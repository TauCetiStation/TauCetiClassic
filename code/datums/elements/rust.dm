...
/datum/element/rust/Attach(atom/target, rust_icon = 'icons/effects/rust_overlay.dmi', rust_icon_state = "rust_default")
	...
	RegisterSignals(target, list(COMSIG_ATOM_SECONDARY_TOOL_ACT(TOOL_WELDER)), PROC_REF(secondary_tool_act))
...
/datum/element/rust/proc/secondary_tool_act(atom/source, mob/user, obj/item/weapon/C)
	SIGNAL_HANDLER

	INVOKE_ASYNC(src, PROC_REF(handle_tool_use), source, user, C)
	return ITEM_INTERACT_BLOCKING

/// We call this from secondary_tool_act because we sleep with do_after
/datum/element/rust/proc/handle_tool_use(atom/source, mob/user, obj/item/weapon/C)
	if(iswelding(C))
		...
		if(!C.use_tool(source, user, 5 SECONDS))
			return
