...
/datum/beam/proc/Draw()
	...
	var/DX = (WORLD_ICON_SIZE*target.x+target.pixel_x)-(WORLD_ICON_SIZE*origin.x+origin.pixel_x)
	var/DY = (WORLD_ICON_SIZE*target.y+target.pixel_y)-(WORLD_ICON_SIZE*origin.y+origin.pixel_y)
	var/N = 0
	var/length = round(sqrt((DX)**2+(DY)**2)) //hypotenuse of the triangle formed by target and origin's displacement

	for(N in 0 to length-1 step WORLD_ICON_SIZE)//-1 as we want < not <=, but we want the speed of X in Y to Z and step X
		...
		if(DX == 0)
			...
			Pixel_x = round(sin(Angle)+WORLD_ICON_SIZE*sin(Angle)*(N+32)/WORLD_ICON_SIZE)
		if(DY == 0)
			Pixel_y = 0
		else
			Pixel_y = round(cos(Angle)+WORLD_ICON_SIZE*cos(Angle)*(N+32)/WORLD_ICON_SIZE)

		//Position the effect so the beam is one continous line
		var/a
		if(abs(Pixel_x)>WORLD_ICON_SIZE)
			a = Pixel_x > 0 ? round(Pixel_x / WORLD_ICON_SIZE) : CEIL(Pixel_x / WORLD_ICON_SIZE)
			X.x += a
			Pixel_x %= WORLD_ICON_SIZE
		if(abs(Pixel_y)>WORLD_ICON_SIZE)
			a = Pixel_y > 0 ? round(Pixel_y / WORLD_ICON_SIZE) : CEIL(Pixel_y / WORLD_ICON_SIZE)
			X.y += a
