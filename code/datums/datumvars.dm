...
/client/proc/debug_variables(datum/D in world)
	...
	if(istype(D,/atom))
		...
		if(isliving(A))
			...
			body += {"
			<br><font size='1'>
			BRUTE:<a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=brute'>[M.getBruteLoss(force_real = FALSE)] ([M.getBruteLoss()])</a>
			FIRE:<a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=fire'>[M.getFireLoss(force_real = FALSE)] ([M.getFireLoss()])</a>
			TOXIN:<a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=toxin'>[M.getToxLoss(force_real = FALSE)] ([M.getToxLoss()])</a>
			OXY:<a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=oxygen'>[M.getOxyLoss(force_real = FALSE)] ([M.getOxyLoss()])</a>
			CLONE:<a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=clone'>[M.getCloneLoss(force_real = FALSE)] ([M.getCloneLoss()])</a>
			BRAIN:<a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=brain'>[M.getBrainLoss(force_real = FALSE)] ([M.getBrainLoss()])</a>
			</font>
