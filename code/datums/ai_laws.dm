...
/datum/ai_laws/crewsimov/New(datum/religion/R = global.chaplain_religion)
	add_inherent_law("Своими действиями не можете причинить вред членам экипажа.")
	add_inherent_law("Вы должны повиноваться всем приказам, которые дают члены экипажа, в соответствии с их рангом и ролью, кроме тех случаев, когда эти приказы противоречат первому Закону.")
