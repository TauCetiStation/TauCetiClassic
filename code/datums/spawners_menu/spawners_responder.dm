...
/datum/spawner/responders/solgov_osg
	...
	leader_text = "Ты - <B>сержант</B> отряда Охраны Внешних Систем Правительства! Возвращаясь с очередного патруля, вы получили сигнал о помощи и поспешили помочь станции. Вы должны действовать в рамках местного корпоративного закона, а также пытаться нейтрализовать угрозу нелетально. Помните, вы не являетесь корпоратом и находитесь здесь только потому что кому-то из высших чинов доплачивают за помощь в защите этой треклятой станции."
