...
/datum/wires/syndicatebomb/UpdatePulsed(index)
	...
	switch(index)
		if(WIRE_BOOM)
			if (P.active)
				P.loc.visible_message("\red [bicon(holder)] An alarm sounds! It's go-")
				P.timer = 0
		if(WIRE_UNBOLT)
			P.loc.visible_message("\blue [bicon(holder)] The bolts spin in place for a moment.")
		if(WIRE_DELAY)
			playsound(P.loc, 'sound/machines/chime.ogg', 30, 1)
			P.loc.visible_message("\blue [bicon(holder)] The bomb chirps.")
			P.timer += 10
		if(WIRE_PROCEED)
			playsound(P.loc, 'sound/machines/buzz-sigh.ogg', 30, 1)
			P.loc.visible_message("\red [bicon(holder)] The bomb buzzes ominously!")
		...
		if(WIRE_ACTIVATE)
			if(!P.active && !P.defused)
				...
				P.loc.visible_message("\red [bicon(holder)] You hear the bomb start ticking!")
				P.active = 1
	...
	switch(index)
		...
		if(WIRE_UNBOLT)
			if (!mended && P.anchored)
				...
				P.loc.visible_message("\blue [bicon(holder)] The bolts lift out of the ground!")
				P.anchored = 0
		if(WIRE_PROCEED)
			if(!mended && P.active)
				P.loc.visible_message("\red [bicon(holder)] An alarm sounds! It's go-")
		...
		if(WIRE_ACTIVATE)
			if (!mended && P.active)
				P.loc.visible_message("\blue [bicon(holder)] The timer stops! The bomb has been defused!")
