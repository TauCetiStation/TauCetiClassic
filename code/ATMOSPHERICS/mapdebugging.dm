client/verb/discon_pipes()
	...
	for(var/obj/machinery/atmospherics/pipe/simple/P in machines)
		if(!P.node1 || !P.node2)
			to_chat(usr, "[P], [P.x], [P.y], [P.z], [P.loc.loc]")

	for(var/obj/machinery/atmospherics/pipe/manifold/P in machines)
		if(!P.node1 || !P.node2 || !P.node3)
			to_chat(usr, "[P], [P.x], [P.y], [P.z], [P.loc.loc]")

	for(var/obj/machinery/atmospherics/pipe/manifold4w/P in machines)
