...
/obj/item/mecha_parts/mecha_equipment/tool/hydraulic_clamp/action(atom/target)
	...
	if(istype(target,/obj))
		...
		if(!O.anchored)
			...
			playsound(target, 'sound/effects/metal_creaking.ogg', VOL_EFFECTS)
...
/obj/item/mecha_parts/mecha_equipment/tool/extinguisher/action(atom/target) //copypasted from extinguisher. TODO: Rewrite from scratch.
	...
	if(do_after_cooldown(target))
		if(istype(target, /obj/structure/reagent_dispensers/watertank) && get_dist(chassis,target) <= 1)
			...
			playsound(chassis, 'sound/effects/refill.ogg', VOL_EFFECTS, null, null, -6)
		else
			if(src.reagents.total_volume > 0)
				playsound(chassis, 'sound/effects/extinguish.ogg', VOL_EFFECTS, null, null, -3)
...
/obj/item/mecha_parts/mecha_equipment/tool/rcd/action(atom/target)
	...
	playsound(chassis, 'sound/machines/click.ogg', VOL_EFFECTS)
	//meh
	switch(mode)
		if(0)
			if (istype(target, /turf/simulated/wall))
				...
				if(do_after_cooldown(target))
					...
					playsound(target, 'sound/items/Deconstruct.ogg', VOL_EFFECTS)
					chassis.use_power(energy_drain)
		...
		if(2)
			if(istype(target, /turf/simulated/floor))
				...
				if(do_after_cooldown(target))
					...
					T.autoclose = 1
					playsound(target, 'sound/items/Deconstruct.ogg', VOL_EFFECTS)
					playsound(target, 'sound/effects/sparks2.ogg', VOL_EFFECTS)
					chassis.use_power(energy_drain*2)
	return
