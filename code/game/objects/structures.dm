...
/obj/structure/proc/on_climb(mob/living/climber, mob/living/user)
	...
	if(get_turf(climber) == T)
		if(climber == user)
			user.visible_message("<span class='warning'>[user] [flags & ON_BORDER ? "перелезает через" : "залезает на"] [CASE(src, NOMINATIVE_CASE)]!</span>")
		else
			user.visible_message("<span class='warning'>[user] перетаскивает [climber] [flags & ON_BORDER ? "через" : "на"] [CASE(src, NOMINATIVE_CASE)]!</span>")
