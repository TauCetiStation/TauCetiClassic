...
/obj/item/stack/medical/attack(mob/living/carbon/M, mob/user)
	...
	if(istype(M, /mob/living/carbon/human))
		...
		if(affecting.body_zone == BP_HEAD)
			...
			if(affecting.is_bandaged())
				...
				for(var/datum/wound/W in affecting.wounds)
					...
					if(W.current_stage <= W.max_bleeding_stage)
						...
						user.visible_message("<span class='notice'>\The [user] places bruise patch over [W.desc] on [M]'s [affecting.name].</span>", \
											"<span class='notice'>You place bruise patch over [W.desc] on [M]'s [affecting.name].</span>" )
					else
						user.visible_message("<span class='notice'>\The [user] places bandaid over [W.desc] on [M]'s [affecting.name].</span>", \
											"<span class='notice'>You place bandaid over [W.desc] on [M]'s [affecting.name].</span>")
					W.bandage()
					used++
		...
		if(affecting.open == 0)
			if(affecting.is_salved())
				...
				if(!do_mob(user, M, 25))
					...
					return 1
				user.visible_message("<span class='notice'>\The [user] salves wounds on [M]'s [affecting.name].</span>", \
			...
			if(affecting.is_bandaged() && affecting.is_disinfected())
				...
				for(var/datum/wound/W in affecting.wounds)
					...
					if(!do_mob(user, M, W.damage))
						...
						break
					if(W.current_stage <= W.max_bleeding_stage)
						user.visible_message("<span class='notice'>\The [user] cleans [W.desc] on [M]'s [affecting.name] and seals edges with bioglue.</span>", \
											"<span class='notice'>You clean and seal [W.desc] on [M]'s [affecting.name].</span>")
					else if (istype(W,/datum/wound/bruise))
						user.visible_message("<span class='notice'>\The [user] places medicine patch over [W.desc] on [M]'s [affecting.name].</span>", \
											"<span class='notice'>You place medicine patch over [W.desc] on [M]'s [affecting.name].</span>")
					else
		...
		if(affecting.open == 0)
			if(affecting.is_salved())
				...
				if(!do_mob(user, M, 25))
					to_chat(user, "<span class='notice'>You must stand still to salve wounds.</span>")
					return 1
				user.visible_message("<span class='notice'>\The [user] covers wounds on [M]'s [affecting.name] with regenerative membrane.</span>", \
...
/obj/item/stack/medical/splint/attack(mob/living/carbon/M, mob/user)
	...
	if(istype(M, /mob/living/carbon/human))
		...
		if(!((affecting.body_zone == BP_L_ARM) || (affecting.body_zone == BP_R_ARM) || (affecting.body_zone == BP_L_LEG) || (affecting.body_zone == BP_R_LEG)))
		...
		if(M != user)
			...
			if((!user.hand && affecting.body_zone == BP_R_ARM) || (user.hand && affecting.body_zone == BP_L_ARM))
