...
/obj/item/weapon/twohanded/shockpaddles/proc/reset_cooldown()
	if(cooldown)
		...
		playsound(src, 'sound/items/defib_ready.ogg', VOL_EFFECTS, null, FALSE)
...
/obj/item/weapon/twohanded/shockpaddles/proc/try_revive(mob/living/carbon/human/H, mob/user)
	...
	playsound(src, 'sound/items/defib_charge.ogg', VOL_EFFECTS, null, FALSE)

	var/error = can_defib(H)
	if(error)
		make_announcement(error)
		playsound(src, 'sound/items/defib_failed.ogg', VOL_EFFECTS, null, FALSE)
		return
	...
	playsound(src, 'sound/items/defib_zap.ogg', VOL_EFFECTS)
	...
	playsound(src, 'sound/items/defib_success.ogg', VOL_EFFECTS, null, FALSE)
...
/obj/item/weapon/twohanded/shockpaddles/emp_act(severity)
	...
	if(safety != new_safety)
		...
		if(safety)
			...
			playsound(src, 'sound/items/defib_safetyOn.ogg', VOL_EFFECTS, null, FALSE)
		else
			make_announcement("beeps, \"Safety protocols disabled!\"")
			playsound(src, 'sound/items/defib_safetyOff.ogg', VOL_EFFECTS, null, FALSE)
		update_icon()
	..()
