...
/obj/item/weapon/wrapping_paper/attackby(obj/item/I, mob/living/user, params)
	...
	if(I.w_class < SIZE_NORMAL)
		if(user.l_hand.get_quality(QUALITY_CUTTING) || user.r_hand.get_quality(QUALITY_CUTTING) || istype(user.l_hand, /obj/item/weapon/scissors) || istype(user.r_hand, /obj/item/weapon/scissors))
