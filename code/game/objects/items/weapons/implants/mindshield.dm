...
/obj/item/weapon/implant/mind_protect/loyalty/implanted(mob/M)
	...
	if(.)
		...
		to_chat(M, "<span class='nicegreen'>НаноТрейзен - лучшая корпорация во всей Вселенной!</span>")
...
/obj/item/weapon/implant/mind_protect/loyalty/process()
	...
	if(prob(1) && prob(25))//1/400
		switch(rand(1, 4))
			if(1)
				to_chat(imp_in, "<span class='nicegreen'>Вы [pick("уверены", "считаете", "убеждены")], что НаноТрейзен - лучшая корпорация во всей Вселенной!</span>")
			if(2)
				to_chat(imp_in, "<span class='nicegreen'>Вы [pick("уверены", "считаете", "убеждены")], что Капитан - величайший человек, который когда-либо жил!</span>")
			if(3)
				to_chat(imp_in, "<span class='nicegreen'>Вы готовы отдать свою жизнь во славу НаноТрейзен!</span>")
			if(4)
				to_chat(imp_in, "<span class='nicegreen'>Вы уверены в том, что все действия НаноТрейзен приведут к всеобщему благу!</span>")
