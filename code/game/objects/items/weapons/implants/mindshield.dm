...
/obj/item/weapon/implant/mind_protect/implanted(mob/M)
	...
	if(iscultist(H))
		...
		to_chat(H, "<span class='notice'Вы ощущаете чувство покоя и безопасности. Теперь вы защищены от промывания мозгов.</span>")

	if(prob(50) && !H.isSynthetic())
		H.visible_message("[H] внезапно становится очень красным и начинает извиваться. В воздухе появляется странный запах....", \
...
/obj/item/weapon/implant/mind_protect/loyalty/get_data()
	...
	<b>Важные примечания:</b> Персоналу, которому вводится это устройство, как правило, гораздо более лояльны к компании.<BR>
