...
/obj/item/weapon/implant/mind_protect/loyalty/process()
	...
	if(prob(1) && prob(25))//1/400
		switch(rand(1, 10))
			...
			if(2)
				to_chat(imp_in, "<span class='italics'>Вы [pick("уверены", "считаете", "убеждены")], Стандартный порядок действий - это величайшая процедура, которая когда-либо существовала!</span>")
			if(3)
			...
			if(5)
				to_chat(imp_in, "<span class='italics'Вы уверены, что НаноТрейзен - это не работа, это стиль жизни, который нельзя покинуть!</span>")
			if(6)
				to_chat(imp_in, "<span class='italics'Вы считаете, что все ваши мысли принадлежат НаноТрейзен - и это так успокаивает!</span>")
			if(7)
				to_chat(imp_in, "<span class='italics'Вы считаете, что ваша жизнь без НаноТрейзен была бы настолько скучной, что вы бы её и не вспомнили!</span>")
			if(8)
				to_chat(imp_in, "<span class='italics'Вы убеждены, что НаноТрейзен заботится о вас так же, как вы заботитесь о своей любимой кружке!</span>")
			if(9)
				to_chat(imp_in, "<span class='italics'Вы считаете, что работать в НаноТрейзен - это лучшее, что могло с вами случиться!</span>")
			if(10)
				to_chat(imp_in, "<span class='italics'Вы считаете, что ваш трудовой договор с НаноТрейзен - это пожизненная гарантия приключений!</span>")
