...
/obj/item/weapon/implant/mind_protect/loyalty/process()
	...
	if(prob(1) && prob(25))//1/400
		switch(rand(1, 4))
			...
			if(2)
				to_chat(imp_in, "<span class='italics'>Вы [pick("уверены", "считаете", "убеждены")], что Стандартный порядок действий - это величайшая процедура, которое когда-либо существовала!</span>")
			if(3)
