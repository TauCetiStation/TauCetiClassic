...
/obj/item/weapon/implant/mind_protect/loyalty/process()
	...
	if(prob(1) && prob(25))//1/400
		switch(rand(1, 4))
			if(1)
				to_chat(imp_in, "\italic Ты [pick("уверен", "считаешь")], что НаноТрейзен - лучшая корпорация во всей Вселенной!")
			if(2)
				to_chat(imp_in, "\italic Ты [pick("уверен", "считаешь")], что гендиректор НаноТрейзен - самый лучший человек во Вселенной!!")
			if(3)
				to_chat(imp_in, "\italic Ты хочешь отдать свою жизнь во славу НаноТрейзен!")
			if(4)
				to_chat(imp_in, "\italic Ты уверен в том, что все действия Глав Станции приведут НаноТрейзен к успеху!")
