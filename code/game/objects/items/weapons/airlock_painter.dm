/obj/item/weapon/airlock_painter
	...
	desc = "An advanced autopainter preprogrammed with several paintjobs for airlock, windows and pipes. Use it on an airlock during or after construction to change the paintjob, or on window or pipe."
	...
	"Public" = /obj/machinery/door/airlock/neutral,
	"Glass" = /obj/machinery/door/airlock/glass,
	...
	"High Security"= /obj/machinery/door/airlock/highsecurity,
	)
...
/obj/item/weapon/airlock_painter/examine(mob/user)
	...
	if(!ink)
		...
		return
	to_chat(user, "<span class='notice'>Its ink levels is at [ink.charges/ink.max_charges*100] percent .</span>")
...
/obj/item/weapon/airlock_painter/afterattack(atom/target, mob/user, proximity, params)
	if(!proximity || loc != user || !src.can_use(user, 1))
		return
