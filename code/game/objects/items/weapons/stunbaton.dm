...
/obj/item/weapon/melee/baton/attack_self(mob/user)
	if(status && (CLUMSY in user.mutations) && prob(50))
		...
		if(charges < 1)
			...
			update_icon()
		return
	if(!handle_fumbling(user, src, SKILL_TASK_VERY_EASY, SKILL_POLICE, SKILL_POLICE_PRO, SKILL_TASK_TRIVIAL, "<span class='notice'>You fumble around figuring out how to toggle [status ? "on" : "off"] [src]...</span>", "<span class='notice'>[user] fumbles around figuring out how to toggle [!status ? "on" : "off"] [src].</span>"))
		return
	if(charges > 0)
		status = !status
