/obj/item/weapon/woodencrate
	name = "wooden crate"
	cases = list("дерев€нный €щик", "дерев€нного €щика", "дерев€нному €щику", "дерев€нный €щик", "дерев€нным €щиком", "дерев€нном €щике")
	desc = "ѕростой €щик дл€ хранени€ одинаковых предметов."
...
/obj/item/weapon/woodencrate/atom_init()
	...
	if(starttype && startamount)
		...
		desc = "[initial(desc)] —одержит: [I.name]"
...
/obj/item/weapon/woodencrate/attackby(obj/item/weapon/W, mob/user)
	...
	if(open)
		...
		desc = "[initial(desc)] —одержит: [W.name]"
