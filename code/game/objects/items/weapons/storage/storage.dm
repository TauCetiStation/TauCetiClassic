...
/obj/item/weapon/storage/attackby(obj/item/I, mob/user, params)
	...
	if((istype(I, /obj/item/weapon/packageWrap) || istype(I, /obj/item/tagger)) && !(src in user)) //prevents package wrap being put inside the backpack when the backpack is not being worn/held (hence being wrappable)
		return FALSE
