...
/obj/item/toy/gun/afterattack(atom/target, mob/user, flag)
	...
	if (src.bullets < 1)
		...
		return
	playsound(user, 'sound/weapons/guns/Gunshot.ogg', VOL_EFFECTS)
	src.bullets--
...
/obj/item/toy/snappop/throw_impact(atom/hit_atom)
	...
	playsound(src, 'sound/effects/snap.ogg', VOL_EFFECTS)
	qdel(src)
...
/obj/item/toy/prize/attack_self(mob/user)
	if(cooldown < world.time - 8)
		...
		playsound(user, 'sound/mecha/mechstep.ogg', VOL_EFFECTS, 20)
		cooldown = world.time
...
/obj/item/toy/prize/attack_hand(mob/user)
	if(loc == user)
		if(cooldown < world.time - 8)
			to_chat(user, "<span class='notice'>You play with [src].</span>")
			playsound(user, 'sound/mecha/mechturn.ogg', VOL_EFFECTS, 20)
			cooldown = world.time
			return
	..()
...
/obj/item/toy/figure/attack_self(mob/user)
	if(cooldown <= world.time)
		...
		playsound(user, 'sound/machines/click.ogg', VOL_EFFECTS, 20)
...
/obj/item/toy/nuke/attack_self(mob/user)
	if (cooldown < world.time)
		...
		spawn(5) //gia said so
			icon_state = "nuketoy"
			playsound(src, 'sound/machines/Alarm.ogg', VOL_EFFECTS, null, FALSE)
			sleep(135)
...
/obj/item/toy/cards/attack_self(mob/user)
	...
	playsound(user, 'sound/items/cardshuffle.ogg', VOL_EFFECTS)
...
/obj/item/toy/carpplushie/attack(mob/M, mob/user)
	. = ..()
	playsound(src, bitesound, VOL_EFFECTS, 20)
...
/obj/item/toy/plushie/attack(mob/M, mob/user)
	. = ..()
	playsound(src, poof_sound, VOL_EFFECTS, 20) // Play the whoosh sound in local area
...
/obj/item/toy/plushie/attack_self(mob/user)
	if(next_hug < world.time)
		...
		playsound(src, poof_sound, VOL_EFFECTS)
...
/obj/item/toy/eight_ball/attack_self(mob/user)
	if(last_time_used < world.time)
		...
		if(answer_sound)
			playsound(src, answer_sound, VOL_EFFECTS, 20)
		return
	else
...
/obj/item/toy/eight_ball/conch/attack_self(mob/user)
	. = ..()
	flick("conch_use",src)
	playsound(src, 'sound/items/polaroid2.ogg', VOL_EFFECTS, 20)
...
/obj/item/toy/moocan/attack_self(mob/user)
	if(!cooldown)
		...
		playsound(user, 'sound/items/moo.ogg', VOL_EFFECTS, 20)
