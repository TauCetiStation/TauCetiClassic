...
/obj/item/device/traitor_caller/attack_self(mob/user)
	...
	for(var/mob/living/carbon/human/player in player_list)
		...
		&& (TRAITOR in player.client.prefs.be_role) && !jobban_isbanned(player, "Syndicate") \
		&& !jobban_isbanned(player, TRAITOR) && !role_available_in_minutes(player, TRAITOR) && !isloyal(player))
