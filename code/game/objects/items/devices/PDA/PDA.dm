...
/obj/item/device/pda/proc/create_message(mob/living/U = usr, obj/item/device/pda/P, tap = 1)
	...
	if(useMS && useTC) // only send the message if it's stable
		...
		tnote.Add(list(list("sent" = 1, "owner" = "[P.owner]", "job" = "[P.ownjob]", "message" = "[t]", "sender" = "[sanitize(src.owner)]", "reciever" = "[sanitize(P.owner)]", "target" = "\ref[P]")))
		P.tnote.Add(list(list("sent" = 0, "owner" = "[owner]", "job" = "[ownjob]", "message" = "[t]", "sender" = "[sanitize(P.owner)]", "reciever" = "[sanitize(src.owner)]",  "target" = "\ref[src]")))
