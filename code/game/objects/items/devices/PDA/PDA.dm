	...
	if (istype(C, /mob/living/carbon))
		switch(scanmode)
			if(1)
				...
				var/has_oxy_damage = (C.getOxyLoss(force_real = FALSE) > 50)
				var/has_tox_damage = (C.getToxLoss(force_real = FALSE) > 50)
				var/has_fire_damage = (C.getFireLoss(force_real = FALSE) > 50)
				var/has_brute_damage = (C.getBruteLoss(force_real = FALSE) > 50)
				user.show_message("<span class='notice'>&emsp; Damage Specifics: <span class='[has_oxy_damage ? "warning" : "notice"]'>[C.getOxyLoss(force_real = FALSE)]</span>-<span class='[has_tox_damage ? "warning" : "notice"]'>[C.getToxLoss(force_real = FALSE)]</span>-<span class='[has_fire_damage ? "warning" : "notice"]'>[C.getFireLoss(force_real = FALSE)]</span>-<span class='[has_brute_damage ? "warning" : "notice"]'>[C.getBruteLoss(force_real = FALSE)]</span></span>", 1)
