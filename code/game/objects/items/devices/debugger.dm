...
/obj/item/device/debugger/afterattack(obj/O, mob/user, proximity)
	...
	if(istype(O, /obj/machinery/power/apc))
		...
		if(!D.density)
			...
			if(use_tool(D, user, 30) && is_used_on(D,user))
				to_chat(user, "<span class='notice'>My time has come, please wait... Starting HackHim3000...</span>")
				if(use_tool(D, user, 20))
					to_chat(user, "<span class='notice'>Faster than light, please wait... Hack in progress...</span>")
					if(use_tool(D, user, 100) && D.density && !(D.operating == -1))
						flick("door_spark", D)
						sleep(6)
						D.open()
						D.operating = -1
