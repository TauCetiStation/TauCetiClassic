...
/obj/structure/girder/attackby(obj/item/W, mob/user)
	...
	else if(W.get_quality(QUALITY_WRENCH) && state == 0)
	...
	else if(W.get_quality(QUALITY_SCREWING) && state == 2 && istype(src,/obj/structure/girder/reinforced))
	...
	else if(W.get_quality(QUALITY_CUTTING) && istype(src,/obj/structure/girder/reinforced) && state == 1)
	...
	else if(W.get_quality(QUALITY_PRYING) && state == 0 && anchored )
