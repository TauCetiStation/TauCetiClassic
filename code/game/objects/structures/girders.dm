			...
			if(/obj/item/stack/sheet/plasteel)
				if(!anchored)
					...
					if (istype (src, /obj/structure/girder/reinforced))
						...
						if(S.use_tool(src, user, SKILL_TASK_AVERAGE, amount = 1, volume = 100, required_proficiency = SKILL_CONSTRUCTION_ADVANCED))
						...
						if(S.use_tool(src, user, 60, amount = 1, volume = 100, required_proficiency = SKILL_CONSTRUCTION_ADVANCED))
