...
/obj/structure/door_assembly/attackby(obj/item/W, mob/user)
	...
	if(W.get_quality(QUALITY_WELDING) && ((glass_material && !glass_only) || mineral || !anchored))
	...
	else if(W.get_quality(QUALITY_WRENCH) && state == ASSEMBLY_SECURED)
	...
	else if(W.get_quality(QUALITY_CUTTING) && state == ASSEMBLY_WIRED)
		if(user.is_busy()) return
	...
	else if(W.get_quality(QUALITY_PRYING) && state == ASSEMBLY_NEAR_FINISHED)
	...
	else if(W.get_quality(QUALITY_SCREWING) && state == ASSEMBLY_NEAR_FINISHED )
		if(user.is_busy(src))
			return
