...
/obj/structure/door_assembly/attackby(obj/item/W, mob/user)
	...
	if(iswelder(W) && ((glass_material && !glass_only) || mineral || !anchored))
		...
		if(glass_material)
			...
			if(WT.use_tool(src, user, 40 * skill_bonus, volume = 50))
		...
		if(coil.use_tool(src, user, 40 * skill_bonus, amount = 1, volume = 50))
		...
		if(W.use_tool(src, user, 40 * skill_bonus, volume = 50))
		...
		if(S.get_amount() >= 1)
			if(istype(S, /obj/item/stack/sheet/rglass))
				if(glass_material)
					...
					if(W.use_tool(src, user, 40 * skill_bonus, amount = 1, volume = 100))
