...
/obj/structure/windoor_assembly/attackby(obj/item/W, mob/user)
	...
	switch(state)
		if("01")
			if(W.get_quality(QUALITY_WELDING) && !anchored )
			...
			if(W.get_quality(QUALITY_WRENCH) && !anchored)
			...
			else if(W.get_quality(QUALITY_WRENCH) && anchored)
		...
		if("02")
			...
			if(W.get_quality(QUALITY_CUTTING) && !src.electronics)
