...
/obj/effect/effect/aqueous_foam/Crossed(atom/movable/AM)
	...
	if(isliving(AM))
		...
		if(HAS_TRAIT(L, TRAIT_SLIME)) // Slimes are vulnerable to us and shouldn't be able to destroy us.
			L.Weaken(5)
...
/obj/effect/effect/aqueous_foam/proc/performAction()
	...
	for(var/atom/A in perform_on)
		if(isliving(A))
			var/mob/living/L = A
			if(HAS_TRAIT(L, TRAIT_SLIME)) // If only ExtinguishMob wasn't so vague, this could be there.
				L.adjustToxLoss(rand(15, 20))
			L.ExtinguishMob()
