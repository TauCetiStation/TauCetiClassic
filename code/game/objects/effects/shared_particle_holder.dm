...
/obj/effect/abstract/shared_particle_holder/atom_init(mapload, particle_path = /particles/smoke, particle_flags = NONE)
	. = ..()
...
/atom/movable/proc/add_shared_particles(particle_type, custom_key = null, particle_flags = NONE, pool_size = 3)
	...
	if (!global.shared_particles[particle_key])
		global.shared_particles[particle_key] = list(list(new /obj/effect/abstract/shared_particle_holder(null, particle_type, particle_flags)), 1)
		vis_contents += global.shared_particles[particle_key][SHARED_PARTICLE_HOLDER_INDEX][1]
		return global.shared_particles[particle_key][SHARED_PARTICLE_HOLDER_INDEX][1]

	var/list/type_holders = global.shared_particles[particle_key][SHARED_PARTICLE_HOLDER_INDEX]
	...
	if (length(type_holders) < pool_size)
		...
		global.shared_particles[particle_key][SHARED_PARTICLE_USER_NUM_INDEX] += 1
		return new_holder
...
/atom/movable/proc/remove_shared_particles(particle_key, delete_on_empty = TRUE)
	...
	for (var/obj/effect/abstract/shared_particle_holder/particle_holder as anything in type_holders)
		...
		if (delete_on_empty && global.shared_particles[particle_key][SHARED_PARTICLE_USER_NUM_INDEX] <= 0)
			QDEL_LIST(type_holders)
			global.shared_particles -= particle_key
		return
