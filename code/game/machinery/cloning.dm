...
/obj/item/weapon/disk/data/demo
	...
	cases = list("диск с информацией о Императоре Человечества", "диска с информацией о Императоре Человечества", "диску с информацией о Императоре Человечества", "диск с информацией о Императоре Человечества", "диском с информацией о Императоре Человечества", "диске с информацией о Императоре Человечества")
	read_only = 1
...
/obj/item/weapon/disk/data/monkey
	...
	cases = list("диск с информацией о Мистере Мугглесе", "диска с информацией о Мистере Мугллесе", "диску с информацией о Мистере Мугллесе", "диск с информацией о Мистере Муглессе", "диском с информацией о Мистере Муглессе", "диске с информацией о Мистере Муглессе")
	read_only = 1
...
/obj/machinery/clonepod/proc/growclone(datum/dna2/record/R)
	...
	H.ckey = R.ckey
	to_chat(H, "<span class='notice'><b>Разум медленно восстанавливается в ваше регенерирующее тело...</b><br><i>Так вот, как ощущается клонирование...</i></span>")
...
/obj/machinery/clonepod/process()
	...
	if((src.occupant) && (src.occupant.loc == src))
		...
		if((src.occupant.stat == DEAD) || (src.occupant.suiciding) || !occupant.key)  //Autoeject corpses and suiciding dudes.
			src.locked = 0
			go_out()
			connected_message("Клон неисправен: сгнил.")
			return
...
/obj/machinery/clonepod/attackby(obj/item/weapon/W, mob/user)
	...
	if (istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
		...
		to_chat(user, "<span class='notice'>[CASE(src, NOMINATIVE_CASE)] обрабатывает [W].</span>")
		biomass += 50
		qdel(W)
		return
	else
...
/obj/machinery/clonepod/emag_act(mob/user)
	...
	to_chat(user, "Вы активировали экстренное извлечение оболочки.")
	src.locked = 0
	go_out()
	return TRUE
...
/obj/item/weapon/paper/Cloning
	name = "Мануал для использования клонирующего аппарата H-87"
	info = {"<h4>Подготовка</h4>
	Поздравляем, ваша станция закупила аппарат H-87!<br>
	Использование H-87 такое же простое, как и нейрохирургия! Просто вставьте гуманоидную оболочку в капсулу, просканируйте её и создайте новый профиль<br>
	<b>Это всё, что нужно сделать!</b><br>
	<i>Сообщаем вам, что аппарат не умеет клонировать мартышек и прочих маленьких созданий, включая и неорганических. До такого мы пока не дошли. Гематомы могут привести к ошибкам в сканировании.</i><br>
	<p>Профили для клонирования можно посмотреть в специальной для этого вкладке. При сканировании, в органика вживляется специальный имплант, информацию из которого можно получить в профиле органика.
	Удаление генетического профиля возможно лишь с доступом \[глав станции\]</p>
	<h4>Клонирование из профиля</h4>
	Для клонирования, нужно нажать кнопку "Клонировать" на выбранном профиле<br>
	Мы соблюдаем соглашение о конфиденциальности сотрудников вашей компании, поэтому клонирование живых членов экипажа аппаратом H-87 невозможно.<br>
	<br>
	<p>Система капсулы для клонирования воспроизводит практически идеального клона субьекта из профиля за 90 секунд.
	...
	Эти дискеты передаются для передачи генетической информации.
	Диалог загрузки или отправки возможно, как вы вставите дискету в аппарат.</p><br>
