			...
			switch(href_list["command"])
				...
				if( "power",
					...
					if(href_list["command"] == "adjust_external_pressure")
						...
						log_investigate("[usr.key] has changed adjust_external_pressure > added [new_val], id_tag = [device_id]",INVESTIGATE_ATMOS)
					if(href_list["command"] == "checks")
						var/new_val = text2num(href_list["val"])
						log_investigate("[usr.key] has changed pressure_checks > now [new_val](1 = ext, 2 = int, 3 = both), id_tag = [device_id]",INVESTIGATE_ATMOS)
