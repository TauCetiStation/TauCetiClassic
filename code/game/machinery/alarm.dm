			...
			switch(href_list["command"])
				if( "power",
					...
					if(href_list["command"] == "adjust_external_pressure")
						...
						msg_admin_attack("[key_name(usr)] messing with sensors atmosphere pressure. \red adjust_external_pressure > adjust_external_pressure > = add [new_val]")
					...
					if(href_list["command"] == "checks")
						...
						usr.attack_log += "\[[time_stamp()]\]<font color='red'> [key_name(usr)] messing with sensors pressure checks. checks > = now [new_val]</font>"
				if("set_threshold")
