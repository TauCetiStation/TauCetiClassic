...
/obj/machinery/vending/cigarette/vend(datum/data/vending_product/R, mob/user)
	if(ishuman(user))
		...
		if(H.age < 21)
			...
			radio.autosay("[H.name] попытал[H.gender == "female" ? "ась" : "ся"] приобрести сигареты не достигнув возраста 21 года. Необходимо провести воспитательную беседу в [loc.loc.name].", "[name]", freq = radiochannels["Security"])
			flick(icon_deny, src)
		else
			..()
