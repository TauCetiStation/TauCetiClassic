...
/mob/camera/Eye/remote/atom_init()
	if(!user_image && user_camera_icon)
		user_image = image(icon = user_camera_icon,loc = src,icon_state = icon_state,layer = FLY_LAYER)
	. = ..()
...
/mob/camera/Eye/remote/engineering
...
/mob/camera/Eye/remote/security
