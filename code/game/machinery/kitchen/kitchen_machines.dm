...
/obj/machinery/kitchen_machine/attackby(obj/item/O, mob/user)
	...
	if(src.broken > 0)
		if(src.broken == 2 && O.get_quality(QUALITY_CUTTING)) // If it's broken and they're using a wirecutters.
		...
		else if(src.broken == 1 && O.get_quality(QUALITY_WELDING) && !user.is_busy(src)) // If it's broken and they're doing the weldingtool.
			user.visible_message( \
				"<span class='notice'>[user] starts to fix part of the [src].</span>", \
				"<span class='notice'>You start to fix part of the [src].</span>" \
			)
