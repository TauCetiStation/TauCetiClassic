...
/obj/machinery/computer/arrival_shuttle/dock
	...
	cases = list("консоль связи трансферного шаттла", "консоли связи трансферного шаттла", "консоле связи трансферного шаттла", "консоль связи трансферного шаттла", "консолью связи трансферного шаттла", "консоле связи трансферного шаттла")
	icon = 'icons/obj/computer.dmi'
	icon_state = "wagon"
...
/obj/machinery/computer/arrival_shuttle/dock/ui_interact(user)
	var/seconds = max(round((lastMove + ARRIVAL_SHUTTLE_COOLDOWN - world.time) * 0.1), 0)
	var/seconds_word = pluralize_russian(seconds, "секунду", "секунды", "секунд")
	var/dat = "<center>Местоположение:[capitalize(CASE(curr_location, NOMINATIVE_CASE))]<br>Готов к полёту[!arrival_shuttle_ready_move() ? " через [seconds] [seconds_word]" : ": сейчас"]<br><b><A href='?src=\ref[src];back=1'>Запросить шаттл обратно</A></b></center><br>"
