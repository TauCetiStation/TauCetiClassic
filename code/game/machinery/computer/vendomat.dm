...
/obj/machinery/computer/vendomat/tgui_data(mob/user)
	...
	var/list/vendomats_hashed_icons = list()
	for(var/obj/machinery/vending/Vend in global.vending_machines)
		if(is_station_level(Vend.z))
			...
			vendomats_hashed[Vend.name] += list(list("area" = A.name, "integrity" = Vend.atom_integrity, "max_integrity" = Vend.max_integrity, "load" = Vend.load, "max_load" = Vend.max_load))
