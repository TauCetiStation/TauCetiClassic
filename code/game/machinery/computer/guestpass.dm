...
/obj/item/weapon/card/id/guest/examine(mob/user)
	. = ..()
	if(world.time < expiration_time)
		. += "<span class='notice'>This pass expires at [time_stamp("hh:mm:ss", expiration_time)].</span>"
	else
		. += "<span class='warning'>It expired at [time_stamp("hh:mm:ss", expiration_time)].</span>"
...
/obj/machinery/computer/guestpass/ui_interact(mob/user)
	...
	if (mode == 1) //Logs
		...
		dat += "Issuing ID: <a href='?src=\ref[src];action=id'>[scan]</a><br>"
		...
		switch(href_list["action"])
			...
			if ("issue")
				if (scan)
					...
					pass.reason = reason
					pass.issuedby = scan.registered_name
					pass.name = "guest pass #[number]"
