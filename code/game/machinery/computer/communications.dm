...
/proc/call_shuttle_proc(mob/user)
	...
	if(world.time < 6000) // Ten minute grace period to let the game get going without lolmetagaming. -- TLE
		to_chat(user, "Шаттл находится на дозаправке. Пожалуйста, подождите еще [round((6000-world.time)/600)] [pluralize_russian(minute, "минута", "минуты", "минут")] до повторного вызова.")
		return
