...
/obj/machinery/life_assist/artificial_ventilation/Destroy()
	if(attached)
		detach()
	return ..()
...
/obj/machinery/life_assist/artificial_ventilation/proc/detach(mob/living/carbon/human/attached)
...
/obj/machinery/life_assist/artificial_ventilation/proc/av_resolve_stranded(datum/component/bounded/bounds)
	if(get_dist(bounds.bound_to, src) == 2 && !anchored)
		...
		if(dist >= bounds.min_dist && dist <= bounds.max_dist)
			return TRUE
	var/obj/machinery/life_assist/artificial_ventilation/AV = src
	AV.detach(bounds.bound_to)
	return TRUE


/obj/machinery/life_assist/artificial_ventilation/process()
	if(attached)
...
/obj/machinery/life_assist/cardiopulmonary_bypass/update_icon()
	if(attached)
		icon_state = "cpb_pumping"
	else
		icon_state = "cpb_idle"

/obj/machinery/life_assist/cardiopulmonary_bypass/Destroy()
	if(attached)
		detach()
	return ..()

/obj/machinery/life_assist/cardiopulmonary_bypass/MouseDrop(over_object, src_location, over_location)
	..()
	if(!iscarbon(usr) && !isrobot(usr))
		return
	if(!in_range(src, usr) || !in_range(over_object, src))
...
/obj/machinery/life_assist/cardiopulmonary_bypass/proc/detach(mob/living/carbon/human/attached)
...
/obj/machinery/life_assist/cardiopulmonary_bypass/proc/cpb_resolve_stranded(datum/component/bounded/bounds)
	if(get_dist(bounds.bound_to, src) == 2 && !anchored)
		step_towards(src, bounds.bound_to)
		var/dist = get_dist(src, get_turf(bounds.bound_to))
		if(dist >= bounds.min_dist && dist <= bounds.max_dist)
			return TRUE
	var/obj/machinery/life_assist/cardiopulmonary_bypass/CPB = src
