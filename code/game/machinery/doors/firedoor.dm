...
/obj/machinery/door/firedoor/attackby(obj/item/weapon/C, mob/user)
	...
	if(istype(C, /obj/item/weapon/weldingtool))
		...
		if(W.remove_fuel(0, user))
			blocked = !blocked
			user.visible_message("<span class=warning'>\The [user] [blocked ? </span>"welds" : "unwelds"] \the [src] with \a [W].",\
	...
	if( istype(C, /obj/item/weapon/crowbar) || ( istype(C,/obj/item/weapon/twohanded/fireaxe) && C:wielded == 1 ) )
		...
		user.visible_message("<span class=warning'>\The [user] starts to force \the [src] [density ? </span>"open" : "closed"] with \a [C]!",\
				"You start forcing \the [src] [density ? "open" : "closed"] with \the [C]!",\
				"You hear metal strain.")
		if(do_after(user,30,target = src))
			if( istype(C, /obj/item/weapon/crowbar) )
				if( stat & (BROKEN|NOPOWER) || !density)
					user.visible_message("<span class=warning'>\The [user] forces \the [src] [density ? </span>"open" : "closed"] with \a [C]!",\
					"You force \the [src] [density ? "open" : "closed"] with \the [C]!",\
					"You hear metal strain, and a door [density ? "open" : "close"].")
			else
				user.visible_message("<span class=warning'>\The [user] forces \the [ blocked ? </span>"welded" : "" ] [src] [density ? "open" : "closed"] with \a [C]!",\
