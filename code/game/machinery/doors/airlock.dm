...
/obj/machinery/door/airlock/proc/bolt()
	if(locked)
		return
	locked = 1
	playsound(src, door_bolt_down_sound, VOL_EFFECTS, 30, FALSE, 3)
	update_icon()
...
/obj/machinery/door/airlock/proc/unbolt()
	if(!locked)
		return
	locked = 0
	playsound(src, door_bolt_up_sound, VOL_EFFECTS, 30, FALSE, 3)
	update_icon()
	...
	switch(animation)
		...
		if("deny")
			if(deny_animation_check())
				...
				playsound(src, door_deni_sound, VOL_EFFECTS, 40, FALSE, 3)
				sleep(6)
...
/obj/machinery/door/airlock/attack_paw(mob/user)
	if(istype(user, /mob/living/carbon/alien/humanoid))
		if(welded || locked)
			...
			playsound(src, door_forced_sound, VOL_EFFECTS, 30, null, -4)
