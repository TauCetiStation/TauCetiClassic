...
/obj/item/weapon/book/tome/attack_self(mob/living/carbon/human/user)
	...
	for(var/obj/structure/obj_to_check in user.loc)
		if(obj_to_check.density)
			...
			return
	if (length(cult_runes) >= CULT_RUNES_LIMIT + length(global.cult_religion.members)) //including the useless rune at the secret room, shouldn't count against the limit of 25 runes - Urist
