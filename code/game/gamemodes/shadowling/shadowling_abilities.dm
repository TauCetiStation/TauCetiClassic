	...
	for(var/turf/T in targets)
		...
		for(var/mob/living/carbon/human/H in T.contents)
			...
			for(var/obj/item/clothing/head/helmet/space/rig/R in H)
				if(R.on)
					R.on = 0
					R.icon_state = "rig[R.on]-[R.item_color]"
					H.update_inv_head()
...
/obj/effect/proc_holder/spell/targeted/collective_mind/cast(list/targets)
	for(var/mob/living/user in targets)
		...
		if(thralls >= 5 && !drain_thrall_acquired)
			...
			user.spell_list += new /obj/effect/proc_holder/spell/aoe_turf/drain_thralls
		...
		if(thralls >= 9 && !thrall_swap_acquired)
			...
			user.spell_list += new /obj/effect/proc_holder/spell/targeted/spatial_relocation
