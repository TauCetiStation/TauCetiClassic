...
/mob/living/silicon/ai/proc/takeover()
	...
	if(length(SSticker.hacked_apcs) < APC_MIN_TO_MALF_DECLARE)
		to_chat(src,"<span class='red'>You don't have enough hacked APCs to take over the station yet. You need to hack at least [APC_MIN_TO_MALF_DECLARE], however hacking more will make the takeover faster. You have hacked [length(SSticker.hacked_apcs)] APCs so far.</span>")
		return
	if(SSticker.malf_announce_stage < 4)
		if(tgui_alert(src, "Are you sure you wish to initiate the takeover? The station hostile runtime detection software is bound to alert everyone. You have hacked [length(SSticker.hacked_apcs)] APCs.", "Takeover", list("Yes", "No")) != "Yes")
			return
