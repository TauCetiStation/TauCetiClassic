...
/mob/living/silicon/ai/proc/takeover()
	...
	if(global.hacked_apcs.len < APC_MIN_TO_MALF_DECLARE)
		to_chat(src,"<span class='red'>You don't have enough hacked APCs to take over the station yet. You need to hack at least 5, however hacking more will make the takeover faster. You have hacked [global.hacked_apcs.len] APCs so far.</span>")
		return
	if(SSticker.Malf_announce_stage < 4)
		if(tgui_alert(src, "Are you sure you wish to initiate the takeover? The station hostile runtime detection software is bound to alert everyone. You have hacked [global.hacked_apcs.len] APCs.", "Takeover", list("Yes", "No")) != "Yes")
			return
...
/mob/living/silicon/ai/proc/infest()
	...
	for(var/mob/living/carbon/human/H in view(2, eyeobj.loc))
		if(!H.mind || !H.key)
			continue
		if(global.hacked_apcs.len < APC_MIN_TO_MALF_DECLARE)
			to_chat(src, "<span class='warning'>Infest Module are not ready. Need hack a [APC_MIN_TO_MALF_DECLARE - global.hacked_apcs.len] APC more.</span>")
			return
