/obj/item/clothing/suit/space/space_ninja/attackby(obj/item/I, mob/user, params)
	if(user == affecting)//Safety, in case you try doing this without wearing the suit/being the person with the suit.
		if(istype(I, /obj/item/device/aicard))//If it's an AI card.
			...
			return FALSE//Return individually so that ..() can run properly at the end of the proc.
		if(istype(I, /obj/item/device/paicard) && !pai)//If it's a pai card.
		...
		if(istype(I, /obj/item/weapon/stock_parts/cell))
			...
			return FALSE
		if(istype(I, /obj/item/weapon/disk/tech_disk))//If it's a data disk, we want to copy the research on to the suit.
