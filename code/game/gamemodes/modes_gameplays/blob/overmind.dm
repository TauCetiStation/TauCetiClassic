...
/mob/camera/blob/proc/blob_help()
	...
	to_chat(src, "<b>Укреплённый блоб</b> является укрепленной версией нормального блоба. Он невосприимчив к огню, используйте это в свою пользу. Улучшите эту часть снова, чтобы получить рефлективную версию, способную отражать лазеры.")
	...
	to_chat(src, "<b>Узел блоба</b> подобно ядру, может расширяться. Однако, оно не повышает производство ресурсов, но может повысить эффективность ближних к ней ячеек")
	to_chat(src, "<b>Производящая ячейка</b> является производителем спор. Установка данного вида у узла блоба повысит его эффективность ; если расположить его очень далеко, то споры воспроизводиться не будут.")
	to_chat(src, "<b>Блоббернауты</b> могут быть произведены из производящей ячейки, их тяжело убить, они сильно бьют и обладают средней мобильностью. Ячейка, которая была использована для производства единицы, временно не сможет производить споры и станет более хрупкой.")
