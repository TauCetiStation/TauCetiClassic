...
/mob/camera/blob/proc/blob_help()
	...
	to_chat(src, "<b>Блоббернауты</b> могут быть произведены из производящей ячейки, их тяжело убить, они сильно бьют и обладают средней мобильностью. Ячейка, который был использована для производства единицы, временно не сможет производить споры и станет более хрупкой.")
