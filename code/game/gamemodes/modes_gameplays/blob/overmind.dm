...
/mob/camera/blob/proc/blob_help()
	...
	to_chat(src, "<b>Ресурсная ячейка</b> является основным способом добычи ресурсов, постройте их как можно больше в начале, чтобы всегда иметь достаточно ресурсов. Она улучшается от нахождения рядом с ядром или узлом; если расположить её очень далеко, то ресурсы воспроизводиться не будут.")
