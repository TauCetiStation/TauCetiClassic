...
/mob/camera/blob/proc/blob_help()
	...
	to_chat(src, "<b>Производящая ячейка</b> создаёт споры. Установка данного вида у узла блоба повысит его эффективность ; если расположить его очень далеко, то споры воспроизводиться не будут.")
