...
/datum/heretic_knowledge/hunt_and_sacrifice/on_research(mob/user, datum/role/heretic/our_heretic)
	. = ..()
 ...
 */
/datum/heretic_knowledge/hunt_and_sacrifice/proc/obtain_targets(mob/living/user, silent = FALSE, datum/role/heretic/heretic_datum)
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/sacrifice_process(mob/living/user, list/selected_atoms, turf/loc)
	...
	var/datum/role/cultist/cultist_datum = GET_CULTIST(sacrifice)
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/grant_reward(mob/living/user, mob/living/sacrifice, turf/loc)
	...
	var/datum/role/heretic/antag = GET_HERETIC(user)
	antag.rewards_given++
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_target_sleeps(mob/living/carbon/human/sac_target, turf/destination)
	...
	RegisterSignal(sac_target, COMSIG_MOB_DIED, PROC_REF(on_target_death)) // Loss condition
...
/**
 * This proc is called from [proc/begin_sacrifice] if the target survived the shadow realm), or [COMSIG_MOB_DIED] if they don't.
 *
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/return_target(mob/living/carbon/human/sac_target)
	...
	UnregisterSignal(sac_target, COMSIG_MOB_DIED)
...
/datum/heretic_knowledge/hunt_and_sacrifice/proc/after_return_live_target(mob/living/carbon/human/sac_target)
	...
	if(isheretic(sac_target))
		SEND_SIGNAL(sac_target, COMSIG_ADD_MOOD_EVENT, "shadow_realm_survived_sadness", /datum/mood_event/shadow_realm_live_sad_heretic)
	else
