...
/datum/heretic_knowledge/spell/basic
	...
	action_to_add = /obj/effect/proc_holder/spell/in_hand/mansus_grasp
	cost = 0
...
/datum/heretic_knowledge/spell/basic/proc/on_grasp_cast(mob/living/carbon/cast_on, obj/effect/proc_holder/spell/in_hand/touch_spell)
	SIGNAL_HANDLER
...
/datum/heretic_knowledge/living_heart/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	playsound(user, 'sound/effects/demon_consume.ogg', 50, TRUE)
	return TRUE
...
/datum/heretic_knowledge/codex_cicatrix/cleanup_atoms(list/selected_atoms)
	...
	if(!le_book)
		stack_trace("Somehow, no book in codex cicatrix selected atoms! [get_english_list(selected_atoms)]")
