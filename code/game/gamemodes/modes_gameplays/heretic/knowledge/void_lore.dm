...
/datum/heretic_knowledge/void_grasp/proc/on_mansus_grasp(mob/living/source, mob/living/target)
	...
	carbon_target.silent += 10 SECONDS
...
/datum/heretic_knowledge/spell/void_conduit
	...
	action_to_add = /obj/effect/proc_holder/spell/conjure/void_conduit
	cost = 1
...
/datum/heretic_knowledge/blade_upgrade/void/do_melee_effects(mob/living/source, mob/living/target, obj/item/weapon/sickly_blade/blade)
	if(source == target || !isliving(target))
		return

	target.apply_status_effect(/datum/status_effect/void_chill, 2)

/datum/heretic_knowledge/blade_upgrade/void/do_ranged_effects(mob/living/user, mob/living/target, obj/item/weapon/sickly_blade/blade)
...
/datum/heretic_knowledge/blade_upgrade/void/proc/follow_up_attack(mob/living/user, mob/living/target, obj/item/weapon/sickly_blade/blade)
...
/datum/heretic_knowledge/ultimate/void_final
	...
	announcement_text = "%SPOOKY% Несущий благородство пустоты %NAME% прибыл, и он откроет бал, где будут и танцы, и песни, и смерть этому миру! %SPOOKY%"
...
/datum/heretic_knowledge/ultimate/void_final/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	. = ..()
	user.add_traits(list(TRAIT_RESISTLOWPRESSURE, TRAIT_NEGATES_GRAVITY, TRAIT_ARIBORN_FLYING), type)
...
/**
 * Signal proc for [COMSIG_MOB_DIED].
 *
