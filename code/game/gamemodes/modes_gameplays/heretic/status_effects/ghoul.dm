...
/datum/status_effect/ghoul/on_apply()
	...
	RegisterSignal(human_target, COMSIG_MOB_DIED, PROC_REF(remove_ghoul_status))
	...
	if(human_target.mind)
		var/datum/role/heretic_monster/heretic_monster = human_target.mind.add_antag_datum(/datum/role/heretic_monster)
		heretic_monster.set_owner(master_mind)
		human_target.mind.remove_antag_datum(/datum/role/cultist)
...
/datum/status_effect/ghoul/proc/remove_ghoul_status(datum/source)
	...
	UnregisterSignal(human_target, COMSIG_MOB_DIED)
	if(!QDELETED(src))
		qdel(src)
