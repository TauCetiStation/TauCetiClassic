...
/obj/structure/trap/eldritch/on_entered(datum/source, atom/movable/entering_atom)
	...
	if(WEAKREF(living_mob) == owner)
		return
	if(ishereticormonster(living_mob))
		return
	return ..()
...
/obj/structure/trap/eldritch/attacked_by(obj/item/weapon, mob/living/user)
	if(istype(weapon, /obj/item/weapon/rune_carver) || istype(weapon, /obj/item/weapon/nullrod))
		...
		playsound(src, 'sound/items/sheath.ogg', 50, TRUE, extrarange = -9, ignore_walls = FALSE)
		qdel(src)
...
/obj/structure/trap/eldritch/mad/trap_effect(mob/living/victim)
	if(!iscarbon(victim))
		return
	var/mob/living/carbon/carbon_victim = victim
	carbon_victim.adjustHalLoss(80)
	carbon_victim.silent += (20 SECONDS)
