...
/obj/structure/lock_tear/atom_init(mapload, datum/mind/ascendant_mind)
	...
	if(!isnull(ascendant_mind))
		...
		RegisterSignals(ascendant_mind.current, list(COMSIG_MOB_DIED, COMSIG_PARENT_QDELETING), PROC_REF(end_madness))
...
/obj/structure/lock_tear/proc/poll_ghosts()
	var/list/candidates = SSpolling.poll_ghost_candidates("Would you like to be a random [span_notice("eldritch monster")] attacking the crew?", check_jobban = ROLE_SENTIENCE, role = ROLE_SENTIENCE, poll_time = 10 SECONDS, ignore_category = IGNORE_HERETIC_MONSTER, alert_pic = src, role_name_text = "eldritch monster")
...
/obj/structure/lock_tear/proc/end_madness(datum/former_master)
	...
	UnregisterSignal(former_master, list(COMSIG_MOB_DIED, COMSIG_PARENT_QDELETING)) // Just in case they die THEN delete
