...
/obj/effect/proc_holder/spell/no_target/draw_rune/cast(list/targets, mob/user = usr)
	for(var/atom/A in range(1, user.loc))
		if(istype(A, /obj/effect/fanatics_rune) || istype(A,/obj/effect/largerune))
	...
	var/datum/fanatics_rune/chosen_power
	var/list/names = list()
	for(var/datum/fanatics_rune/AR as anything in F.known_runes)
		names += AR.name
	var/chosen_rune_effect = tgui_input_list(user,"Вы собираетесь нарисовать...","Кровавые Чары", names)
	if(!chosen_rune_effect)
		revert_cast(user)
		return
	for(var/datum/fanatics_rune/selection as anything in F.known_runes)
		if(selection.name == chosen_rune_effect)
			chosen_power = selection

	carve(user, chosen_power)

/obj/effect/proc_holder/spell/no_target/draw_rune/proc/carve(mob/living/carbon/human/user, datum/fanatics_rune/power)
	...
	var/obj/effect/fanatics_rune/rune = new(user.loc)
...
/obj/effect/proc_holder/spell/no_target/draw_final_rune/proc/carve(mob/living/carbon/human/user)
	...
	rune.power = new /datum/fanatics_rune/final_ritual
	rune.power.holder = rune
	rune.blood_DNA = list()
	rune.blood_DNA[user.dna.unique_enzymes] = user.dna.b_type
