...
/obj/item/changeling_egg/proc/Pop()
	...
	var/datum/role/changeling/C = origin.GetRoleByType(/datum/role/changeling)
	if(C)
		if(!(/obj/effect/proc_holder/changeling/lesserform in C.purchasedpowers)) //If ling doesnt have lesserfrom, give them one-use
			var/obj/effect/proc_holder/changeling/lesserform/A = new (null)
			A.last_resort = TRUE
			C.purchasedpowers += A
			A.on_purchase(M)
