var/global/list/datum_donotcopy = list("tag", "type", "parent_type", "vars", "gcDestroyed", "being_sent_to_past", "disposed")
...
/proc/datum2list(datum/D, list/do_not_copy = datum_donotcopy, parent_datum = null)
	var/list/L = list()
	for(var/I in D.vars)
		if(I in do_not_copy)
			continue
...
/proc/datum2json(datum/D, list/do_not_copy = datum_donotcopy)
	ASSERT(istype(D))
