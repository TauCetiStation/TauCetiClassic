...
/mob/living/carbon/human/proc/RevConvert()
	...
	if(isrevhead(M) || isrev(M))
		...
		if(world.time < lead.rev_cooldown)
			to_chat(src, "<span class='warning'>Обождите пять секунд чтобы вы снова попытались конвертировать.</span>")
			return
		to_chat(src, "<span class='warning'>Попытка конвертировать [M]...</span>")
