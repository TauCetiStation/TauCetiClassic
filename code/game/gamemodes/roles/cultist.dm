...
/datum/role/cultist/Greet(greeting, custom)
	...
	to_chat(antag.current, "<span class='cult'>Вы Культист поклоняющийся древнему злому богу Нар-Си. Вам была дарована потустороняя сила для достижения поставленых целей. Если вы их не выполните...</span>")
	to_chat(antag.current, "<span class='cult'>Вам был дарован Том древних писаний культа. Открыв Том вы сможете начертить руны которые позволят вам телепортироватся, отправлять предметы или же себя в Рай, захватывать зоны, призывать невидимые баръеры, активировать пилоны да бы они стреляли в неверных.</span>")
...
/datum/role/cultist/proc/equip_cultist(mob/living/carbon/human/mob)
	...
	if(mob.mind)
		if(mob.mind.assigned_role == "Clown")
			to_chat(mob, "Ваши тренировки позволили вам превзойти свою клоунаду, позволяя вам пользоватся боевым снаряжением без вреда для себя.")
