...
/datum/role/shadowling/Greet(greeting, custom)
	...
	[sanitize("- Тень получила особые перчатки, имеющие особые эффекты на каждом из интентов: ")]<BR>
	[sanitize("Хелп - Лечение траллов")]<BR>
	[sanitize("Дизарм - Оглушение, 35 урона болью, требуется 4 удара до стана")]<BR>
	[sanitize("Граб) При нажатии вдаль кидает \"крюк\", станящий жертву на пол секунды и вас на четверть, притягивает к вам жертву, наности 20 урона")]<BR>
	[sanitize("Харм) Искаженная речь, размытое зрение, 10 урона болью")]<BR>
	[sanitize("- Если вы поработили кого-то с меткой от тралла, тралл получает бонусы в порядке: ")]<BR>
	[sanitize("1) Лечение в темноте ")]<BR>
	[sanitize("2) Слабое перемещение в тенях (слабый джаунт) ")]<BR>
	[sanitize("3) Глаза тени, усиление лечения в темноте, а также начинаете получать урон от света ")]<BR>
	[sanitize("4) Глушение света на манер тенелинга ")]<BR>
	[sanitize("5) Усиление лечения в темноте и урона на свету, усиление джаунта ")]<BR>
	[sanitize("6) Становление новым тенелингом.")]<BR>
	"}
...
/datum/role/thrall/Greet(greeting, custom)
	...
	[sanitize("- В полной форме тень потеряла способность Glare (открывается при достижении 75% от необходимых для победы траллов). Ищите новые способы красть людей.")]<BR>
	[sanitize("- Тень не умирает от 1 выстрела лазера, а также получила часть слотов")]<BR>
	[sanitize("- Ваша способность значительно ускоряет порабощение жертвы тенелингом в его второй форме, при этом вы получаете бонусы в следующем порядке за каждую новую жертву: ")]<BR>
	[sanitize("1) Лечение в темноте ")]<BR>
	[sanitize("2) Слабое перемещение в тенях (слабый джаунт) ")]<BR>
	[sanitize("3) Глаза тени, усиление лечения в темноте, а также начинаете получать урон от света ")]<BR>
	[sanitize("4) Глушение света на манер тенелинга ")]<BR>
