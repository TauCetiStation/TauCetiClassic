...
/datum/role/blob_overmind/Greet(greeting,custom)
	...
	to_chat(antag.current, "<span class='warning'>Найдите хорошее место для создания ядра после чего взять контроль и захлестнуть станцию!</span>")
	to_chat(antag.current, "<span class='warning'>Когда вы найдете подходящее место, ожидайте созревание ядра; это произойдет автоматически и вы никак не можете ускорить данный процесс.</span>")
	to_chat(antag.current, "<span class='warning'>Если вы покинете сектор станции, или улетите в космос, тогда вы умрете; убедитесь в том что ваше место имеет достаточно земли/пола чтобы закрепится на станции.</span>")
