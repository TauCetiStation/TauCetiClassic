...
/datum/role/blob_overmind/Greet(greeting,custom)
	...
	to_chat(antag.current, "<span class='warning'>Найдите хорошее место для создания ядра, после чего возьмите контроль и захлестните станцию!</span>")
	...
	to_chat(antag.current, "<span class='warning'>Если вы покинете сектор станции, или улетите в космос, тогда вы умрете; убедитесь в том, что ваше место имеет достаточно пола, чтобы закрепиться на станции.</span>")
