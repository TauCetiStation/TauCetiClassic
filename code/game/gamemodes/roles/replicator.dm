...
/datum/role/replicator/Greet(greeting, custom)
	. = ..()
	to_chat(antag.current, {"<span class='notice'><b>Вы Репликатор, часть Роя. Вы должны поглощать материалы, возводить свою инфраструктуру или использовать станционную для постройки Блюспейс Катапульты, которая распространит Рой дальше по галактике через трещену. Реплицируйся и процветай!</b></span>"})
