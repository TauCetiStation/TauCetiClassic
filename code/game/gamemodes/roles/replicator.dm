...
/datum/role/replicator/Greet(greeting, custom)
	. = ..()
	to_chat(antag.current, {"<span class='notice'><b>Вы Репликатор, часть Роя. Вы должны поглощать материалы, возводить свою инфраструктуру или использовать станционую для постройки Bluespace Catapult, которая распростронит Рой дальше по Галактике через трещену. Реплицируйся и процветай!</b></span>"})
