...
/datum/game_mode/epidemic/send_intercept()
	...
	for(var/mob/M in player_list)
		if(!isnewplayer(M))
			M.playsound_local(null, 'sound/AI/commandreport.ogg', 50, channel = 802, wait = 1m is_global = 1)
...
/datum/game_mode/epidemic/proc/announce_to_kill_crew()
	...
	for(var/mob/M in player_list)
		if(!isnewplayer(M))
			M.playsound_local(null, 'sound/AI/commandreport.ogg', 50, channel = 802, wait = 1, is_global = 1)
...
/datum/game_mode/epidemic/post_setup()
	...
	var/list/crew = list()
	for(var/mob/living/carbon/human/H in human_list) if(H.client)
