...
/obj/effect/proc_holder/changeling/weapon/try_to_sting(mob/user, mob/target)
	...
	user.visible_message(
		"<span class='warning'>With a sickening crunch, [user] reforms his [weapon_name_simple] into an arm!</span>",
...
/obj/effect/proc_holder/changeling/weapon/sting_action(mob/user)
	if(!user.dropItemToGround(user.get_active_hand()))
...
/obj/effect/proc_holder/changeling/suit/sting_action(mob/living/carbon/human/user)
	if(!user.dropSlotToGround(slot_wear_suit))
		to_chat(user, "We cannot grow a [suit_name_simple] over our body!")
		return
	if(!user.dropSlotToGround(slot_head))
		to_chat(user, "We cannot grow a [helmet_name_simple] over our head!")
		return

	var/failed_count = 0
	if(!user.equip_to_slot_if_possible(new suit_type, slot_wear_suit, 1, 1, 1)) // in case, bodypart does not support this suit or head.
		to_chat(user, "We failed to properly grow a [suit_name_simple] over this body!")
		failed_count++
	if(!user.equip_to_slot_if_possible(new helmet_type, slot_head, 1, 1, 1))
		to_chat(user, "We failed to properly grow a [helmet_name_simple] over this head!")
		failed_count++
...
/obj/item/weapon/melee/arm_blade/dropped(mob/user)
	user.visible_message(
		"<span class='warning'>With a sickening crunch, [user] reforms his blade into an arm!</span>",
		"<span class='notice'>We assimilate the blade back into our body.</span>",
		"<span class='warning>You hear organic matter ripping and tearing!</span>")
	..()
...
/obj/item/clothing/suit/space/changeling/process()
	if(ishuman(loc))
		...
		var/obj/item/organ/lungs/IO = H.organs_by_name[BP_LUNGS]
		if(IO.damage >= 5)
			IO.damage -= 5
