...
/obj/effect/proc_holder/changeling/sting/proc/sting_fail(mob/user, mob/target)
	...
	if(ishuman(target))
		...
		if(target:check_thickmaterial(affecting))
			to_chat(user, "<span class='warning'>We broke our sting about [target.name]'s [affecting.name]!</span>")
			to_chat(target, "<span class='warning'>You feel a tiny push in your [affecting.name]!</span>")
			unset_sting(user)
			user.mind.changeling.chem_charges -= rand(5,10)
			if(ishuman(user))
				user:drip(10)
