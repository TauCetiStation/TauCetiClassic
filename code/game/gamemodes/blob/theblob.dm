...
/obj/effect/blob/attackby(obj/item/weapon/W, mob/user)
	...
	src.visible_message("<span class=warning'><B>The [src.name] has been attacked with \the [W][(user ? </span>" by [user]." : ".")]")
	var/damage = 0
	switch(W.damtype)
		if("fire")
...
/obj/effect/blob/attack_animal(mob/living/simple_animal/M)
	...
	playsound(src.loc, 'sound/effects/attackblob.ogg', 50, 1)
	src.visible_message("<span class=warning'><B>The [src.name] has been attacked by \the [M].</span>")
