...
/datum/faction/revolution/process()
	...
	if(last_command_report == 0 && world.time >= 10 MINUTES)
		command_report("С сожалением сообщаем, что ваша производительность оставляет желать лучшего, и мы вынуждены сократить финансовую поддержку вашей станции. Для достижения этой цели зарплата всего персонала, кроме руководящего состава и сотрудников службы безопасности, будет уменьшена в два раза.")
		...
		command_report("Согласно статистическим данным, свободное время и связанные с ним деятельности могут быть причиной неудовлетворительной работы нескольких наших станций. Поэтому рекомендуется закрыть все развлекательные объекты, включая спортзал, голодек, театр и бар. Питание экипажа может проводиться через поставщиков и кухню.")
		last_command_report = 2
	else if(last_command_report == 2 && world.time >= 45 MINUTES)
		command_report("Мы начали подозревать, что руководители персонала могут быть нелояльными к НаноТрейзен. Мы просим Вас и других руководителей ввести имплант лояльности, если вы еще не вводили его в себя. Руководители, которые не хотят вводить имплант, должны быть арестованы за неисполнение приказов Центрального Командования до конца смены.")
		last_command_report = 3
	else if(last_command_report == 3 && world.time >= 60 MINUTES)
		command_report("Сообщается, что простое закрытие развлекательных учреждений не дало результатов. Вы и ваши руководители должны обеспечить максимальную эффектиность всего экипажа и не допустить пустой траты времени и энергии. Любой член экипажа, который будет замечен на отдыхе без разрешения своего руководителя, должен быть предупрежден, а при повторном нарушении - помещен в тюрьму до прибытия следующего трансферного шаттла, который доставит их в учреждения, где они могут быть более полезными.")
