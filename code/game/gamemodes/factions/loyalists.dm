...
/datum/mutiny_scenario/communism/get_second_report()
	...
	report_dat += "Все киборги на станции должны выбрать модули подходящие для работы в отсеках медбея и переместиться в соответствующие помещения на постоянной основе.<br>"
	return report_dat
