	...
	switch(href_list["choice"])
		...
		if("Inject")
			if( (href_list["tag"]=="radium"? (reagents.get_reagent_amount("radium"))<=(a_boost*a_transfer) : !reagents.get_reagent_amount(href_list["tag"])) )//Special case for radium. If there are only a_boost*a_transfer radium units left.
				to_chat(display_to, "<span class=warning'>Error: the suit cannot perform this function. Out of [href_list[</span>"name"]].")
			else
		...
		if("RESEARCH")
			...
			spawn(0)
				...
				for(var/mob/living/silicon/ai/AI in player_list)
					to_chat(AI, "<span class=warning'><b>Network Alert: Hacking attempt detected[location?</span>" in [location]":". Unable to pinpoint location"]</b>.")
