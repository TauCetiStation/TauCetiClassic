...
/obj/item/clothing/suit/space/space_ninja/proc/ninitialize(delay = s_delay, mob/living/carbon/human/U = loc)
	if(U.mind && U.mind.assigned_role=="MODE" && !s_initialized && !s_busy)//Shouldn't be busy... but anything is possible I guess.
			...
			switch(i)
				...
				if(3)
					if(U.stat==2||U.health<=0)
						to_chat(U, "<span class='warning'><B>F?AL ùRrùR</B>: 344--93#ù&&21 BRùùN |/|/aVù PATT$RN <B>RED</B>\nA-A-aBùrTùNG...</span>")
						unlock_suit()
						break
					lock_suit(U,1)//Check for icons.
					U.regenerate_icons()
					to_chat(U, "<span class='notice'>Linking neural-net interface...\nPattern <span class='green'><B>GREEN</B></span>, continuing operation.</span>")
				if(4)
		...
		if(!U.mind||U.mind.assigned_role!="MODE")//Your run of the mill persons shouldn't know what it is. Or how to turn it on.
			...
			to_chat(U, "<span class='warning'>The suit is already functioning. \black <b>Please report this bug.</b></span>")
		else
			to_chat(U, "<span class='warning'><B>ERROR</B>: \black You cannot use this function at this time.</span>")
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/deinitialize(delay = s_delay)
	if(affecting==loc&&!s_busy)
		...
		if(!s_initialized)
			to_chat(U, "<span class='warning'>The suit is not initialized. \black <b>Please report this bug.</b></span>")
			return
		...
		for(var/i = 0,i<7,i++)
			switch(i)
				...
				if(4)
					to_chat(U, "<span class='notice'>Disconnecting neural-net interface...<span class='green'><B>Success</B></span>.</span>")
				if(5)
					to_chat(U, "<span class='notice'>Disengaging neural-net interface...<span class='green'><B>Success</B></span>.</span>")
				if(6)
	...
	switch(href_list["choice"])
		...
		if("Shock")
			...
			if(damage>1)//So they don't spam it when energy is a factor.
				...
				to_chat(A, "<span class='warning'><b>ERROR</b>: \black Not enough energy remaining.</span>")
			...
			if(input(U)=="Divine Wind")
				if( !(U.stat||U.wear_suit!=src||!s_initialized) )
					if( !(cell.charge<=1||s_busy) )
						...
						to_chat(U, "<span class='warning'><b>ERROR<b>: \black Unable to initiate mode.</span>")
				else
		...
		if("Eject Disk")
			...
			if(!U.get_active_hand())
				...
				if(T)
					...
					to_chat(U, "<span class='warning'><b>ERROR<b>: \black Could not eject disk.</span>")
		...
		if("Eject pAI")
			...
			if(!U.get_active_hand())
				...
				if(T)
					...
					to_chat(U, "<span class='warning'><b>ERROR<b>: \black Could not eject pAI card.</span>")
				...
				if(A.laws.zeroth)//Gives a few seconds to re-upload the AI somewhere before it takes full control.
					...
					for(var/i,i<5,i++)
						if(AI==A)
							switch(i)
								if(0)
									to_chat(A, "<span class='warning'><b>WARNING</b>: \black purge procedure detected. \nNow hacking host...</span>")
									to_chat(U, "<span class='warning'><b>WARNING</b>: HACKING ATùùTEMPù IN PR0GRESs!</span>")
									spideros = 0
									k_unlock = 0
									U << browse(null, "window=spideros")
								if(1)
									to_chat(A, "Disconnecting neural interface...")
									to_chat(U, "<span class='warning'><b>WARùNING</b>: ùRùO0ùGrù--S 2&3%</span>")
								if(2)
									to_chat(A, "Shutting down external protocol...")
									to_chat(U, "<span class='warning'><b>WARNING</b>: PùùùùR?Grù5S 677^%</span>")
									cancel_stealth()
								if(3)
									to_chat(A, "Connecting to kernel...")
									to_chat(U, "<span class='warning'><b>WARNING</b>: ùRùrùR_404</span>")
									A.control_disabled = 0
								if(4)
									to_chat(A, "Connection established and secured. Menu updated.")
									to_chat(U, "<span class='warning'><b>Wùr#nING</b>: #%@!!W?|_4ù54@ \nUnùB88l3 Tù Lù-ùo-LùCaT2 ##$!ùRNù0..%..</span>")
									grant_AI_verbs()
									return
							sleep(s_delay)
						else	break
...
/obj/item/clothing/suit/space/space_ninja/proc/ai_holo(turf/T in oview(3,affecting))//To have an internal AI display a hologram to the AI and ninja only.
	...
	if(s_initialized&&affecting&&affecting.client&&istype(affecting.loc, /turf))//If the host exists and they are playing, and their location is a turf.
		if(!hologram)//If there is not already a hologram.
			...
			to_chat(AI, "<span class='warning'>ERROR: \black Image feed in progress.</span>")
	else
		to_chat(AI, "<span class='warning'>ERROR: \black Unable to project image.</span>")
	return
	...
	if(U==affecting)//Safety, in case you try doing this without wearing the suit/being the person with the suit.
		if(istype(I, /obj/item/device/aicard))//If it's an AI card.
			if(s_control)
				...
				to_chat(U, "<span class='warning'><b>ERROR</b>: \black Remote access channel disabled.</span>")
			...
			if(TD.stored)//If it has something on it.
				...
				if(do_after(U,s_delay,target = U))
					...
					to_chat(U, "<span class='warning'><b>ERROR</b>: \black Procedure interrupted. Process terminated.</span>")
			else
				I.loc = src
				t_disk = I
				to_chat(U, "<span class='notice'>You slot \the [I] into \the [src].</span>")
...
/obj/item/clothing/suit/space/space_ninja/proc/toggle_stealth()
	...
	if(s_active)
		...
		to_chat(U, "<span class='notice'>You are now invisible to normal detection.</span>")
...
/obj/item/clothing/suit/space/space_ninja/proc/cancel_stealth()
	...
	if(s_active)
		...
		s_active=!s_active
		to_chat(U, "<span class='notice'>You are now visible.</span>")
		U.invisibility = 0
		for(var/mob/O in oviewers(U))
	...
	if(s_initialized)
		if(user == affecting)
			if(s_control)
				...
				to_chat(user, "ùrrùR ùaùùaùù No-ù-ù fùùNù 3RRùr")
...
//Var Initialize
	...
	if(target_type!="RESEARCH")//I lumped research downloading here for ease of use.
		to_chat(U, "<span class='notice'>Now charging battery...</span>")
	...
	switch(target_type)
		...
		if("RESEARCH")
			var/obj/machinery/A = target
			to_chat(U, "<span class='notice'>Hacking \the [A]...</span>")
			spawn(0)
						...
						for(var/datum/tech/analyzing_data in A:files.known_tech)
							if(current_data.id==analyzing_data.id)
								if(analyzing_data.level>current_data.level)
									to_chat(U, "<span class='notice'>Database: \black <b>UPDATED</b>.</span>")
...
/obj/effect/energy_net
	...
	process(var/mob/living/carbon/M as mob)
		...
		/*if(!isnull(src))//As long as both net and person exist.
			...
			if(!isnull(master))//As long as they still exist.
				to_chat(master, "<span class='notice'><b>SUCCESS</b>: \black transport procedure of \the [affecting] complete.</span>")
