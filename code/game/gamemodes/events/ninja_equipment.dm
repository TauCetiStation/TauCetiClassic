			...
			if(A.cell && A.cell.charge)
				...
				while(G.candrain && A.cell.charge > 0 && !maxcapacity)
					...
					if (do_after(U,10,target = A))
						spark_system.start()
						playsound(A, "sparks", VOL_EFFECTS)
						A.cell.charge-=drain
						S.cell.charge+=drain
						totaldrain+=drain
					else	break
...
/obj/effect/energy_net/process(var/mob/living/carbon/M as mob)
	...
	/*if(!isnull(src))//As long as both net and person exist.
		...
		spawn(0)
			playsound(M, 'sound/effects/sparks4.ogg', VOL_EFFECTS)
		...
		if(holdingfacility.len)
			...
			spawn(0)
				...
				spark_system.start()
				playsound(M, 'sound/effects/phasein.ogg', VOL_EFFECTS, 25)
				playsound(M, 'sound/effects/sparks2.ogg', VOL_EFFECTS)
