...
/obj/item/clothing/suit/space/space_ninja/proc/ninjasmoke()
	...
	if(!ninjacost(,2))
		...
		smoke.start()
		playsound(U, 'sound/effects/bamf.ogg', VOL_EFFECTS)
		s_bombs--
		s_coold = 1
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjashift(turf/T in oview())
	...
	if(!ninjacost(C,1))
		...
		if((!T.density)&&istype(mobloc, /turf))
			spawn(0)
				playsound(U, 'sound/effects/sparks4.ogg', VOL_EFFECTS)
			...
			spawn(0)
				spark_system.start()
				playsound(U, 'sound/effects/phasein.ogg', VOL_EFFECTS, 25)
				playsound(U, 'sound/effects/sparks2.ogg', VOL_EFFECTS)
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjapulse()
	...
	if(!ninjacost(C,0)) // EMP's now cost 1,000Energy about 30%
		var/mob/living/carbon/human/U = affecting
		playsound(U, 'sound/effects/EMPulse.ogg', VOL_EFFECTS)
	...
	if(!ninjacost(C,0)) //Same spawn cost but higher upkeep cost
		...
		if(!kamikaze)
			...
			if(!U.get_active_hand()&&!istype(U.get_inactive_hand(), /obj/item/weapon/melee/energy/blade))
				...
				spark_system.start()
				playsound(U, "sparks", VOL_EFFECTS)
				U.put_in_hands(W)
				cell.use(C*10)
			else
