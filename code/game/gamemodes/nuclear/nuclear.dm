...
/datum/game_mode/nuclear/announce()
	...
	to_chat(world, "<B>Gorlex Maradeurs are approaching NFS Gamma!</B>")
...
/datum/game_mode/nuclear/declare_completion()
	...
	if      (!disk_rescued &&  station_was_nuked &&          !syndies_didnt_escape)
		...
		completion_text += "<BR><B>Gorlex Maradeurs operatives have destroyed NFS Gamma!</B>"
		score["roleswon"]++

	else if (!disk_rescued &&  station_was_nuked &&           syndies_didnt_escape)
		feedback_set_details("round_end_result","halfwin - syndicate nuke - did not evacuate in time")
		completion_text += "<FONT size = 3, color='red'><B>Total Annihilation</B></FONT>"
		completion_text += "<BR><B>Gorlex Maradeurs operatives destroyed NFS Gamma but did not leave the area in time and got caught in the explosion.</B> Next time, don't lose the disk!"

	else if (!disk_rescued && !station_was_nuked &&  nuke_off_station && !syndies_didnt_escape)
		feedback_set_details("round_end_result","halfwin - blew wrong station")
		completion_text += "<FONT size = 3, color='red'><B>Crew Minor Victory</B></FONT>"
		completion_text += "<BR><B>Gorlex Maradeurs operatives secured the authentication disk but blew up something that wasn't NFS Gamma.</B> Next time, don't lose the disk!"

	else if (!disk_rescued && !station_was_nuked &&  nuke_off_station &&  syndies_didnt_escape)
		feedback_set_details("round_end_result","halfwin - blew wrong station - did not evacuate in time")
		completion_text += "<FONT size = 3, color='red'><B>Gorlex Maradeurs operatives have earned Darwin Award!</B></FONT>"
		completion_text += "<BR><B>Gorlex Maradeurs operatives blew up something that wasn't NFS Gamma and got caught in the explosion.</B> Next time, don't lose the disk!"
		...
		completion_text += "<FONT size = 3, color='red'><B>Syndicate Minor Victory!</B></FONT>"
		completion_text += "<BR><B>Gorlex Maradeurs operatives recovered the abandoned authentication disk but detonation of NFS Gamma was averted.</B> Next time, don't lose the disk!"
