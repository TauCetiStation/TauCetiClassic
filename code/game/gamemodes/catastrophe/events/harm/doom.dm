...
/datum/catastrophe_event/doom/on_step()
	switch(step)
		if(1)
			announce("Здравствуйте, Исход. Мы сообщаем вам, что на ЦК сейчас провод[JA_PLACEHOLDER]т исследовани[JA_PLACEHOLDER] одного кристалла, полученного из блюспейс пространства. Не то чтобы эта процедура может быть небезопасной. Скорее всего, всё пройдёт нормально… скорее всего. Наш лучший ученый в данный момент доставл[JA_PLACEHOLDER]ет его в специальный Антимасс-Спектрометр, мы будем сообщать вам о ходе исследовани[JA_PLACEHOLDER]")
...
/datum/catastrophe_event/doom/proc/spawn_first_portal()
	announce("… Отключи эту штуку! Отключи немедленно! Исход, кристалл оказалс[JA_PLACEHOLDER] крайне нестабильным и в результате вызвал каскадный резонанс. По всей системе создаютс[JA_PLACEHOLDER] межпространственные разломы в блюспейс, не смейте подходить к ним. Мы ищем решение проблемы")
