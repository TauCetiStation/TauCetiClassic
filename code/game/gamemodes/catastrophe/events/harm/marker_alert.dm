...
/datum/catastrophe_event/marker_alert/on_step()
	switch(step)
		if(1)
			...
			announce("Внимание, недалеко от вашей станции замечен неизвестный объект. По нашим данным он, скорее всего, не представл[JA_PLACEHOLDER]ет угрозы, но будьте осторожны. Доставьте объект в научный отдел и начните исследовани[JA_PLACEHOLDER]. Результаты пришлите в виде отчёта. Примерные координаты объекта: [T.x], [T.y]")

			message_admins("Marker was created in [T.x],[T.y],[T.z] [ADMIN_JMP(T)]")
		if(2)
			announce("У нас дл[JA_PLACEHOLDER] вас новости, Исход. Объект, похожий на ваш, был обнаружен р[JA_PLACEHOLDER]дом со станцией ЦК. Ученые присвоили ему кодовое им[JA_PLACEHOLDER] “Обелиск”. Похоже, у Обелиска есть несколько свойств. Мертвые клетки под его вли[JA_PLACEHOLDER]нием могут каким-то странным образом оживать. А так же, похоже, что Обелиск может вли[JA_PLACEHOLDER]ть на сознание. Не давайте р[JA_PLACEHOLDER]довому персоналу подходить к Обелиску, это может быть опасно. Мы будем держать вас в курсе, Исход")
		if(3)
		...
		if(4)
			announce("Эти монстры, они убивают людей! Отрывайте им конечности, это единственный способ их остановить. В данный момент мы ведём ожесточённое сражение с ними. Не дайте Исходу пасть, экипаж")
...
/datum/catastrophe_event/marker_alert/proc/marker_report()
	announce("Внимание, Исход. Обелиск несёт в себе смертельную опасность, изолируйте его немедленно. Сигналы, которые он посылает, мен[JA_PLACEHOLDER]ют генетический код окружающих, превраща[JA_PLACEHOLDER] их в насто[JA_PLACEHOLDER]щих монстров! Немедленно создайте карантинный периметр, не дайте персоналу пострадать от этих монстров")
