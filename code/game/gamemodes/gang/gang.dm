...
/datum/game_mode/gang/proc/domination(var/gang,var/modifier=1,var/dominatorloc)
	...
	if(gang && dominatorloc)
		...
		captain_announce("Hostile runtimes detected in all station systems. A network breach by the [gang_name(gang)] Gang has been traced to [dominatorloc].")
...
/datum/game_mode/gang/check_win()
	...
	if(winner)
		if(winner == 3) //Edge Case: If both dominators activate at the same time
			domination("A",0.5)
			domination("B",0.5)
			captain_announce("Multiple station takeover attempts have made simultaneously. Conflicting hostile runtimes have delayed both attempts.")
		else if(winner == 1)
