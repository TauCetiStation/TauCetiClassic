...
// Gates the role into SSjob.active_occupations: when the edict is off (value 0) the role is absent
// from manifests, preferences assignment and late-join entirely.
/datum/job/cargo_psc/map_check()
	return is_edict_active(EDICT_CARGO_GUARD)
