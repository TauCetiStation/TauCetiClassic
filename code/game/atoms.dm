...
/atom/proc/examine(mob/user, distance = -1)
	...
	if(reagents && is_open_container()) //is_open_container() isn't really the right proc for this, but w/e
		msg += "<br>Внутри содержится:"
		if(reagents.reagent_list.len)
			if(istype(src, /obj/structure/reagent_dispensers)) //watertanks, fueltanks
				for(var/datum/reagent/R in reagents.reagent_list)
					msg += "<br><span class='info'>[R.volume] юнит(ов) вещества [R.name]</span>"
			else if (is_skill_competent(user, list(/datum/skill/chemistry = SKILL_LEVEL_MASTER)))
				if(length(reagents.reagent_list) == 1)
					msg += "<br><span class='info'>[reagents.reagent_list[1].volume] юнит(ов) вещества [reagents.reagent_list[1].name]</span>"
				else
					for(var/datum/reagent/R in reagents.reagent_list)
						msg += "<br><span class='info'>[R.volume + R.volume * rand(-25,25) / 100] юнит(ов) вещества [R.name]</span>"
			else
				msg += "<br><span class='info'>[reagents.total_volume] юнит(ов) жидкости.</span>"
		else
			msg += "<br>Ничего."
