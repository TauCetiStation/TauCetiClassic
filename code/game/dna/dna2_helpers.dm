...
/proc/randmutg(mob/living/M)
	if(!M) return
	M.dna.check_integrity()
	var/list/g_blocks = list(HULKBLOCK,XRAYBLOCK,FIREBLOCK,TELEBLOCK,NOBREATHBLOCK,REMOTEVIEWBLOCK,REGENERATEBLOCK,INCREASERUNBLOCK,REMOTETALKBLOCK,MORPHBLOCK,INVISBLOCK,BLENDBLOCK,NOPRINTSBLOCK,SHOCKIMMUNITYBLOCK,SMALLSIZEBLOCK,COLDBLOCK)
	var/list/possible_blocks = list()
