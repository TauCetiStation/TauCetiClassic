...
/mob/proc/playsound_lobbymusic()
	if(!SSticker || !SSticker.login_music || !SSticker.login_welcome || !client)
		return
	playsound_music(SSticker.login_welcome, VOL_MUSIC, null, TRUE, CHANNEL_MUSIC)
	playsound_music(SSticker.login_music, VOL_MUSIC, null, TRUE, CHANNEL_MUSIC) // MAD JAMS
