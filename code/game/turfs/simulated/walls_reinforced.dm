...
/turf/simulated/wall/r_wall/attackby(obj/item/W, mob/user)
	...
	switch(d_state)
		if(INTACT)
			if (iswirecutter(W))
				if(!handle_fumbling(user, src, SKILL_TASK_TOUGH, SKILL_ENGINEERING, SKILL_ENGINEERING_PRO, SKILL_TASK_VERY_EASY ,"<span class='notice'>You fumble around figuring out how to cut the outer grille.</span>"))
					return
		...
		if(SUPPORT_LINES)
			if (isscrewdriver(W))
				...
				if(W.use_tool(src, user, SKILL_TASK_AVERAGE, volume = 100, required_skills = list(SKILL_ENGINEERING = SKILL_ENGINEERING_PRO)))
			...
			//REPAIRING (replacing the outer grille for cosmetic damage)
				...
				if(!O.use(1))
					return
				if(!handle_fumbling(user, src, SKILL_TASK_AVERAGE, list(SKILL_ENGINEERING = SKILL_ENGINEERING_PRO), SKILL_TASK_VERY_EASY ,"<span class='notice'>You fumble around figuring out how to replace the outer grille.</span>"))
					return
				d_state = INTACT
				update_icon()
		...
		if(COVER)
			...
			if(istype(W, /obj/item/weapon/pickaxe/plasmacutter))
				to_chat(user, "<span class='notice'>You begin slicing through the metal cover.</span>")
				if(W.use_tool(src, user, SKILL_TASK_TOUGH, volume = 100, required_skills = list(SKILL_ENGINEERING = SKILL_ENGINEERING_PRO)))
					if(!istype(src, /turf/simulated/wall/r_wall) || !T)
						return
		...
		if(CUT_COVER)
			if (iscrowbar(W))
				...
				if(W.use_tool(src, user, SKILL_TASK_DIFFICULT, volume = 100,  required_skills = list(SKILL_ENGINEERING = SKILL_ENGINEERING_PRO)))
					if(!istype(src, /turf/simulated/wall/r_wall) || !T)
						return
