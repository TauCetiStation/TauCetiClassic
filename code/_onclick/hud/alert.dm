...
/atom/movable/screen/alert/bloodsense/atom_init(mapload)
	. = ..()
...
/atom/movable/screen/alert/bloodsense/proc/target_selection()
	var/datum/faction/cult/C = cult_religion.mode
	if(C.sacrifice_target)
		//for(var/datum/mind/sac_objective in C.sacrifice_target)
		var/datum/mind/sac_objective = C.sacrifice_target
		if(get_turf(sac_objective))
			target = sac_objective
			//break
		if(!target)
			for(var/datum/objective/target/sacrifice/O in C.objective_holder.objectives)
				if(O && O.check_completion() == OBJECTIVE_WIN)
					continue
				else
					if(icon_state == "runed_sense0")
						return
					desc = "Nar'Sie demands that [O.target] be sacrificed."
					animate(src, transform = null, time = 1, loop = 0)
					angle = 0
					cut_overlays()
					icon_state = "runed_sense0"

					var/icon/target_icon = icon('icons/effects/effects.dmi', "nothing")
					var/icon/partial = getFlatIcon(O.target, defdir = SOUTH)
					target_icon.Insert(partial, dir = SOUTH)
					add_overlay(target_icon)
					return
	var/datum/objective/cult/summon_narsie/summon_objective = locate() in C.objective_holder.objectives
	if(!summon_objective)
		return
	...
	desc = "The sacrifice is complete, summon Nar'Sie!" //The summoning can only take place in [get_english_list(location_list)]!"
...
/atom/movable/screen/alert/bloodsense/greed/target_selection(item_target)
	. = ..()
	if(isatom(item_target))
		target = item_target
		...
		var/icon/target_icon = icon('icons/effects/effects.dmi', "nothing")
		var/icon/partial = getFlatIcon(target, defdir = SOUTH)
		target_icon.Insert(partial, dir = SOUTH)
		add_overlay(target_icon)
