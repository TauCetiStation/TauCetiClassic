...
/proc/do_after(mob/user, delay, needhand = TRUE, atom/target, can_move = FALSE, progress = TRUE, datum/callback/extra_checks, distance = 1)
	...
	while (world.time < endtime)
		...
		if(distance >= 1 && get_dist(user.loc, Tloc) > distance)
			. = FALSE
			break
