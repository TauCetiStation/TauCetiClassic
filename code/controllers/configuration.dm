	...
	for (var/T in (typesof(/datum/game_mode) - /datum/game_mode))
		...
		if(!modeset || modeset == "random" || modeset == "secret")
			...
			switch(modeset)
				if("bs12")
					switch(M.config_tag)
						if("traitorchan","traitor","blob","gang","heist","infestation","meme","meteor","mutiny","ninja","rp-revolution","shadowling")
							qdel(M)
							continue
				if("tau classic")
					switch(M.config_tag)
						if("traitor","blob","extended","gang","heist","infestation","meme","meteor","mutiny","ninja","rp-revolution","shadowling")
							qdel(M)
							continue
