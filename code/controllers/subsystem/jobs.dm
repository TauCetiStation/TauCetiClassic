...
/datum/controller/subsystem/job/proc/EquipRank(mob/living/carbon/human/H, rank, joined_late=FALSE)
	...
	var/datum/money_account/M = create_random_account_and_store_in_mind(H, job.salary, job.department_stocks, job.department ? (job.head_position ? department_accounts["Command"] : department_accounts[job.department]) : null, job.salary_priority)	//starting funds = salary
