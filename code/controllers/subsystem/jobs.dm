...
/datum/controller/subsystem/job/proc/AssignRole(mob/dead/new_player/player, rank, latejoin=0)
	...
	if(player && player.mind && rank)
		...
		if(!job.map_check())
			return FALSE
		var/position_limit = job.baseline_positions
		if(!latejoin)
...
/datum/controller/subsystem/job/proc/FreeRole(rank)	//making additional slot on the fly
	var/datum/job/job = GetJob(rank)
	if(job && job.current_positions >= job.baseline_positions && job.baseline_positions != -1)
		job.baseline_positions++
		return TRUE
	return FALSE
...
/datum/controller/subsystem/job/proc/ResetOccupations()
	...
	for(var/datum/job/J as anything in active_occupations)
		...
		if(!isnull(J.map_total_positions))
			J.baseline_positions = J.map_total_positions
		else
			J.baseline_positions = initial(J.baseline_positions)
...
/datum/controller/subsystem/job/proc/FillHeadPosition()
	for(var/level in JP_LEVELS)
		for(var/command_position in heads_positions)
			...
			if(!job)
				continue
			if((job.current_positions >= job.baseline_positions) && job.baseline_positions != -1)
				continue
...
/datum/controller/subsystem/job/proc/FillAIPosition()
	...
	if(istype(SSticker.mode, /datum/game_mode/malfunction) && job.spawn_positions)//no additional AIs with malf
		job.baseline_positions = job.spawn_positions
		job.spawn_positions = 0
	for(var/i = job.baseline_positions, i > 0, i--)
		for(var/level in JP_LEVELS)
			var/list/candidates = list()
