				...
				for(var/tpos = 1 to length(line) - key_len + 1 step key_len)
					...
					if(xcrd >= 1)
						...
						if(!no_afterchange || (model_key != space_key))
							...
							stuff += build_coordinate(areaCache, cache, locate(xcrd, ycrd, zcrd), no_afterchange, placeOnTop)
...
/datum/parsed_map/proc/build_coordinate(list/areaCache, list/model, turf/crds, no_changeturf as num, placeOnTop as num)
	...
	if(members[index] != /area/template_noop)
		...
		_preloader.setup(members_attributes[index], atype)//preloader for assigning  set variables on atom creation
		var/atom/instance = areaCache[atype]
		if (!instance)
			instance = areas_by_type[atype]
			if (!instance)
	...
	for(index in 1 to first_turf_index-1)
		inst = instance_atom(members[index],members_attributes[index],crds,no_changeturf,placeOnTop)
		if(isobj(inst))
			. += inst
...
/datum/map_template/proc/load_new_z()
	...
	var/datum/parsed_map/parsed = load_map(file(mappath), x, y, level.z_value, placeOnTop=TRUE)
	var/list/bounds = parsed.bounds
	if(!bounds)
		return FALSE

	//initialize things that are normally initialized after map load
	initTemplateBounds(parsed.bounds)
	...
	if(crds)
		if(ispath(path, /turf))
			...
			if(placeOnTop)
				...
				crds.baseturfs = initial(turfpath.baseturfs)
			else
...
/datum/parsed_map/proc/trim_text(what, trim_quotes = 0)
	if(trim_quotes)
...
/datum/parsed_map/proc/find_next_delimiter_position(text, initial_position, delimiter=",", opening_escape = "\"", closing_escape = "\"")
...
/datum/parsed_map/proc/readlist(text, delimiter=",")
	. = list()
	if (!text)
		return
