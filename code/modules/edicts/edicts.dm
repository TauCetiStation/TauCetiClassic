 ...
 * positive number is a magnitude (for Cargo Guard - the number of guard slots, see cargo_guard.dm).
 *
 * `available_edicts` is a whitelist that guards against typo'd edict keys.
 */
var/global/list/available_edicts = list(
	EDICT_CARGO_GUARD = TRUE,
)

// Set once the QM console prints a Cargo Guard request this round, to prevent re-printing.
// Reset at round start by the edict controller.
var/global/cargo_guard_edict_requested = FALSE
