	...
	if((src.last_shot + src.fire_delay) <= world.time)
		...
		switch(strength)
			if(0)
				...
				playsound(src, 'sound/machines/cyclotron.ogg', VOL_MISC, 30, FALSE)
			if(1)
				A = new/obj/effect/accelerated_particle(T, dir)
				playsound(src, 'sound/machines/cyclotron.ogg', VOL_MISC, 70, FALSE)
			if(2)
				A = new/obj/effect/accelerated_particle/strong(T, dir)
				playsound(src, 'sound/machines/cyclotron.ogg', VOL_MISC, null, FALSE) // null for default, which is maximum volume.
		if(A)
			A.dir = src.dir
			return 1
	return 0
