...
/obj/machinery/power/apc
	...
	desc = "Терминал управления для электрических систем в этой зоне." //Терминал контроля электроснабжения этой зоны (?)
	...
	if(src in oview(1, user))
		...
		if(opened)
			if(has_electronics && terminal)
				...
				to_chat(user, "Плата установлена, но нет проводов.")
			else /* if(!has_electronics && !terminal) */
				to_chat(user, "Здесь нет платы и нет проводов.")
		else
			if(stat & MAINT)
				to_chat(user, "Крышка закрыта, но с ней что-то не так. Ее заклинило")
			else if(malfhack)
				to_chat(user, "Крышка сломана. Без лома ее не открыть")
			else
...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if(iscrowbar(W) && opened)
		if(has_electronics == 1)
			...
			if(user.is_busy(src))
				return
			to_chat(user, "Вы пытаетесь извлечь управляющую плату.")//lpeters - fixed grammar issues
			if(W.use_tool(src, user, 50, volume = 50))
				has_electronics = 0
				if((stat & BROKEN) || malfhack)
					user.visible_message(\
						"<span class='warning'>[user.name] сломал управляющую плату [src.name], пытаясь ее извлечь!</span>",\
						"Вы сломали обугленную плату управления и извлекли ее обломки.",
						"Вы слышите треск!")
					//SSticker.mode:apcs-- //XSI said no and I agreed. -rastaf0
				else
					user.visible_message(\
						"<span class='warning'>[user.name] извлек плату управления [src.name]!</span>",\
						"Вы извлекли плату управления APC.")
		...
		if(stat & BROKEN)
			user.visible_message("<span class='warning'>[user.name] попытался открыть крышку [src.name].</span>", "<span class='notice'>You try open [src.name] cover.</span>")
			if(W.use_tool(src, user, 25, volume = 25))
				opened = TRUE
				locked = FALSE
				if(cell)
					to_chat(user, "<span class='notice'>Батарея [src.name] выпала</span>")
		...
		if(coverlocked)
			to_chat(user, "<span class='notice'>Уберите защитные болты.</span>")
			if(W.use_tool(src, user, 5, volume = 5))
				coverlocked = FALSE
				update_icon()
		else
			to_chat(user, "<span class='warning'>Защитные болты уже подняты.</span>")

	else if	(istype(W, /obj/item/weapon/stock_parts/cell) && opened)	// trying to put a cell inside
		if(cell)
			to_chat(user, "Внутри уже установлена батарея.")
			return
		else
			if(stat & MAINT)
				to_chat(user, "<span class='warning'>Отсутствует разъем для батареи этого типа.</span>")
				return
			user.drop_item()
			W.loc = src
			cell = W
		...
		if(opened)
			if(cell)
				...
				if(has_electronics == 1 && terminal)
					...
					stat |= MAINT
					playsound(src, 'sound/items/Screwdriver.ogg', VOL_EFFECTS_MASTER)
					to_chat(user, "Вы открутили плату.")
				else /* has_electronics == 0 */
					to_chat(user, "<span class='warning'>Здесь нечего прикручивать.</span>")
					return
				update_icon()
		...
		if(opened)
			...
			if(W.use_tool(src, user, 6, volume = 50))
				if(prob(50))
					emagged = 1
					locked = 0
					to_chat(user, "Вы взломали интерфейс APC")
					update_icon()
				else
					to_chat(user, "Попытка [ locked ? "разблокировать" : "заблокировать"] интерфейс APC провалилась.")

	else if(iscoil(W) && !terminal && opened && has_electronics != 2)
		var/turf/TT = get_turf(src)
		if(TT.intact)
			to_chat(user, "<span class='warning'>Сначала нужно снять напольное покрытие перед APC.</span>")
			return
		var/obj/item/stack/cable_coil/C = W
		if(C.get_amount() < 10)
			to_chat(user, "<span class='warning'>Вам нужно больше проводов.</span>")
			return
		if(user.is_busy()) return
		to_chat(user, "Вы наали устанавливать провода в корпус APC..")
		if(C.use_tool(src, user, 20, volume = 50) && C.get_amount() >= 10)
			...
			C.use(10)
			user.visible_message(\
				"<span class='warning'>[user.name] установил провода в корпус APC!</span>",\
				"Вы установили провода в корпус APC.")
			make_terminal()
		...
		if(user.is_busy()) return
		to_chat(user, "Вы пытаетесь установить управляющую плату в корпус...")
		if(W.use_tool(src, user, 10, volume = 50))
			has_electronics = 1
			to_chat(user, "Вы установили управляющую плату в корпус APC.")
			qdel(W)
	else if(istype(W, /obj/item/weapon/module/power_control) && opened && has_electronics == 0 && ((stat & BROKEN) || malfhack))
		to_chat(user, "<span class='warning'>В не можете установить плату. Корпус поврежден.</span>")
		return
	else if(iswelder(W) && opened && has_electronics == 0 && !terminal)
		if(user.is_busy()) return
		var/obj/item/weapon/weldingtool/WT = W
		if(WT.get_fuel() < 3)
			to_chat(user, "<span class='notice'>Вам нужно больше сварочного топлив для завершения.</span>")
			return
		to_chat(user, "Вы начали сваривать корпус APC...")
		if(WT.use_tool(src, user, 50, amount = 3, volume = 50))
			if(emagged || malfhack || (stat & BROKEN) || opened == 2)
				new /obj/item/stack/sheet/metal(loc)
				user.visible_message(\
					"<span class='warning'>[src] был разрезан на части [user.name] при помощи сварки.</span>",\
					"Вы разобрали сломанный корпус APC",\
					"<span class='warning'>Вы слышите звуки сварки.</span>")
			else
				new /obj/item/apc_frame(loc)
				user.visible_message(\
					"<span class='warning'>[src] был отсоединен от стены [user.name] при помощи сварки.</span>",\
					"Вы отсоединили APC от стены.",\
					"<span class='warning'>Вы слышите звуки сварки.</span>")
			qdel(src)
			return
		...
		if(opened == 2)
			opened = 1
		user.visible_message(\
			"<span class='warning'>[user.name] заменяет поврежденную переднюю панель новой.</span>",\
			"Вы заменили поврежденную переднюю панель.")
		qdel(W)
		update_icon()
	else if(istype(W, /obj/item/apc_frame) && opened && ((stat & BROKEN) || malfhack))
		if(has_electronics)
			to_chat(user, "YВы не можете починить APC пока не извлечете электронику внутри.")
			return
		if(user.is_busy()) return
		to_chat(user, "Вы начали заменять поврежденный корпус APC...")
		if(W.use_tool(src, user, 50, volume = 50))
			user.visible_message(\
				"<span class='warning'>[user.name] заменил поврежденный корпус APC новым.</span>",\
				"Вы заменили поврежденный корпус APC новый.")
			qdel(W)
			stat &= ~BROKEN
			malfai = null
			malfhack = 0
...
/obj/machinery/power/apc/interact(mob/user)
	...
	if(ishuman(user))
		...
		if(H.species.flags[IS_SYNTHETIC] && H.a_intent == INTENT_GRAB && C)
			...
			if(emagged || (stat & BROKEN))
				...
				s.set_up(3, 1, src)
				s.start()
				to_chat(H, "<span class='warning'>Ток в APC резко возрастает повреждая ваше шасси!</span>")
				H.adjustFireLoss(10,0)
			else if(src.cell && src.cell.charge > 500 && H.a_intent == INTENT_GRAB)
				if(H.nutrition < C.maxcharge*0.9)
					if(src.cell.charge)
						to_chat(user, "<span class='notice'>Вы вставляете ваши пальцы в разъем APC и начинаете откачивать энергию для собственного пользования</span>")
						while(H.nutrition < C.maxcharge)
							if(do_after(user,10,target = src) && H.a_intent == INTENT_GRAB)
								if(!src.cell)
									to_chat(user, "<span class='notice'>Здесь нет батареи.</span>")
									break
									...
									to_chat (user, "<span class='notice'>В этом APC недостаточно заряда для откачки.</span>")
									break

								else if(src.cell.use(500))
									H.nutrition += C.maxcharge*0.1
									to_chat(user, "<span class='notice'>Откачка в батарее осталось[round(100.0*H.nutrition/C.maxcharge)]% заряда.</span>")

							else
								to_chat (user, "<span class='warning'>Процедура прервана, работа протокола завершена.</span>")
								break
					else
				...
				to_chat(user, "В этом APC недостаточно заряда для откачки.")
			return
	...
	if(usr == user && opened && !issilicon(user) && !isobserver(user))
		if(cell)
			...
			src.cell = null
			user.visible_message("<span class='warning'>[user.name] извлекает батарею из [src.name]!</span>", "Вы извлекли батарею.")
...
/obj/machinery/power/apc/attack_alien(mob/living/carbon/xenomorph/humanoid/user)
	to_chat(user, "Вы не можете сломать эту вещь");
	return
...
/obj/machinery/power/apc/Topic(href, href_list, usingUI = TRUE)
	...
	if(!can_use(usr, 1))
		...
		var/new_preset = input(usr, "Выберите уровень освещения ночной смены.") as null|anything in lighting_presets
		...
		if( issilicon(usr) && !src.aidisabled )
			if(malfai.malfhacking)
				to_chat(malfai, "Вы уже взломали этот APC.")
				return FALSE
			to_chat(malfai, "Начат процесс перехвата контроля APC. Данный процесс занимает некоторое время, в течение которого вы не сможете выполнять другие действия до завершения процесса.")
			malfai.malfhack = src
...
/obj/machinery/power/apc/process()
	...
	if(area.powerupdate)
		if(debug) log_debug("обновление питания в [area.name] / [name]")
		area.clear_usage()
