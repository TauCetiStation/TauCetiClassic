...
/datum/surgery_step/retract/can_use(mob/living/user, mob/living/carbon/human/surgery_victim, target_zone, obj/item/tool)
	...
	switch(target_zone)
		...
		// Retract Eyes
			...
			if(EYES_SURGERY)
				msg = "<span class='notice'>[user] begin to [EYES_ACTION].</span>"
		...
		// Retract Face
			...
			if(MOUTH_SURGERY)
				msg = "<span class='notice'>[user] begin [MOUTH_ACTION].</span>"
				self_msg = "<span class='notice'>You start [MOUTH_ACTION].</span>"
				user.visible_message(msg, self_msg)
		else // Head, Chest, Groin, L|R Arm, L|R Leg
			if(FATTAS_SURGERY)
				msg = "<span class='notice'>[user] begin to [FATTAS_ACTION].</span>"
				self_msg = "<span class='notice'>You start to [FATTAS_ACTION].</span>"
				user.visible_message(msg, self_msg)
				return TRUE
			//ribcage & retract skin & remove fat
			if(RETRACT_PRY)
				msg = "<span class='notice'>[user] begin to [SIMPLE_PRY_RETRACT_ACTION].</span>"
