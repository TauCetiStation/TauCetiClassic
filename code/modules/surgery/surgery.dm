...
/proc/do_surgery(mob/living/carbon/M, mob/living/user, obj/item/tool)
	...
	for(var/datum/surgery_step/S in surgery_steps)
		...
		if(S.tool_quality(tool) && S.can_use(user, M, target_zone, tool) && S.is_valid_mutantrace(M))
			...
			if(user.mood_prob(S.tool_quality(tool), 0.2, 1)) && tool.use_tool(M,user, step_duration, volume=100, required_skills_override = S.required_skills, skills_speed_bonus = S.skills_speed_bonus) && user.get_targetzone() && target_zone == user.get_targetzone())
