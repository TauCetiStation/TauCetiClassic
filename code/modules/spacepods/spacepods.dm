...
/obj/spacepod
	...
	var/lights = 0
	var/lights_power = 6
	var/list/icon_light_color = list("pod_civ" = "#FFFFFF", \
									 "pod_mil" = "#BBF093", \
									 "pod_synd" = "#B40000", \
									 "pod_gold" = "#FFFFFF", \
									 "pod_black" = "#3B8FE5", \
...
/obj/spacepod/attack_animal(mob/living/simple_animal/user)
	if((user.a_intent == "help" && user.ckey) || user.melee_damage_upper == 0)
...
/obj/spacepod/proc/deal_damage(var/damage)
	...
	if(oldhealth > health && percentage <= 25 && percentage > 0)
		playsound(loc,'sound/effects/engine_alert2.ogg',50,1)
...
/obj/spacepod/attackby(obj/item/weapon/W, mob/user, params)
...
/obj/spacepod/verb/go_in_space_pod()
	set category = "Object"
...
/obj/spacepod/proc/enter_pod(mob/user)
	...
	var/fukkendisk
	for(var/obj/item/weapon/disk/nuclear/FKD in user.client.screen)
		if(FKD)
			fukkendisk = FKD
			break
...
/obj/spacepod/proc/move_inside(mob/user)
	...
	if(passengers.len <= max_passengers)
		...
		if(do_after(user, 40, target = src) && !user.is_busy())
			if(!pilot || pilot == null)
				user.stop_pulling()
				pilot = user
				user.forceMove(src)
...
/obj/spacepod/verb/toggleDoors()
	...
	for(var/obj/machinery/door/spacepoddoor/multi_tile/P in orange(3,src))
...
/datum/global_iterator/pod_preserve_temp  //normalizing cabin air temperature to 20 degrees celsium
	...
	process(var/obj/spacepod/spacepod)
		if(spacepod.cabin_air && spacepod.cabin_air.return_pressure() > 0)
...
/datum/global_iterator/pod_tank_give_air/process(var/obj/spacepod/spacepod)
	if(spacepod && spacepod.internal_tank)
		...
		var/release_pressure = spacepod.internal_tank_valve
