...
/obj/item/gun_modular/module/chamber/init_default_components_module()
	...
	var/datum/gun_modular/component/proc_gun/user_recoil/user_recoil_proc = new (src)
	var/datum/gun_modular/component/proc_gun/visible_message_user/announse_shot_visible_message_user = new(src, "<span class='danger'>КТО ТО fires [src]!</span>", "<span class='danger'>You fire [src]!</span>", "You hear a gunshot!")
	...
	var/datum/gun_modular/component/proc_gun/return_ammo_insert_chamber/return_ammo_insert_chamber_proc = new (src)
	var/datum/gun_modular/component/check/ammo_returned/ammo_returned_check = new(src, null, null)
	var/datum/gun_modular/component/awaiter/await_return_ammo = new (src, return_ammo_insert_chamber_proc, ammo_returned_check, list(COMSIG_GUN_CHECK_SUCCESS), null)
	await_return_ammo.id_component = "await return ammo"

	var/datum/gun_modular/component/data/gun_get_ammo/gun_get_ammo_data = new(src, TRUE)
	gun_get_ammo_data.AddLastComponent(await_return_ammo)
	var/datum/gun_modular/component/check/chamber_Chambered/chamber_Chambered_check_from_get_ammo = new (src, null, gun_get_ammo_data)
