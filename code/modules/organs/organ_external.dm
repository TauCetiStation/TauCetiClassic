...
/obj/item/organ/external/proc/process_grasp(obj/item/c_hand, hand_name)
	...
	if(is_broken())
		...
		owner.emote("grunt", MESSAGE_VISIBLE | MESSAGE_AUDIBLE, "[(owner.species && owner.species.flags[NO_PAIN]) ? "" : emote_scream ] drops what they were holding in their [hand_name]!", auto = TRUE)
	if(is_malfunctioning())
		owner.drop_from_inventory(c_hand)
		owner.emote("grunt", MESSAGE_VISIBLE | MESSAGE_AUDIBLE, "drops what they were holding, their [hand_name] malfunctioning!", auto = TRUE)
