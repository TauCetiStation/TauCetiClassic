	...
	switch(disintegrate)
		...
		if(DROPLIMB_BURN)
			...
			for(var/obj/item/I in src)
				if(I.w_class > SIZE_SMALL && !istype(I, /obj/item/organ))
					I.loc = get_turf(src)
			should_delete = TRUE
		if(DROPLIMB_BLUNT)
...
/obj/item/organ/external/chest
	...
	vital = TRUE
	w_class = SIZE_HUGE // Used for dismembering thresholds, in addition to storage. Humans are w_class 6, so it makes sense that chest is w_class 5.
