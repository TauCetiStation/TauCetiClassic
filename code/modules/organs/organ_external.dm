...
/datum/organ/external
	...
	var/list/datum/organ/internal/bodypart_organs
...
/datum/organ/external/proc/take_damage(brute, burn, sharp, edge, used_weapon = null, list/forbidden_limbs = list())
	...
	if(bodypart_organs && ( (sharp && brute >= 5) || brute >= 10) && prob(5))
		...
		var/datum/organ/internal/IO = pick(bodypart_organs)
...
/datum/organ/external/proc/rejuvenate()
	...
	// handle organs
	for(var/datum/organ/internal/IO in bodypart_organs)
		IO.rejuvenate()
		...
		if((type == CUT || type == BRUISE) && damage >= 5)
			...
			if(compatible_wounds.len)
				...
				if(prob(25))
					...
					owner.visible_message("<span class='warning'>The wound on [owner.name]'s [name] widens with a nasty ripping voice.</span>",\
					"<span class='warning'>The wound on your [name] widens with a nasty ripping voice.</span>",\
...
/datum/organ/external/proc/handle_germ_effects()
	...
	if(germ_level >= INFECTION_LEVEL_TWO && antibiotics < 5)
		...
		var/datum/organ/internal/target_organ = null	//make organs become infected one at a time instead of all at once
		for (var/datum/organ/internal/IO in bodypart_organs)
...
/datum/organ/external/proc/droplimb(override = 0,no_explode = 0)
	...
	if(status & ORGAN_DESTROYED)
		...
		for(var/datum/organ/external/BP in owner.bodyparts)
			if(BP.parent == src)
				BP.droplimb(1)
		...
		if(bodypart)
			...
			if(status & ORGAN_ROBOT && !no_explode && sabotaged)
				owner.visible_message("<span class='warning'>\The [owner]'s [name] explodes violently!</span>",\
				"<span class='warning'><b>Your [name] explodes!</b></span>",\
			...
			owner.visible_message("<span class='warning'>[owner.name]'s [name] flies off in an arc.</span>",\
