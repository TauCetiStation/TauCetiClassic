/datum/event/aspect/area/replace/del_cable
	special_area_types = list(/area/station/maintenance)
	replace_types = list(/obj/structure/cable = null)
	num_replaceable = 1

/datum/event/aspect/area/replace/del_cable/setup()
	. = ..()
	num_replaceable = rand(1, 2)
	replace_callback = CALLBACK(src, .proc/remove_wire)

/datum/event/aspect/area/replace/del_cable/proc/remove_wire(obj/structure/cable/C)
