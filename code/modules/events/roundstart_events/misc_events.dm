...
/datum/event/feature/empty_sec_lockers/start()
	for(var/obj/structure/closet/secure_closet/security/S in world)
		if(prob(20) && is_station_level(S.z))
			for(var/obj/item/I in S)
				if(prob(70))
					qdel(I)
			message_admins("RoundStart Event: Security locker emptied at [COORD(S)]")
			log_game("RoundStart Event: Security locker emptied at [COORD(S)]")
...
/datum/event/feature/empty_vendors/start()
	for(var/obj/machinery/vending/V in machines)
		if(prob(30) && is_station_level(V.z))
			for(var/datum/data/vending_product/R in V.product_records)
				if(prob(40))
					R.amount = max(0, R.amount - rand(1,3))
			message_admins("RoundStart Event: [V] products reduced at [COORD(V)]")
