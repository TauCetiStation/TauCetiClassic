	...
	if(useMS)
		...
		if(prob(2))
			...
			switch(pick(1,2,3,4,5,6,7))
				if(1)
					sender = pick("MaxBet","MaxBet Онлайн Казино","Нет лучшего времени для регистрации","Я рад, что ты присоединишься к нам")
					message = pick("Тройные депозиты ждут вас на MaxBet Онлайн, когда вы зарегистрируетесь, чтобы играть у нас.",\
					"Вы можете получить 200% приветственный бонус в Max Bet Онлайн, когда зарегистрируетесь сегодня.",\
