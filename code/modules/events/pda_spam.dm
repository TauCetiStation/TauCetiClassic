		...
		if(prob(2))
			...
			switch(pick(1,2,3,4,5,6,7))
				if(1)
					...
					message = pick("Тройные депозиты, ставки, казино всё это и не только на MaxBet Онлайн уже сейчас!",\
					"Регестрируйтесь на MaxBet сегодня и получайте игровые бонусы уже завтра!",\
					"Став игроком на MaxBet, вы также будете получать выгодные еженедельные акции и промокоды на 100 кредитов ежемесячно, пройди регестрацию сейчас!",\
					"Более 450 первоклассных игр в казино, крупные выигрыши, надежность и проверенность 100 миллионами пользователей по всей вселенной, ждут вас в MaxBet!")
				if(2)
					...
					message = pick("Ваш профиль привлек мое внимание, и я хотел бы узнать вас поближе (Быстрые Знакомства).",\
					"Напишете мне на почту [pick(first_names_female)]@[pick(last_names)].[pick("ru","ck","tj","ur","nt")] и я обязательно пришлю вам моё горячее фото (Быстрые Знакомства).",\
				...
				if(3)
					sender = pick("Галактическая Платежная Ассоциация","Лучшее Бизнес-Бюро","Электронные выплаты Тау Киты","Финансовый Департамент НаноТрейзен","Копии Люкс")
					message = pick("Премиум часы Галактика по небольшой цене!",\
					"Часы, Ювелирные Изделия и Аксессуары, Сумки и модные Кошельки, всё по последнему писку галактической моды!",\
					"Вложите первые 100 кредитов и получите на 300% больше совершенно бесплатно!",\
				...
				if(4)
					...
					"После семи лет исследований доктор Акуилар и его команда придумали эту простую революционную формулу улучшения мужских качеств, которая состоит из...",\
					"Мужчины всех видов сообщают об УДИВИТЕЛЬНОМ увеличении длины, ширины и выносливости всего от трех...")
				if(5)
				...
				if(6)
					...
					message = pick("Подразделение морального духа НаноТрейзен хочет предоставить вам качественный развлекательный сайт WetSkrell.nt.",\
				...
				if(7)
					sender = pick("Вы выиграли бесплатные билеты!", "Нажмите сюда, чтобы забрать свой приз!", "Вы - 1000-й посетитель!", "Бесплатный билет на лучшее кино галактики!")
					message = pick("Вы выиграли билеты на новейший фильм ТЕОРИЯ ДЖЕКСОНА!",\
					"Вы выиграли билеты на новейшую криминальную драму ДЕТЕКТИВНАЯ ЗАГАДКА В КАПЕРСАХ МОЛЛЮСКОВ!",\
					"Вы выиграли билеты на новейшую семейную комедию ХОНК ЛЕГКОГО ПОВЕДЕНИЯ 2: ПЛАН БАНАН!",\
					"Вы выиграли билеты на новейший триллер ГИБЕЛЬ ПЛУТОНА")
