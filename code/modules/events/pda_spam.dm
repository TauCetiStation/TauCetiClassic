	...
	if(useMS)
		...
		if(prob(2))
			...
			switch(pick(1,2,3,4,5,6,7))
				if(1)
					sender = pick("MaxBet","MaxBet Онлайн Казино","Зарегистрируйся прямо сейчас!","С нетерпением ждем вас!")
					message = pick("Тройные депозиты, ставки, казино всё это только на MaxBet Онлайн!",\
					"Зарегистрируйтесь на MaxBet Онлайн сегодня и получайте игровые бонусы!",\
					"Став игроком на MaxBet Онлайн, вы будете получать выгодные еженедельные акции и промокоды на 100 кредитов ежемесячно. Пройдите регистрацию прямо сейчас!",\
					"Более 450 первоклассных азартных игр, крупные выигрыши, надежность и проверенный миллионами пользователей сервис ожидают вас. MaxBet Онлайн!")
				if(2)
				...
				if(3)
					sender = pick("Галактическая Ассоциация Выплат","Лучшее Бюро Бизнеса","Электронные выплаты Тау Кита","Финансовый Департамент НаноТрасен","Роскошные реплики")
					message = pick("Роскошные часы по взрывным ценам!",\
					"Часы, Ювелирные Изделия и Аксессуары, Сумки и модные Кошельки!",\
					"Вложите первые 100 кредитов и получите на 300 кредитов кэшбэка совершенно бесплатно!",\
				...
				if(4)
					...
					"Доктор Максман был создан Джорджем Окуляр, доктором медицинских наук, сертифицированным урологом CentComm, который вылечил более 70 000 пациентов во всем секторе от 'мужских проблем'.",\
					"После семи лет исследований, Доктор Окуляр и его команда придумали эту простую и революционную формулу улучшения мужской силы.",\
					"Мужчины всех видов сообщают об ПОТРЯСАЮЩЕМ увеличении длины, ширины и выносливости.")
				if(5)
					...
					message = pick("ВАШ СЧЕТ БЫЛ ПЕРЕМЕЩЕН В [pick("Salusa","Segunda","Cepheus","Andromeda","Gruis","Corona","Aquila","ARES","Asellus")] БАНК РАЗВИТИЯ ДЛЯ ДАЛЬНЕЙШИХ ПЕРЕВОДОВ СРЕДСТВ.",\
					...
					"В связи с нехваткой моих агентов, мне требуется независимый финансовый счет, чтобы немедленно перевести на него 1 ТОЧКА ПЯТЬ МИЛЛИОНОВ кредитов.",\
					"Приветствую сэр. С сожалением сообщаю, что пока я здесь умираю, из-за отсутствия наследников, я выбрал вас, чтобы перевести полную сумму моих сбережений в размере 1,5 миллиарда кредитов.")
				if(6)
					sender = pick("Подразделение морального духа НаноТрайзен","Одиноко?","Скучно?","www.wetskrell.nt")
					...
					"Wetskrell.nt предоставляет только самое высокое качество мужских развлечений для сотрудников НаноТрасен!",\
					"Просто введите номер вашего счета и PIN-код. Сделав три простых шага, сервис развлекательных услуг станет вашими!")
				if(7)
					...
					message = pick("Вы выиграли билеты на новейший фильм АКТИВНЫЙ ДЖЕКСОН!",\
