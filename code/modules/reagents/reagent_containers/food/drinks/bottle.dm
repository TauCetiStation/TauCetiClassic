...
/obj/item/weapon/reagent_containers/food/drinks/bottle/attack(mob/living/target, mob/living/user)
	...
	var/obj/item/bodypart/BP = user.zone_sel.selecting //Find what the player is aiming at
	...
	if(ishuman(target))
		...
		armor_block = H.run_armor_check(BP, "melee") // For normal attack damage

		//If they have a hat/helmet and the user is targeting their head.
		if(istype(H.head, /obj/item/clothing/head) && BP == "head")
		...
		armor_block = target.run_armor_check(BP, "melee")
		if(BP == "head")
			armor_duration = duration + force
	armor_duration /= 10

	//Apply the damage!
	target.apply_damage(force, BRUTE, BP, armor_block, sharp=0)

	// You are going to knock someone out for longer if they are not wearing a helmet.
	if(BP == "head" && istype(target, /mob/living/carbon/))
