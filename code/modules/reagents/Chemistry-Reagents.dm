...
/datum/reagent/consumable/sprinkles/on_general_digest(mob/living/M)
	..()
	M.nutrition += nutriment_factor
	if(istype(M, /mob/living/carbon/human) && M.job in list("Star Vigil Officer", "Head of Security", "Detective", "Star Vigil Sergeant", "Captain")) //if we want some FUN and FEATURES we should uncomment it
		M.heal_bodypart_damage(1, 1)
		M.nutrition += nutriment_factor
