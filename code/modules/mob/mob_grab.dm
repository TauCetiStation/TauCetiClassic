...
/obj/item/weapon/grab/atom_init(mapload, mob/victim, initial_state = GRAB_PASSIVE)
	...
	switch(state)
		if(GRAB_PASSIVE)
			...
			if(HAS_TRAIT(affecting, TRAIT_HARDTOGET))
				var/datum/component/mood/mood = affecting.GetComponent(/datum/component/mood)
				if(mood)
					new /obj/effect/temp_visual/heart/annoyed(affecting.loc)
					if(mood.mood_level < MOOD_LEVEL_NEUTRAL)
						SEND_SIGNAL(affecting, COMSIG_ADD_MOOD_EVENT, "bad_hug", /datum/mood_event/very_bad_hug)
					else
						SEND_SIGNAL(affecting, COMSIG_ADD_MOOD_EVENT, "bad_hug", /datum/mood_event/bad_hug)
		if(GRAB_AGGRESSIVE)
