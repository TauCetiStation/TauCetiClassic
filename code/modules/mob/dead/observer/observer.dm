...
/mob/dead/observer/proc/become_velocity_officer()
	...
	var/msg
	msg += "Вы - <B>Офицер Велосити!</B>\n"
	msg += "Будучи ответственным за сохранность 42-го дока ТСКИ Велосити, в Ваши задачи входит защита его имущества и клиентов.\n"
	msg += "Вы должны убедиться в том, что клиент сможет безопасно воспользоваться шаттлом и никому не повредит за время пребывания в доке.\n"
	msg += "Если клиент путается в руках и едва двигается - помогите ему, а если ведёт себя вызывающе и атакует Вас или других клиентов - нейтрализуйте его и сообщите администрации Велосити.\n"
	msg += "Вы не должны проверять документы и устраивать блокпост - идентификационная карта несет на себе все нужные данные и редко когда бывает ошибочной.\n"
	msg += "<B>Ни в коем случае не покидайте свое рабочее место.</B> За такое к Вам могут быть применены санкции не только со стороны администрации Велосити, но и со стороны глав Исхода.\n"
	to_chat(H, msg)
