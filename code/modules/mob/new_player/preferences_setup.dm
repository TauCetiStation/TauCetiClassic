datum/preferences
	...
	proc/update_preview_icon()		//seriously. This is horrendous.
		...
		if(undershirt > 0 && undershirt < undershirt_t.len && current_species.flags & HAS_UNDERWEAR)
			undershirt_s = new/icon("icon" = 'icons/mob/human_undershirt.dmi', "icon_state" = "undershirt[undershirt]_s")
		...
		if(job_civilian_low & ASSISTANT)//This gives the preview icon clothes depending on which job(if any) is set to 'high'
			...
			switch(job_civilian_high)
				...
				if(BOTANIST)
					...
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('tauceti/items/clothing/backpacks/backpacks.dmi', "backpack-hyd_mob"), ICON_OVERLAY)
						if(3)
			...
			switch(job_medsci_high)
				if(RD)
					...
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('tauceti/items/clothing/backpacks/backpacks.dmi', "backpack-tox_mob"), ICON_OVERLAY)
						if(3)
				...
				if(CHEMIST)
					...
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('tauceti/items/clothing/backpacks/backpacks.dmi', "backpack-chem_mob"), ICON_OVERLAY)
						if(3)
				...
				if(GENETICIST)
					...
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('tauceti/items/clothing/backpacks/backpacks.dmi', "backpack-gen_mob"), ICON_OVERLAY)
						if(3)
				...
				if(VIROLOGIST)
					...
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('tauceti/items/clothing/backpacks/backpacks.dmi', "backpack-vir_mob"), ICON_OVERLAY)
						if(3)
				...
				if(DETECTIVE)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon('icons/mob/back.dmi', "satchel"), ICON_OVERLAY)
				if(FORENSIC)
					clothes_s = new /icon('icons/mob/uniform.dmi', "forensicsred_s")
					clothes_s.Blend(new /icon('icons/mob/feet.dmi', "red"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon('icons/mob/hands.dmi', "lgloves"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon('icons/mob/suit.dmi', "forensics_red"), ICON_OVERLAY)
					switch(backbag)
						if(2)
							clothes_s.Blend(new /icon('icons/mob/back.dmi', "backpack"), ICON_OVERLAY)
						if(3)
