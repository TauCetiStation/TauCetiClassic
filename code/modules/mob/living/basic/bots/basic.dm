/mob/living/basic
    name = "Ты не должен видеть это"
    desc = "Что то пошло не так. Сообщите об ошибке"
    health = 20
	maxHealth = 20
	immune_to_ssd = TRUE

    /// 1 for full damage, 0 for none, -1 for 1:1 heal from that source.
    var/list/damage_coeff = list(brute = 1, burnt = 1, tox = 1, halloss = 0, oxy = 0)
	...
	var/minbodytemp = 350
	var/maxbodytemp = 250
	var/heat_damage_per_tick = 0 // amount of damage applied if animal's body temperature is higher than maxbodytemp
	var/cold_damage_per_tick = 0 // same as heat_damage_per_tick, only if the bodytemperature it's lower than minbodytemp
	...
	var/min_n2 = 0
	var/max_n2 = 0
	var/unsuitable_atoms_damage = 0	// This damage is taken when atmos doesn't fit all the requirements above
