...
/mob/living/simple_animal/borer/atom_init(mapload, request_ghosts = FALSE)
	...
	if(request_ghosts)
		for(var/mob/dead/observer/O in observer_list)
			try_request_n_transfer(O, "A new Cortical Borer was born. Do you want to be him?", XENOMORPH, IGNORE_BORER)

/mob/living/simple_animal/borer/attack_ghost(mob/dead/observer/O)
	try_request_n_transfer(O, "Cortical Borer, are you sure?", XENOMORPH, , show_warnings = TRUE)
