...
/obj/effect/proc_holder/spell/aoe_turf/hulk_jump/cast(list/targets)
	...
	if (istype(usr.loc,/turf) && !(istype(usr.loc,/turf/space)))
		...
		for(var/i=0, i<14, i++)
			...
			sleep(1)
		playsound(usr, 'sound/effects/explosionfar.ogg', VOL_EFFECTS)
		...
		if ((FAT in usr.mutations) && prob(66))
			...
			playsound(usr, 'sound/misc/slip.ogg', VOL_EFFECTS)
			usr.weakened += 10
			usr.stunned += 5
	...
	if (istype(usr.loc,/obj))
		...
		playsound(usr, 'sound/effects/bang.ogg', VOL_EFFECTS)
		var/wiggle = 6
		while(wiggle > 0)
			wiggle--
...
/mob/living/simple_animal/hulk/unathi/proc/try_to_eat(mob/living/target)
	...
	if(target.stat == DEAD)
		...
		if(do_after(usr,50,target = target))
			...
			playsound(usr, 'sound/weapons/zilla_eat.ogg', VOL_EFFECTS)
			target.gib()
	else
...
/obj/effect/proc_holder/spell/aoe_turf/hulk_lazor/cast(list/targets)
	...
	playsound(usr, 'sound/weapons/guns/gunpulse_taser2.ogg', VOL_EFFECTS)
	A.original = U
	A.current = U
	A.starting = T
	A.yo = U.y - T.y
...
/obj/effect/proc_holder/spell/aoe_turf/HulkHONK/cast(list/target)
	if (usr.lying || usr.stunned || usr.stat)
		...
		return
	playsound(usr, 'sound/items/AirHorn.ogg', VOL_EFFECTS)
...
/obj/item/weapon/organ/attack_animal(mob/user)
	...
	if(istype(user, /mob/living/simple_animal/hulk))
		...
		playsound(user, 'sound/weapons/zilla_eat.ogg', VOL_EFFECTS)
		Hulk.health += 10
