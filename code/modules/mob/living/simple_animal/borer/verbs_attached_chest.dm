...
/mob/living/simple_animal/borer/proc/advanced_analyze_host()
	...
	spawn(5 SECONDS)
		...
		src << browse(entity_ja(dat), "window=borerscan;size=430x600")
		return
...
/mob/living/simple_animal/borer/proc/get_host_data()
	...
	var/list/host_data = list(
		...
		"borer_present_chest" = H.has_brain_worms(BP_CHEST),
		"borer_present_r_arm" = H.has_brain_worms(BP_R_ARM),
		"borer_present_l_arm" = H.has_brain_worms(BP_L_ARM),
		"borer_present_r_leg" = H.has_brain_worms(BP_R_LEG),
		"borer_present_l_leg" = H.has_brain_worms(BP_L_LEG),
		...
		"bicaridine_amount" = H.reagents.get_reagent_amount("bicardine"),
		...
		"external_organs" = H.bodyparts.Copy(),
		"internal_organs" = H.organs.Copy()
		)
	return host_data
...
/mob/living/simple_animal/borer/proc/format_host_data(list/occ_in)
	...
	for(var/datum/reagent/R in occ["all_chems"])
		if(R.id == BLOOD || R.id == "inaprovaline" || R.id == "stoxin" || R.id == "dermaline" || R.id == "bicardine" || R.id == "dexalin")
			continue //no repeats
		else
	...
	for(var/obj/item/organ/external/e in occ["external_organs"])
		...
		if(istype(e, /obj/item/organ/external/chest) && host.is_lung_ruptured())
