...
/datum/species/diona
	...
	has_bodypart = list(
		 BP_CHEST  = /obj/item/organ/external/chest/diona
		,BP_GROIN  = /obj/item/organ/external/groin/diona
	...
	abilities = list(/mob/living/carbon/human/proc/seed_tile,
...
/datum/species/diona/on_life(mob/living/carbon/human/H)
	if(H.gestalt_direct_control && H.queued_orders.len)
		H.say(H.queued_orders[1])
		H.queued_orders.Cut(1, 2)
			...
			if(istype(H_T, /turf/space))
				...
				if(environment)
					...
					if(pressure < SOUND_MINIMUM_PRESSURE)
						language_key = ":f"
			queue_order("[language_key] [D.my_number] [choices[command_]] [pos_selectors[selector_]]")
			...
			if(subject_)
				...
				if(istype(H_T, /turf/space))
					...
					if(environment)
						...
						if(pressure < SOUND_MINIMUM_PRESSURE)
							language_key = ":f"
				queue_order("[language_key] [subject_choices[subject_]] [choices[command_]] [pos_selectors[selector_]]")
...
/mob/living/carbon/human/proc/seed_tile()
	set name = "Seed Tile"
	set desc = "As you patiently grow you push your nutrients into seeding the ground for later use..."
	set category = "Diona"
	...
	var/obj/structure/diona_plant/DP = locate() in get_turf(H)
	if(!DP)
		DP = new(get_turf(H))
		nutrition -= 10

	while(!is_busy() && do_after(H, 5 SECONDS, TRUE, DP, FALSE, TRUE) && H.nutrition > 230)
		DP.nutrition += 45
		nutrition -= 45
...
/datum/species/diona/on_gain(mob/living/carbon/human/H)
	if(H.unique_diona_hive_color == "")
		H.unique_diona_hive_color = rgb(H.r_eyes, H.g_eyes, H.b_eyes)

/datum/species/diona/handle_post_spawn(mob/living/carbon/human/H)
	H.gender = NEUTER

	for(var/obj/item/organ/external/BP in H.bodyparts)
		BP.rejuvenate() // There we have some code that makes organs nymphified.
...
/datum/species/diona/handle_death(mob/living/carbon/human/H)
	var/list/potential_hosts = shuffle(H.gestalt_subordinates)
	for(var/mob/living/carbon/monkey/diona/D in potential_hosts)
		D.set_gestalt(null)
		if(D.client)
			continue
		if(D.stat == DEAD)
