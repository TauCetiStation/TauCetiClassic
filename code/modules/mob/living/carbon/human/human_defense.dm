...
/mob/living/carbon/human/disarmReaction(mob/living/carbon/human/attacker, show_message = TRUE)
	...
	var/obj/item/organ/external/BP = bodyparts_by_name[ran_zone(attacker.get_targetzone())]
	...
	if (randn <= 25)
		...
		if(armor_check < 2)
			if(show_message)
				visible_message("<span class='danger'>[attacker] has pushed [src]!</span>")
		else if(show_message)
			visible_message("<span class='warning'>[attacker] attempted to push [src]!</span>")
		return
	...
	if(randn <= 60)
		...
		if(!contact)
			if( (l_hand && l_hand.flags & ABSTRACT) || (r_hand && r_hand.flags & ABSTRACT) )
				...
				drop_item()
				if(show_message)
					visible_message("<span class='warning'><B>[attacker] has disarmed [src]!</B></span>")
	...
	if(show_message)
		visible_message("<span class='warning'><B>[attacker] attempted to disarm [src]!</B></span>")
