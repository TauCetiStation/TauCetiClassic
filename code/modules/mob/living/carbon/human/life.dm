			...
			if(13 to 18)
				if(getBrainLoss() >= 60 && (!HAS_TRAIT(src, TRAIT_STRONGMIND) || get_species() != SKRELL))
					switch(rand(1, 3))
						...
						if(2)
							say(pick("Как минять руки?","ебучие фурри!", "Подебил", "Проклятые трапы!", "лолка!", "вжжжжжжжжж!!!", "джеф скваааад!", "БРАНДЕНБУРГ!", "БУДАПЕШТ!", "ПАУУУУУК!!!!", "ПУКАН БОМБАНУЛ!", "ПУШКА", "РЕВА ПОЦОНЫ", "Пати на хопа!", "Я ВЫПОЛНЯЮ ПРИКАЗ ХУЕСОСА ЦАРЕВИЧА!", "ДЕАХАКА ГЕНИЙ", "АЙМ ШУТ АЙМ ШУТ ВИИИУ ВИИИИУ", "ВЫЕБАН ПРЕЛЮДНО", "ЩАС УЛЬТАНУ", "ЕБАНЫЙ КУКОЛД"))
						if(3)
							emote("drool")
