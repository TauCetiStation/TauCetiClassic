...
/mob/living/carbon/human/proc/update_body()
	...
	if((underwear > 0) && (underwear < 12) && species.flags[HAS_UNDERWEAR])
		var/mutable_appearance/MA = (!fat) ? mutable_appearance('icons/mob/human_underwear.dmi', "underwear[underwear]_[g]_s", -BODY_LAYER) : mutable_appearance('icons/mob/human_underwear_fat.dmi', "underwear[underwear]_[g]_s", -BODY_LAYER)
	...
	if((undershirt > 0) && (undershirt < undershirt_t.len) && species.flags[HAS_UNDERWEAR])
		var/mutable_appearance/MA = (!fat) ? mutable_appearance('icons/mob/human_undershirt.dmi', "undershirt[undershirt]_s_[g]", -BODY_LAYER) : mutable_appearance('icons/mob/human_undershirt_fat.dmi', "undershirt[undershirt]_s_[g]", -BODY_LAYER)
		MA.pixel_x += species.offset_features[OFFSET_UNIFORM][1]
		MA.pixel_y += species.offset_features[OFFSET_UNIFORM][2]
		MA = update_height(MA, TRUE)
		standing += MA
	...
	if(socks > 0 && socks < socks_t.len && species.flags[HAS_UNDERWEAR])
		...
		if(r_foot && !r_foot.is_stump && l_foot && !l_foot.is_stump)
			var/mutable_appearance/MA = (!fat) ? mutable_appearance('icons/mob/human_socks.dmi', "socks[socks]_s_[g]", -BODY_LAYER) : mutable_appearance('icons/mob/human_socks_fat.dmi', "socks[socks]_s_[g]", -BODY_LAYER)
