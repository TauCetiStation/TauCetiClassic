/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M)
	...
	var/datum/organ/external/BPH = M:organs_by_name[BP_R_HAND]
	if (M.hand)
		BPH = M:organs_by_name[BP_L_HAND]
		...
		if(G.cell)
			if(M.a_intent == "hurt")//Stungloves. Any contact will stun the alien.
				if(G.cell.charge >= 2500)
					...
					var/datum/organ/external/BP = get_organ(M.zone_sel.selecting) // We're checking the outside, buddy!
					var/calc_power
