...
/mob/living/carbon/human/equip_to_slot(obj/item/W, slot, redraw_mob = 1)
	...
	switch(slot)
		...
		if(SLOT_R_HAND)
			...
			update_inv_r_hand()
		if(SLOT_BELT)
			playsound(src, 'sound/effects/equip_belt.ogg', VOL_EFFECTS_MASTER, 100, FALSE, -5)
			src.belt = W
			W.equipped(src, slot)
			update_inv_belt()
		if(SLOT_WEAR_ID)
		...
		if(SLOT_HEAD)
			...
			update_inv_head()
		if(SLOT_SHOES)
			playsound(src, 'sound/effects/equip_shoes.ogg', VOL_EFFECTS_MASTER, 100, FALSE, -5)
			src.shoes = W
			W.equipped(src, slot)
			update_inv_shoes()
		if(SLOT_WEAR_SUIT)
			...
			update_inv_wear_suit()
		if(SLOT_W_UNIFORM)
			playsound(src, 'sound/effects/equip_uniform.ogg', VOL_EFFECTS_MASTER, 100, FALSE, -5)
			src.w_uniform = W
			W.equipped(src, slot)
			update_inv_w_uniform()
		if(SLOT_L_STORE)
