		...
		if(get_species() == DIONA)
			...
			for(var/mob/living/carbon/monkey/diona/V in contents)
				if(istype(V) && V.gestalt == src)
					choices += V
			var/mob/living/carbon/monkey/diona/V = input(src, "Who do wish you to expel from within?") in null|choices
			if(V)
				to_chat(src, "<span class='notice'>You wriggle [V] out of your insides.</span>")
				V.splitting(src)
