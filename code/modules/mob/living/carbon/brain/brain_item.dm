...
/obj/item/cortical_stack
	name = "cortical stack with brain"
	desc = "A piece of juicy meat with wires attached to some device found in the head."
	icon = 'icons/obj/surgery.dmi'
...
/obj/item/cortical_stack/atom_init()
	. = ..()
	//Shifting the brain "mob" over to the brain object so it's easier to keep track of. --NEO
	//WASSSSSUUUPPPP /N
	spawn(5)
...
/obj/item/cortical_stack/proc/transfer_identity(mob/living/carbon/H)
	name = "[H]'s brain with wires"
	brainmob = new(src)
	brainmob.name = H.real_name
	brainmob.real_name = H.real_name
	brainmob.dna = H.dna.Clone()
