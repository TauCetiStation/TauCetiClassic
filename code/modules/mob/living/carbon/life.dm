...
/mob/living/carbon/Life()
	...
	in_stasis = istype(loc, /obj/structure/closet/body_bag/cryobag)
	...
	if(radiation)
		...
		if (radiation > 100)
			...
			if(!species.flags[RAD_ABSORB])
				Weaken(10)
				if(!lying)
					to_chat(src, "<span class='red'>You feel weak.</span>")
					emote("collapse")
		...
		if (radiation < 0)
			...
			if(radiation > 75)
				...
				damage = 3
				if(prob(1))
					to_chat(src, "<span class='red'>You mutate!</span>")
					randmutb(src)
					domutcheck(src,null)
					emote("gasp")
...
/mob/living/carbon/proc/get_heat_protection(temperature) //Temperature is the temperature you're being exposed to.
...
/mob/living/carbon/proc/get_cold_protection_flags(temperature)
	var/thermal_protection_flags = 0
	//Handle normal clothing
