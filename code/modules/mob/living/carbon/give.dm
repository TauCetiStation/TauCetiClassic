/mob/living/carbon/verb/give(mob/M in oview(1))
	...
	if(!I)
		to_chat(usr, "<span class='red'>You don't have anything in your hand to give to [M.name]</span>")
		return
	...
	if(!M.get_active_hand() || !M.get_inactive_hand())
		if(!can_give(M))
			return
		switch(alert(M,"[usr] wants to give you \a [I]?",,"Yes","No"))
			if("Yes")
				if(!can_give(M))
					return
				if(!I)
				...
				if(!Adjacent(M))
					...
					to_chat(M, "<span class='red'>[usr.name] moved too far away.</span>")
					return
				if(usr.get_active_hand() != I)
					to_chat(usr, "<span class='red'>You need to keep the item in your active hand.</span>")
					to_chat(M, "<span class='red'>[usr.name] seem to have given up on giving \the [I.name] to you.</span>")
					return
				if(M.get_active_hand() && M.get_inactive_hand())
					...
					M.put_in_hands(I)
				I.add_fingerprint(M)
				M.visible_message("<span class='notice'>[usr.name] handed \the [I.name] to [M.name].</span>")
			if("No")
				M.visible_message("<span class='red'>[usr.name] tried to hand [I.name] to [M.name] but [M.name] didn't want it.</span>")
	else
