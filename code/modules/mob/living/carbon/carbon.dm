...
/mob/living/carbon/vomit(punched = FALSE, masked = FALSE, vomit_type = DEFAULT_VOMIT, blood = FALSE, stun = TRUE, force = FALSE)
	var/mask_ = masked
	if(head && (head.flags & HEADCOVERSMOUTH))
		mask_ = TRUE

	. = ..(punched, mask_, vomit_type, blood, stun, force)
	if(. && !mask_)
