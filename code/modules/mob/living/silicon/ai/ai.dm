...
/mob/living/silicon/ai/show_malf_ai()
	...
	if(M)
		...
		if(global.hacked_apcs.len >= APC_MIN_TO_MALF_DECLARE)
			stat(null, "Time until station control secured: [max(malf.AI_capture_timeleft/(global.hacked_apcs.len/APC_MIN_TO_MALF_DECLARE), 0)] seconds")
