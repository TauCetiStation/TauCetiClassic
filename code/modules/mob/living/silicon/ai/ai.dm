...
/mob/living/silicon/ai/show_malf_ai()
	...
	if(M)
		...
		if (length(SSticker.hacked_apcs) >= APC_MIN_TO_MALF_DECLARE)
			stat(null, "Time until station control secured: [max(malf.AI_win_timeleft/(length(SSticker.hacked_apcs)/APC_MIN_TO_MALF_DECLARE), 0)] seconds")
