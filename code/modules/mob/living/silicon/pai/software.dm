		...
		if(istype(hackobj, /obj/item/device/paicard)) //Mod. main law, Mod. secondary law, Manage marked objects, Reset marked objects, Clear software, Unbound, Personality shift
			...
			switch(interaction_type)
				...
				if(INTERACTION_PAI_MODIFY_SEC_LAW)
					targetPersonality.laws = input("Insert new secondary law here.", "PAI exploiter", targetPersonality.laws.supplied[1]) as text
					to_chat(targetPersonality, "Your supplemental directives have been updated. Your new supplemental directive are: [targetPersonality.laws.supplied[1]]")
		...
		if(hacksuccess && src.hackobj)
			...
			if(istype(hackobj, /obj/item/device/paicard))
				...
				if(Temp.pai)
					...
					dat += "<a href='byond://?src=\ref[src];software=interaction;interactwith=[INTERACTION_PAI_MODIFY_SEC_LAW];sub=0'>Modify Secondary Laws</a> (Current: [Temppai.laws.supplied[1]]) <br>"
