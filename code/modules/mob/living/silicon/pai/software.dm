			...
			if(href_list["cable"])
				...
				if(href_list["cable"] == "1")
					...
					for (var/mob/M in viewers(T))
						M.show_message("<span class='warning'>A port on [src] opens to reveal [cable], which promptly falls [!M || (С && !C.is_in_hands(cable)) ? "to the floor" : "onto [M == C ? "your" : "someone's"] hand"].</span>", 3, "<span class='warning'>You hear the soft click of something light and hard falling [M ? "onto someone's hand" : "to the ground"].</span>", 2)
