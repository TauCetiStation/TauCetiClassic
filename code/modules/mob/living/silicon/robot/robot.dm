	...
	if(statpanel("Status"))
		...
		if(module)
			...
			if(current_jetpack) // if you have a jetpack, show the internal tank pressure
				stat(null, text("Internal Atmosphere Info: [current_jetpack.name]")) //Borgs have few types of jetpacks?
				stat(null, text("Tank Pressure: [current_jetpack.air_contents.return_pressure()]"))
