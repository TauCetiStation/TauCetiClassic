	...
	if(candidates.len)
		...
		for (var/obj/effect/landmark/L in /area/syndicate_mothership/elite_squad)
			...
			if (L.name == "Syndicate-Commando")
				...
				to_chat(new_syndicate_commando, "<span class='notice'>You are an Elite Syndicate. [!syndicate_leader_selected?</span>"commando":"<B>LEADER</B>"] in the service of the Syndicate. \nYour current mission is: \red<B> [input]</B>")
	...
	if(candidates.len)
		...
		for (var/obj/effect/landmark/L in world)
			if(L.name == "voxstart")
				...
				to_chat(new_vox, "<span class='notice'>You are a Vox Primalis, fresh out of the Shoal. Your ship has arrived at the Tau Ceti system hosting the NSV Exodus... or was it the Luna? NSS? Utopia? Nobody is really sure, but everyong is raring to start pillaging! Your current goal is: \red<B> [input]</B></span>")
...
/datum/admins/proc/create_vox_raider(obj/spawn_location, leader_chosen = 0)
	...
	var/datum/organ/external/BP = new_vox.bodyparts_by_name[BP_HEAD]
