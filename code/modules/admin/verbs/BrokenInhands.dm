/proc/getbrokeninhands()
	var/text
	for(var/A in typesof(/obj/item))
		var/obj/item/O = new A( locate(1,1,1) )
		if(!O) continue
		var/icon/IL = new(O.lefthand_file)
		var/list/Lstates = IL.IconStates()
		var/icon/IR = new(O.righthand_file)
		var/list/Rstates = IR.IconStates()
		var/icon/J = new(O.icon)
		var/list/istates = J.IconStates()
		if(!Lstates.Find(O.icon_state) && !Lstates.Find(O.inhand_state))
			if(O.icon_state)
				text += "[O.type] is missing left hand icon called \"[O.icon_state]\".\n"
		if(!Rstates.Find(O.icon_state) && !Rstates.Find(O.inhand_state))
			if(O.icon_state)
				text += "[O.type] is missing right hand icon called \"[O.icon_state]\".\n"


		if(O.icon_state)
			if(!istates.Find(O.icon_state))
				text += "[O.type] is missing normal icon called \"[O.icon_state]\" in \"[O.icon]\".\n"
		//if(O.inhand_state)
		//	if(!istates.Find(O.inhand_state))
		//		text += "[O.type] MISSING NORMAL ICON CALLED\n\"[O.inhand_state]\" IN \"[O.icon]\"\n"
		//text+="\n"
		qdel(O)
	if(text)
		var/F = file("broken_icons.txt")
		fdel(F)
		to_chat(F, text)
		to_chat(world, "Completeled successfully and written to [F]")


