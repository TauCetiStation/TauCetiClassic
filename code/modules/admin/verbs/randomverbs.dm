	...
	if(M.client.prefs.muted & mute_type)
		...
		if(tgui_alert(usr, "Would you like to make it permament?","Permamute?", list("Yes","No, round only")) == "Yes")
			...
			if(permmutreason)
				...
				M.client.prefs.save_preferences()
				M.client.prefs.muted |= mute_type
				notes_add(M.ckey, "Permamute from [mute_string]: [permmutreason]", admin_key = usr.client.ckey)
			...
			if(mutereason)
				notes_add(M.ckey, "Muted from [mute_string]: [mutereason]", admin_key = usr.client.ckey)
