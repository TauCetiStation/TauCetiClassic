...
/datum/filter_editor/tgui_static_data(mob/user)
	var/list/data = list()
	data["filter_info"] = global.master_filter_info
	return data
...
/datum/filter_editor/tgui_act(action, list/params)
	. = ..()
	if(.)
		return
	...
	switch(action)
		...
		if("modify_filter_value")
			...
			for(var/entry in new_filter_data)
				if(entry == global.master_filter_info[old_filter_data["type"]]["defaults"][entry])
