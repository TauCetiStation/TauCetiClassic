...
/datum/select_equipment/tgui_interact(mob/user, datum/tgui/ui)
...
/datum/select_equipment/tgui_state(mob/user)
	return global.admin_state

/datum/select_equipment/tgui_status(mob/user, datum/tgui_state/state)
	if(QDELETED(target_mob))
		return UI_CLOSE
	return ..()

/datum/select_equipment/tgui_close(mob/user)
...
//global.custom_outfits lists outfit *objects* so we'll need to do some custom handling for it
...
/datum/select_equipment/tgui_data(mob/user)
	var/list/data = list()
	if(!dummy)
		init_dummy()
...
/datum/select_equipment/tgui_static_data(mob/user)
	var/list/data = list()
	if(!cached_outfits)
		cached_outfits = list()
		cached_outfits += list(outfit_entry("General", /datum/outfit, "Naked", priority=TRUE))
		cached_outfits += make_outfit_entries("General", subtypesof(/datum/outfit) - typesof(/datum/outfit/job) - /datum/outfit/varedit - /datum/outfit/fun - /datum/outfit/families_police)
...
/datum/select_equipment/tgui_act(action, params)
	if(..())
		return
	. = TRUE
	switch(action)
