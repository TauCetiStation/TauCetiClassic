...
//		stickyban(href_list["stickyban"],href_list)
	...
	if(href_list["makeAntag"])
		...
		captain_announce("The emergency shuttle has been called. It will arrive in [round(SSshuttle.timeleft()/60)] minutes.")
		message_admins("\blue [key_name_admin(usr)] edited the Emergency Shuttle's timeleft to [SSshuttle.timeleft()]", 1)
		href_list["secretsadmin"] = "check_antagonist"
...
/*
	...
	//Antagonist (Orange)
		...
		if(jobban_isbanned(M, "Abductor") || isbanned_dept)
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Abductor;jobban4=\ref[M]'><font color=red>Abductor</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Abductor;jobban4=\ref[M]'>Abductor</a></td>"
		...
		switch(where)
			if("inhand")
				...
				if(isrobot(usr))
					...
					if(!R.module)
						R << "Cyborg doesn't has module, you can't do that."
						return
				target = usr
