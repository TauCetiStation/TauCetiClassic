...
/datum/admins/proc/DB_ban_record(var/bantype, var/mob/banned_mob, var/duration = -1, var/reason, var/job = "", var/rounds = 0, var/banckey = null, var/banip = null, var/bancid = null)
	...
	send2slack_logs(key_name_admin(usr), "has added a [bantype_str] for [ckey] [(job)?"([job])":""] [(duration > 0)?"([duration] minutes)":""] with the reason: \"[reason]\" to the ban database.", "(BAN)")
...
/datum/admins/proc/DB_ban_edit(var/banid = null, var/param = null)
	...
	switch(param)
		if("reason")
			...
			send2slack_logs(key_name_admin(usr), "has edited a ban for [pckey]'s reason from [reason] to [value]", "(BANEDIT)")
		if("duration")
			if(!value)
			...
			send2slack_logs(key_name_admin(usr), "has edited a ban for [pckey]'s duration from [duration] to [value]", "(BANEDIT)")
			update_query.Execute()
		if("unban")
...
/datum/admins/proc/DB_ban_unban_by_id(var/id)
	...
	send2slack_logs(key_name_admin(usr), "has lifted [pckey]'s ban.", "(UNBAN)")
