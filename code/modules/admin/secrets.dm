...
/*					for(var/obj/machinery/vehicle/pod/O in world)
		...
		*/
		...
		if("flicklights")
			...
			if(!ghost_mode_timer)
				message_admins("<span class='notice'>[key_name_admin(usr)] turned ghost mode ON</span>")
				log_admin("[key_name(usr)] turned ghost mode ON")
				ghost_mode_timer = addtimer(CALLBACK(GLOBAL_PROC, .proc/ghost_mode_ticker, FALSE), rand(30, 45) SECONDS, TIMER_UNIQUE|TIMER_STOPPABLE)
			else
				message_admins("<span class='notice'>[key_name_admin(usr)] turned ghost mode OFF</span>")
