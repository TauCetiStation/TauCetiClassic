...
/datum/outfit_manager/tgui_state(mob/user)
	return global.admin_state

/datum/outfit_manager/tgui_close(mob/user)
	qdel(src)

/datum/outfit_manager/tgui_interact(mob/user, datum/tgui/ui)
...
/datum/outfit_manager/tgui_data(mob/user)
	var/list/data = list()

	var/list/outfits = list()
	for(var/datum/outfit/custom_outfit in global.custom_outfits)
		outfits += list(entry(custom_outfit))
	data["outfits"] = outfits

	return data

/datum/outfit_manager/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return
	. = TRUE
