ADD_TO_GLOBAL_LIST(/obj/machinery/computer, computer_list)
...
/obj/machinery/computer/tgui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Computer", name)
		ui.open()

/obj/machinery/computer/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	var/list/data = ..()

	data["broken"] = stat & BROKEN
	data["power"] = stat & NOPOWER

	data["stationTime"] = worldtime2text()

	if(active_program)
		data = active_program.process_data(data, user)
		data["program"] = active_program.name
	else
		data["program"] = null
		var/list/folder_to_front = list()
		var/list/folder = hard_drive.folders[1]
		for(var/i in folder)
			var/datum/digital/file/File = hard_drive.filesystem[i]
...
/obj/machinery/computer/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	if(action)
		...
		switch(action)
			...
			if("create_file")
				hard_drive.add_file(1, name = sanitize(params["file_name"]), filetype = sanitize(params["file_type"]))
				return TRUE
			...
			if("program")
				active_program.act(sanitize(params["program_action"]), params, ui.user)
				return TRUE

/obj/machinery/computer/proc/open_file(file_id)
	if(file_id < 1 || file_id > hard_drive.filesystem_storage)
		return
...
/obj/machinery/computer/security
	preinstalled_programs = list(list("folder" = 1, "name" = null, "content" = null, "path" = /datum/digital/file/program/secrecords, "type" = null))