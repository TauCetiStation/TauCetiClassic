...
/datum/tgui_list_input/tgui_static_data(mob/user)
	...
	data["items"] = items
	data["large_buttons"] = FALSE //user.client.prefs.read_preference(/datum/preference/toggle/tgui_input_large)
	data["message"] = message
	data["swapped_buttons"] = TRUE //user.client.prefs.read_preference(/datum/preference/toggle/tgui_input_swapped)
	data["title"] = title
	return data
...
/datum/tgui_list_input/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	. = ..()
	if (.)
		return
	switch(action)
