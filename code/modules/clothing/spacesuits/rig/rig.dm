...
/obj/item/clothing/head/helmet/space/rig/skrell/attack_self(mob/user)
	if(!isturf(user.loc))
		to_chat(user, "You cannot turn the light on while in this [user.loc]")//To prevent some lighting anomalities.
		return
	on = !on
...
/obj/item/clothing/head/helmet/space/rig/skrell/black
...
/obj/item/clothing/suit/space/rig/skrell/black
...
/obj/item/clothing/head/helmet/space/rig/unathi/helmet_cheap
...
/obj/item/clothing/head/helmet/space/rig/unathi/helmet_cheap/attack_self(mob/user)
	if(!isturf(user.loc))
		to_chat(user, "You cannot turn the light on while in this [user.loc]")//To prevent some lighting anomalities.
		return
	on = !on
...
/obj/item/clothing/suit/space/rig/unathi/rig_cheap
...
/obj/item/clothing/suit/space/rig/vox/medic/equipped(mob/user, slot)
	..()
	if(slot == SLOT_WEAR_SUIT)
		wearer = user
		START_PROCESSING(SSobj, src)
...
/obj/item/clothing/suit/space/rig/vox/medic/dropped(mob/user)
	wearer = null
	STOP_PROCESSING(SSobj, src)
	..()

/obj/item/clothing/suit/space/rig/vox/medic/Destroy()
	STOP_PROCESSING(SSobj, src)
	wearer = null
	return ..()

/obj/item/clothing/suit/space/rig/vox/medic/process()
	if(!wearer)
		STOP_PROCESSING(SSobj, src)
		return
	if(wearer.stat == DEAD)
		STOP_PROCESSING(SSobj, src)
		return
	if(!istype(wearer.head, /obj/item/clothing/head/helmet/space/rig/vox/medic))
		return
	if(damage > 9)
...
/obj/item/clothing/suit/space/rig/vox/stealth/examine(mob/user)
	..()
	if(wearer)
...
/obj/item/clothing/suit/space/rig/vox/stealth/ui_action_click()
	toggle_stealth()

/obj/item/clothing/suit/space/rig/vox/stealth/Destroy()
	STOP_PROCESSING(SSobj, src)
	wearer = null
	return ..()

/obj/item/clothing/suit/space/rig/vox/stealth/process()
	if(on)
...
/obj/item/clothing/suit/space/rig/vox/stealth/equipped(mob/user, slot)
	..()
	if(slot == SLOT_WEAR_SUIT)
		wearer = user

/obj/item/clothing/suit/space/rig/vox/stealth/dropped(mob/user)
	toggle_stealth(TRUE)
	wearer = null
	..()

/obj/item/clothing/suit/space/rig/vox/stealth/proc/toggle_stealth(deactive = FALSE)
	if(on)
		...
		if(!istype(wearer.head, /obj/item/clothing/head/helmet/space/rig/vox/stealth))
...
/obj/item/clothing/suit/space/rig/vox/stealth/proc/is_damaged(low_damage_check = FALSE)
	if(damage > 6)
...
/obj/item/clothing/suit/space/rig/vox/stealth/proc/overload()
...
/obj/item/clothing/suit/space/rig/vox/stealth/attack_reaction(mob/living/L, reaction_type, mob/living/carbon/human/T = null)
	if(on)
		if(reaction_type == REACTION_ITEM_TAKE || reaction_type == REACTION_ITEM_TAKEOFF)
