...
/atom/proc/balloon_alert_to_viewers(message, self_message, vision_distance = 4, list/ignored_mobs)
	SHOULD_NOT_SLEEP(TRUE)
...
/atom/proc/balloon_alert_perform(mob/viewer, text)
	...
	var/image/balloon_alert = image(loc = isturf(src) ? src : get_atom_on_turf(src), layer = INDICATOR_LAYER)
