...
/obj/machinery/disposal
	...
	desc = "Пневматический мусоросброс внушительного размера. Кажется, в нём вполне мог бы поместиться живой человек?"
	...
	if(mode<=0) // It's off
		if(isscrewing(I))
			...
			if(W.use(0,user))
				...
				to_chat(user, "<span class='warning'>Нужно больше топлива для сварки.</span>")
				return
