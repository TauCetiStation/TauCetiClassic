...
/obj/machinery/disposal/attackby(obj/item/I, mob/user)
	...
	if(mode<=0) // It's off
		if(isscrewdriver(I))
			...
			if(W.use(0,user))
				...
				if(W.use_tool(src, user, 20, volume = 100, required_skills = list(/datum/skill/atmospherics/trained)))
	...
	if(istype(G))	// handle grabbed mob
		if(ismob(G.affecting))
			...
			if(G.use_tool(src, usr, 20, required_skills = list(/datum/skill/atmospherics/trained)))
...
/obj/structure/disposalpipe/attackby(obj/item/I, mob/user)
	...
	if(iswelder(I))
		...
		if(W.use(0,user))
			...
			if(W.use_tool(src, user, 30, volume = 100, required_skills = list(/datum/skill/atmospherics/trained)))
