...
/obj/machinery/computer/cargo/Topic(href, href_list)
	...
	if(href_list["order"])
		...
		if(href_list["order"] == "categories")
			...
			temp += "<div class='blockCargo'><table>"
			for(var/supply_name in SSshuttle.supply_packs)
				...
				if(requestonly)
					...
					continue
				temp += {"<tr><td><span class="cargo32x32 [replacetext(replacetext("[N.crate_type]", "/obj/", ""), "/", "-")]"></span></td>"}
				temp += "<td colspan='2'><A href='?src=\ref[src];doorder=[supply_name]'>[supply_name]</A> Cost: [N.cost]</td></tr>"		//the obj because it would get caught by the garbage
				if(ispath(N.crate_type, /obj/structure/closet/critter))
					...
					temp += {"<tr><td></td><td><span class="cargo32x32 [replacetext(replacetext("[animal]", "/mob/", ""), "/", "-")]"></span></td><td>[initial(animal.name)]</td></tr>"}
					continue
				...
				//======================================
					...
					if(element in sprite_32x48)
						size = "32x48"
					temp += {"<tr><td></td><td><span class="cargo[size] [replacetext(replacetext("[element]", "/obj/", ""), "/", "-")]"></span></td><td>[final_name]</td></tr>"}
			temp +="</table></div>"
