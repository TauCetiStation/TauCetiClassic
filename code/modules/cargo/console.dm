	...
	if(href_list["order"])
		...
		if(href_list["order"] == "categories")
			...
			for(var/supply_name in SSshuttle.supply_packs)
				...
				temp += {"<table><tr><td><span class="cargo32x32 [replacetext(replacetext("[N.crate_type]", "/obj/", ""), "/", "-")]"></span></td>"}
				...
				if(ispath(N.crate_type, /obj/structure/closet/critter))
					...
					temp += {"<tr><td><span class="cargo32x32 [replacetext(replacetext("[animal]", "/mob/", ""), "/", "-")]"></span></td><td>[initial(animal.name)]</td></tr>"}
				else
					...
					//======================================
						...
						if(element in sprite_32x48)
							size = "32x48"
						temp += {"<tr><td><span class="cargo[size] [replacetext(replacetext("[element]", "/obj/", ""), "/", "-")]"></span></td><td>[final_name]</td></tr>"}
				temp += "</table></div></div>"
			temp += "</div>"
