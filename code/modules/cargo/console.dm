		...
		if(href_list["order"] == "categories")
			...
			for(var/supply_name in SSshuttle.supply_packs)
				...
				if(requestonly)
					...
					continue
				temp += {"<div class="spoiler"><input type="checkbox" id='[N]'>"}
				temp += {"<table><tr><td><span class="cargo32x32 [replace_characters("[N.crate_type]",  list("[/obj]/" = "", "/" = "-"))]"></span></td>"}
				temp += {"<td><label for='[N]'><b>[N]</b></label></td><td><A href='?src=\ref[src];doorder=[supply_name]'>Cost: [N.cost]</A></td></tr></table>"}		//the obj because it would get caught by the garbage
				temp += "<div><table>"
