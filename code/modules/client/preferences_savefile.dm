...
/datum/preferences/proc/save_preferences_sqlite(var/user, var/ckey)
	...
	if(check.Execute(db))
		if(!check.NextRow())
			...
			q.Add("INSERT into client (ckey, ooc_color, lastchangelog, UI_style, default_slot, toggles, UI_style_color, UI_style_alpha, warns, warnbans, randomslot) VALUES (?,?,?,?,?,?,?,?,?,?,?)",\
			ckey, ooccolor, lastchangelog, UI_style, default_slot, toggles, UI_style_color, UI_style_alpha, warns, warnbans, randomslot)
			if(!q.Execute(db))
				message_admins("Error #: [q.Error()] - [q.ErrorMsg()]")
				warning("Error #:[q.Error()] - [q.ErrorMsg()]")
				return 0
			...
			q.Add("UPDATE client SET ooc_color=?,lastchangelog=?,UI_style=?,default_slot=?,toggles=?,UI_style_color=?,UI_style_alpha=?,warns=?,warnbans=?,randomslot=? WHERE ckey = ?",\
			ooccolor, lastchangelog, UI_style, default_slot, toggles, UI_style_color, UI_style_alpha, warns, warnbans, randomslot, ckey)
			if(!q.Execute(db))
				message_admins("Error #: [q.Error()] - [q.ErrorMsg()]")
				warning("Error #:[q.Error()] - [q.ErrorMsg()]")
				return 0
