	...
	switch(submenu_type)	//Submenu
		...
		if("appearance")
			...
			if(specie_obj.flags[HAS_HAIR_COLOR])
				. += "<a href='?_src_=prefs;preference=hair;task=input'>Change Color</a> [color_square(r_hair, g_hair, b_hair)]</font>"
			...
			. += "<b>Facial</b>"
			. += "<br><a href='?_src_=prefs;preference=facial;task=input'>Change Color</a> [color_square(r_facial, g_facial, b_facial)]</font>"
	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				...
				if("f_style")
					var/list/valid_facialhairstyles = get_valid_facialhairstyles()
