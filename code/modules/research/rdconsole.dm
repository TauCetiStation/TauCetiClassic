...
/obj/machinery/computer/rdconsole/proc/check_mat(datum/design/being_built, var/M)
	switch(M)
		...
		if("$phoron")
			return (linked_lathe.phoron_amount - (being_built.materials[M]/linked_lathe.efficiency_coeff) >= 0) ? 1 : 0
		if("$uranium")
			...
			for(var/datum/design/D in files.known_designs)
				...
				for(var/M in D.materials)
					...
					if(copytext(M, 1, 2) == "$")
						switch(M)
							if("$glass")
								if(D.materials[M]/coeff > linked_lathe.g_amount)
									check_materials = 0
							if("$metal")
								if(D.materials[M]/coeff > linked_lathe.m_amount)
									check_materials = 0
							if("$gold")
								if(D.materials[M]/coeff > linked_lathe.gold_amount)
									check_materials = 0
							if("$silver")
								if(D.materials[M]/coeff > linked_lathe.silver_amount)
									check_materials = 0
							if("$phoron")
								if(D.materials[M]/coeff > linked_lathe.phoron_amount)
									check_materials = 0
							if("$uranium")
								if(D.materials[M]/coeff > linked_lathe.uranium_amount)
									check_materials = 0
							if("$diamond")
								if(D.materials[M]/coeff > linked_lathe.diamond_amount)
									check_materials = 0
							if("$clown")
								if(D.materials[M]/coeff > linked_lathe.clown_amount)
									check_materials = 0
				...
				for(var/M in D.materials)
					...
					if(copytext(M, 1, 2) == "$")
						switch(M)
							...
							if("$uranium")
								if(D.materials[M]/coeff > linked_imprinter.uranium_amount) check_materials = 0
