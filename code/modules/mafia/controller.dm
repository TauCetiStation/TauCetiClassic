...
/datum/mafia_controller/proc/toggle_night_curtains(close)
	for(var/obj/machinery/door/poddoor/mafia/D in global.machines) //I really dislike pathing of these
...
/datum/mafia_controller/tgui_data(mob/user)
	. = ..()
	switch(phase)
...
/datum/mafia_controller/tgui_assets(mob/user)
	return list(
		get_asset_datum(/datum/asset/spritesheet/mafia),
	)

/datum/mafia_controller/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	. = ..()
	if(.)
		return
...
/datum/mafia_controller/tgui_state(mob/user)
	return global.always_state

/datum/mafia_controller/tgui_interact(mob/user, datum/tgui/ui)
...
/datum/mafia_controller/proc/basic_setup()
	...
	for(var/key in global.mafia_signup)
		if(global.directory[key])
			var/client/C = global.directory[key]
			if(isobserver(C.mob))
				possible_keys += key
				continue
		global.mafia_signup -= key //not valid to play when we checked so remove them from signups
	...
	for(var/i in 1 to req_players)
		...
		global.mafia_signup -= chosen_key
...
/datum/mafia_controller/proc/try_autostart()
	if(phase != MAFIA_PHASE_SETUP)
		return
	if(global.mafia_signup.len >= MAFIA_MAX_PLAYER_COUNT || custom_setup.len)//enough people to try and make something (or debug mode)
		basic_setup()
