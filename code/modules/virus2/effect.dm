...
/datum/disease2/effect/sensory_restoration
	...
	desc = "Вирус стимулирует циркуляцию крови, производство и замену тканей глаз и ушей, заставляя хозяина регенерировать при их повреждении."
	level = 3
	max_stage = 4
	cooldown = 1
	chance_minm = 100
...
/datum/disease2/effect/radian
	...
	desc = "Вирус мутирует в клетках кожи хозяина, увеличивая воздействие радиации."
	level = 3
	max_stage = 3
	cooldown = 10
	pools = list(POOL_NEUTRAL_VIRUS, POOL_NEGATIVE_VIRUS)
...
/datum/disease2/effect/telepathic
	...
	desc = "Вирус странным образом мутирует в мозге, наделяя носителя способностью общаться с другими людьми посредством своего разума."
	level = 3
	max_stage = 3
	cooldown = 60
	pools = list(POOL_POSITIVE_VIRUS)
		...
		switch(holder.stage)
			...
			if(3)
				if(!(H.f_style == "Dwarf Beard") && !(H.f_style == "Very Long Beard"))
					to_chat(H, "<span class='warning'>Вы чувствуете себя гигачадом!</span>")
...
/datum/disease2/effect/scream/activate_mob(mob/living/carbon/mob, datum/disease2/effectholder/holder, datum/disease2/disease/disease)
	if(prob(20) || holder.stage	== 1)
		...
		if(prob(30))
			to_chat(mob, "<span class='userdanger'>[pick("ААААА!","ДОЛЖЕН ОРАТЬ", "Вы не можете замолчать.")]</span>")
		else
			mob.emote("scream")
...
/datum/disease2/effect/hungry/activate_mob(mob/living/carbon/mob, datum/disease2/effectholder/holder, datum/disease2/disease/disease)
	if(prob(20) || holder.stage	== 1)
		...
		if(mob.nutrition < 10 && prob(5))
			to_chat(mob, "<span class='userdanger'>Голод делает вас слабым.</span>")
			mob.apply_effect(35,AGONY,0)
...
/datum/disease2/effect/drool
	name = "Drooling"
	desc = "Вирус вызывает воспаление внутри мозга, что приводит к постоянному слюнотечению."
	level = 1
	max_stage = 3
	cooldown = 10
	pools = list(POOL_NEUTRAL_VIRUS)
