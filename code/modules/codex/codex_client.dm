...
/client/verb/search_codex(searching as text)
	...
	if(codex_on_cooldown || !mob.can_use_codex())
		to_chat(src, "<span class='danger'>You cannot perform codex actions currently.</span>")
		return
	...
	if(mob && mob.mind && ! (usr.mind.special_role == TRUE))
...
/client/verb/list_codex_entries()
	...
	var/antag_check = mob && mob.mind && (usr.mind.special_role == TRUE)
