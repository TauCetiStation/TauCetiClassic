...
/obj/item/weapon/paper/update_icon()
	if(icon_state == "paper") //so we don't override custom icons for certain papers
		if(info)
...
//tele-derelict "lore" notes

/obj/item/weapon/paper/derelict_loveletter1
	icon_state = "note"
	name = "love letter"
	info = {"Дорогая Мери, <br> \
	Хоть я и знаю что это письмо до тебя не дойдёт из-за договора о неразглашении, я всё равно пишу его, так как мне приятно воображать как ты его читаешь. <br> \
	Работа замечательная, и вопреки твоим опасениям, абсолютно безопасная. Мою пробирки, слежу за показаниями приборов, таскаю оборудование туда сюда. Вообщем, всё тоже самое и что и в университете, но платят в разы больше. И коллектив тоже отличный - сплочённый и культурный. <br> \
	Также поражает оснащение станции; Все двери - механизированы и имеют электронный замок, воздух автоматически подаётся в комнаты при необходимости, везде имеется искусственная гравитация. <br> \
	Одним словом - ощущение что я попал в какой-то фантастический роман. До сих пор не верится что я здесь."}

/obj/item/weapon/paper/derelict_loveletter2
	icon_state = "note"
	name = "love letter"
	info = {"Дорогая Мери, <br> \
	Ты была права насчёт этой работы. Не стоило сюда лететь. Но теперь уже ничего не поменяешь. <br> \
	Установка телепортировала не груз а нас. Темнота космоса сменилась ужасающим, красно-оранжевым пейзажем. А потом... люди начали менятся. Как будто их что-то выворачивало на изнанку. <br> \
	Как бы я сейчас хотел домой к тебе.<br>"}

/obj/item/weapon/paper/derelict_testreport1
	icon_state = "log"
	name = "test report 1"
	info = {"<h4> Отчёт об результатах испытаний. </h4><br> \
	Груз - Металлический контейнер заполненный песком, масса - 15 килограмм. Дополнительно установлен радиомаяк и видео-камера, для отслеживания перемещения и возвращения груза. <br> \
	Мощность - 4 мВт. <br> \
	Поворот - 4380 минут по вертикали, 1200 по горизонтали. <br> \
	Хронология событий: <br> \
	12:03 - Подготовка установки к пуску. <br> \
	12:11 - Открытие воронки, запуск груза. Сигнал с радиомаяка и изображение с камеры пропали. Температура установки - 72 градуса. <br> \
	12:17 - Сигнал радиомаяка обнаружен, изображение с камеры крайне нечёткое и содержит большое количество помех. Положение груза отличается от предполагаемого примерно на 112 метров. Подготовка к возвражению груза. <br> \
	12:20 - Открытие воронки, успешное возвращение груза. Температура установки - 112 градусов. <br> \
	Груз имеет незначительные повреждения, установка в норме. <br> \
	<h4>Итог: установка требует доработки для уменьшения отклонения при перемещении. Необходимо провести испытания с живым грузом. </h4><br>"}

/obj/item/weapon/paper/derelict_testreport2
	icon_state = "log"
	name = "test report 2"
	info = {"<h4> Отчёт об результатах испытаний. </h4><br> \
	Масса груза - Несколько связанных металлических контенеров заполненных песком, масса - 300 килограмм. Дополнительно установлен радиомаяк, видео-камера и контейнер объёмом 5 литров с 4 мышами, для отслеживания перемещения и возвращения груза, а также проверки воздействия телепортации на живые организмы. <br> \
	Мощность - 6 мВт. <br> \
	Поворот - 3060 минут по вертикали, 2400 по горизонтали. <br> \
	Хронология событий: <br> \
	16:00 - Подготовка установки к пуску. <br> \
	16:04 - Открытие воронки, запуск груза. Сигнал с радиомаяка и изображение с камеры пропали. Температура установки - 146 градусов. <br> \
	16:11 - Сигнал радиомаяка и камеры вновь обнаружен. Показания с радиомаяка некорректны и постоянно меняются. Камера транслирует изображение с различными дефектами, как и при первом испытании установки. <br> \
	16:13 - Сигнал радиомаяка и камеры вновь утерян. <br> \
	16:20 - Сигнал радиомаяка вновь обнаружен, камера работает в нормальном режиме. Положение груза отличается от предролагаемого примерно на 4 метра. Подготовка к возвражению груза. <br> \
	16:34 - Открытие воронки, успешное возвращение груза. Температура установки - 322 градуса, приближается к максимальной рабочей температуре (400 градусов). <br> \
	Груз имеет незначительные повреждения, установка в норме. Проверка камеры и радиомаяка никаких дефектов не выявила. <br> \
	Мыши выглядят здоровыми, однако одна имеет странные опухоли на спине и шее, а также проявляет куда меньшую активность чем остальные. Отправлена в био-лабораторию для изучения. <br> \
	<h4> Итог: необходима доработка системы охлаждения установки для предотвращения аварий. Необходимо выяснить причину появления опухолей у одной из мышей. </h4><br>"}

/obj/item/weapon/paper/derelict_testreport3
	icon_state = "log"
	name = "test report 3"
	info = {"<h4> Отчёт об результатах испытаний. </h4><br> \
	Масса груза - Доброволец, экипированный скафандром с прикреплёнными к нему датчиками. В скафандр установлен радиомаяк, видео-камера, гарнитура для связи с добровольцем, барометр для измерения кровяного давления, пульсометр, датчик КГР и термометр.<br> \
	Мощность - 2 мВт. <br> \
	Поворот - 7010 минут по вертикали, 2600 по горизонтали. <br> \
	Хронология событий: <br> \
	7:45 - Подготовка установки к пуску. <br> \
	7:47 - Открытие воронки, запуск груза. Сигнал с радиомаяка и изображение с камеры пропали. Температура устаноВК=-юю, ..Одми..ркцешРФФФФФФФФФФФФФ"}

/obj/item/weapon/paper/derelict_scientistnote
	icon_state = "note"
	name = "note"
	info = {"Привет. Слышал новости? С Земли прислали заключённого для испытаний. Это наш шанс узнать что стало с той крысой. Не могу дождатся запуска."}

/obj/item/weapon/paper/derelict_engineernote
	icon_state = "note"
	name = "note"
	info = {"Директор, <br> \
	Прошу разрешить на время испытаний установки отключать от питания все остальные отсеки, кроме нашего, естественно. Нагрузка настолько большая что проводка просто выгорает, а СМЭХ-и автоматом уходят в защиту. Вообщем, тот ещё геморрой. <br> \
	Павел."}

/obj/item/weapon/paper/derelict_medicnote
	icon_state = "note"
	name = "note"
	info = {"Меня крайне беспокоят эксперименты которые проводят наши учёные. <br> \
	Вся эта мистика вокруг 'установки', запрос на испытания на людях, слухи про мышь которая отгрызла палец учёному в лаборатории ПОСЛЕ ВСКРЫТИЯ, когда она уже должна была быть мертва. <br> \
	Добром это не кончится."}

/obj/item/weapon/paper/derelict_officernote
	icon_state = "note"
	name = "note"
	info = {"Как будто проёба от яйцеголовых было мало, турели начали шмалять по всем леталом, а роботы - размахивать своими дубинками во все стороны. Видимо какой-то придурок сменил параметры целеуказания, хотя доступ для этого есть только у меня и директора. Что же с нами будет..."}

/obj/item/weapon/paper/derelict_biologistnote
	icon_state = "note"
	name = "note"
	info = {"Директор, оставил отчёт о вскрытии в офисе, в надёжном месте. Заберите скорее, и желательно сожгите после прочтения. Про мой палец ещё можно соврать, а если отчёт прочитает кто то ещё, то паники не миновать."}

/obj/item/weapon/paper/derelict_systemnote
	icon_state = "log"
	name = "security system log"
	info = {"|7:50| - АЛЬФРЕД БАЙЕРС зарегистрировался в системе (уровень доступа = полный). <br> \
	|7:52| - АЛЬФРЕД БАЙЕРС поднял код безопасности (зелёный => чёрный). <br> \
	|7:53| - АЛЬФРЕД БАЙЕРС изменил настройки автоматизированной системы безопасности (цели: (под_аррестом => все); режим: (задержать => уничтожить)). <br> \
	|7:53| - АЛЬФРЕД БАЙЕРС вышел из системы. <br>"}

/obj/item/weapon/paper/derelict_tokamakguide
	icon_state = "log"
	name = "reactor manual"
	info = {"Том, <br> \
	По хорошему тебя надо отсюда гнать пинками, за то что ты не знаешь элементарных вещей, и при этом работаешь инженером. Но мне тебя немного жалко, да и не с кем будет в карты играть, поэтому написал тебе 'мануал'. Прочитай всё до конца и правильно настрой реактор завтра.<br> \
	У тебя есть три основные консоли: консоль управления ядром (core control), консоль подачи топлива (fuel control) и консоль управления гиротронами (gyrotron control).<br> \
	Первым делом необходимо их настроить. Возьми мультитул и задай имя для каждой консоли (просто приложи мультитул к панели, и консоль сама выведет сообщение об смене имени). Имя у всех трёх должно быть одинаковым, это может быть любой текст на латинице (tokamak/reactor/rust/что угодно ещё).<br> \
	После того как задал имена консолям, иди в саму камеру реактора. Используя мультитул задай такое же имя гиротрону (одна единственная штука прямо перед окном нашей диспетчерской) и 6 инжекторам топлива (шесть одинаковых штук по бокам стеклянной комнаты с укреплённым полом).<br> \
	Дальше надень скафандр и заходи в эту самую стеклянную комнату по центру камеры, с большой, круглой штукой по середине. Эта штука - ядро реактора, самая важная часть. Убедись что ты плотно закрыл первый внешний шлюз перед тем как входить, дабы воздух не попал внутрь. В камере должен быть вакуум. После того как зашёл, задай ядру такое же имя как и всем прошлым машинам и выходи из камеры, опять же, не запустив воздух внутрь.<br> \
	Если ты всё сделал правильно, то время загружать топливо в инжекторы. На юге камеры, перед стеной, находится стол с брикетами твёрдого топлива - дейтерия (светло-серые брикеты) и трития (тёмно-серые брикеты). Загрузи тритий внутрь компрессора топлива (fuel compressor) который прямо у этого стола. Из одного брикета который там есть у тебя должно получится два тритиевых стержня, отложи их на время. Дальше загрузи дейтерий. Из двух брикетов дейтерия должно получится четыре дейтериевых стержня. <br> \
	Два плюс четыре равно шесть. Столько же у нас и инжекторов. Загрузи все полученные стержни внутрь всех шести инжекторов, и отправляйся обратно к консолям. (Опционально: закачай водород внутрь камеры при помощь коннектора снаружи, для большей энерговыработки). <br> \
	Теперь чётко следуй инструкциям: <br> \
	1. На консоли управления ядром у тебя должно высветится что-то вроде 'Core #имя_ядра, status: good'. Если это так - нажимай на кнопку 'access'. <br> \
	2. Должно быть поле 'field power density' и его значение, равное нулю. Используя кнопки с плюсиками, поставь его значение на 200, НО НЕ БОЛЬШЕ 500, иначе будет херово. <br> \
	3. Нажимай кнопку '', вокруг ядра должно появится голубая расплывчатая хрень, значит поле настроено. При этом она не должна выходить за стекла камеры. Если выходит - проверь что 'power density' равна двум сотням. ЕСЛИ ПОЛЕ 'FIELD INSTABILITY' РАСТЁТ, ТО ЧИТАЙ САМЫЙ НИЗ МАНУАЛА.<br> \
	С ядром закончили, теперь переходим к консоли подачи топлива. <br> \
	1. На консоли подачи должно высветится шесть инжекторов (строк). В каждой строке находится кнопка 'start injecting', нажми на все. <br> \
	Ну и наконец гиротрон, тут тоже всё просто, <br> \
	1. Должна высветится только одна строка, с двумя пунктами; 'fire delay' - задержка, и 'power' - мощность. Задержку ставишь на 2, мощность на 10-20.
	Теперь можешь расслабится, ибо надо только ждать. Завари себе кофейку, и снова садись за консоль управления ядром. Там есть поле 'plasma temperature'. Если ты всё правильно сделал, то скоро оно поднимется до 20 тысяч. Когда это произойдёт, то пересаживайся за консоль управления гиротроном и поменяй там мощность на более низкую, от пяти до десяти. <br> \
	После этого идёшь бухать с нами и играть в покер. Ну или ты опять обосрался, и что-то пошло не так, тогда читай ниже.<br> \
	<br> \
	Секция 'АААААА ПОМОГИТЕ, Я ОПЯТЬ ОБОСРАЛСЯ!!!':<br> \
	'АААА, НА КОНСОЛИ НЕТ КНОПКИ!!!' - проверь что у этой консоли и всех машин с ней связанных одно и то же имя. <br> \
	'АААА, НА КОНСОЛИ ЕСТЬ ЯДРО, НО ПИШЕТСЯ ЧТО ОНО НЕАКТИВНО!!!' - проверь что оно закреплено к полу болтами. <br> \
	'АААА, СТРАШНАЯ ОРАНЖЕВАЯ/КРАСНАЯ ПОЛОСКА РАСТЁТ!!!' - скорее всего это 'field instability' и если она дойдёт до конца то ядро ёбнет, поэтому как можно скорей: <br> \
	1. Проверь что размер поля не выше 500. Если нестабильность всё ешё растёт - уменьши до 200. <br> \
	2. Проверь что внутри стеклянной камеры нет НИЧЕГО кроме ядра и вентиляции которая ведёт к коннектору для канистры снаружи. Если туда каким то хером что-то попало, то переходи к пункту 4.<br> \
	3. Проверь что в камере нет пожара. Скорее всего он возник из-за того что ты добавил внутрь водород, при этом впустив внутрь воздух когда ставил имя ядру. <br> \
	4. Если внутри камеры посторонний объект/никакой из выше перечисленных пунктов не помог снизить нестабильность - выключай поле, НО ТОЛЬКО ЕСЛИ ПЛАЗМА ОСТЫЛА. Если она горячая, то немедленно выключи гиротрон и подачу топлива, и жди пока она остынет. <br> \
	Если ты всё таки конченный идиот, с фекалиями вместо мозгов, и ядро бахнуло - то тебя не просто уволят а оштрафуют, да так что придётся пахать на 10 работах несколько лет. Но этого ТОЧНО не случится, если ты просто будешь следовать инструкциям. <br> \
	Павел.<br> /
	<br> /
	P.S. Не забывай про радиацию и жар от ядра. В запущенном состоянии рядом лучше не стоять."}

/obj/item/weapon/paper/derelict_chaplainnote
	icon_state = "note"
	name = "note"
	info = {"Директор, <br> \
	Я ещё раз убедительно прошу проверить состояние моего заказа на колокол для церкви. Подвес уже как месяц стоит пустым, грешно это, и выглядит глупо. <br> \
	Максвелл."}
