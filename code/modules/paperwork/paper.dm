...
/obj/item/weapon/paper/changeling_object
	name = "Object - Changeling"
	info = {"<center><img src="top_secret.png" width="222" height="77"></center>
	<center><img src="unclassified.png" width="222" height="77"></center>
	<h1 align="center">Объект – Генокрад</h1>
	<h1 align="center">Класс объекта: Особо опасен</h1>
	<h1 align="center">Текущий статус объекта: подлежит уничтожению</h1>
	<h1 align="center">Описание объекта</h1>
	<p>Генокрады — обозначение группы существ-метаморфов. На данный момент правлением корпорации принято считать, \
	что менее 0,01% населения в системах подконтрольных НаноТрейзен являются генокрадами. Внешний вид – обладают возможностью принимать облик различных гуманоидных сущностей. \
	До недавнего момента НТ не располагало данными о механизме выбора облика, однако все сотрудники корпорации, облик которых принимали объекты, были найдены мёртвыми. \
	После получения нескольких образцов ДНК генокрадов и их анализа, были обнаружены сходства с нуклеиновыми кислотами погибших сотрудников станции, \
	при этом ДНК динамически изменялась и принимала разные формы. Именно поэтому до недавнего времени считалось, что генокрад может принимать форму любого объекта, который был им поглощён. \
	Однако после инцидента №3007 были зафиксированы случаи выживания жертв после поглощения их генетического кода. В некоторых случаях уничтожения тела особи, \
	таковая принимала форму небольшого червя. Червь обладает значительной мобильностью, а также сохраняет ДНК жертв.</p> <br>
	<h1 align="center"><b>Идентификация объектов</b></h1>
	<p>В данный момент корпорация не обладает возможностью обнаружения генокрадов до их проявления. Также до этого момента отсутствуют как различные внешние признаки, \
	так и изменения в характере и поведении, из чего в данный момент считается, что вычислить генокрада попросту невозможно.</p> <br>
	<h1 align="center"><b>Особые черты объекта</b></h1>
	<p>Каждый генокрад уникален и обладает своим уникальным набором способностей. Учеными уже было зарегистрировано несколько уникальных способностей особей данного вида. \
	Из них: высокочастотный крик, дезориентирующий живые формы жизни и выводящий из строя электроприборы, почти моментальная перестройка тканей тела в целях создания оружия, \
	брони, а также маскировочного покрова. Адаптированные ткани генокрада по своему строению и физическим свойствам схожи с хитином. \
	В данный момент исследователи сходятся во мнении, что число способностей генокрада зависит от количества поглощённых им организмов. \
	Но некоторые способности являются характерными для всего вида. Например, регенеративный стазис. В случае, если генокрад находится на грани клинической смерти, он впадает в данное состояние. \
	Отследить его невозможно, однако через некоторое время объект возобновляет процессы жизнедеятельности. Также есть основания полагать, \
	что генокрады обладают общей сетью связи (средствами таковой) и могут коммуницировать друг с другом. Считается, что связь осуществляется посредством гиперзвуковых волн, \
	генерируемыми непосредственно генокрадами, но из-за специфики их волн, получение подтверждение этой теории так и не было получено.</p> <br>
	<h1 align="center"><b>Протокол контакта</b></h1>
	<p>В случае контакта с генокрадом необходимо немедленно иммибилизовать объект и поместить его в изолятор научного отдела. \
	Научный экипаж станции авторизован проводить любые необходимые эксперименты до тех пор, пока это не угрожает безопасности транспортировки субъекта на ЦК. \
	В случае, если доставка генокрада на ЦК не представляется возможной или несёт за собой серьёзную угрозу жизни экипажа или целостности станции, \
	необходима <b>полная</b> ликвидация особи – сжигание, расчленение или уничтожение в двигателе сингулярности.</p> <br>
	<i><b>Приложение:</b> Несмотря на то, что исследования данного вида продолжаются уже более 10 лет, количество данных крайне мало. \ 
	В данный момент приоритетной задачей является изучение вида и разработка устройств для обнаружения и противостояния ему.</i>"}