...
/obj/item/weapon/paper/old_station_note_one/proc/write_info()
	...
	info += "3.09.2221. На станцию попытался проникнуть раздвечик Синдиката. Турели нейтрализовали врага, его снаряжение было передано научному персоналу.<br>"
	info += "Похоже, это был не самый ценный кадр, раз его послали почти без оружия и в древнем как мир скафандре.<br>"
...
/obj/item/weapon/paper/old_station_note_two/proc/write_info()
	...
	info += "<i>Если на эту проклятую станцию попытались напасть, значит о ней кто-то да знает. Эвакуация это просто вопрос времени.</i>"
...
/obj/item/weapon/paper/old_station_note_three/proc/write_info()
	...
	info += "17.09.2221. В дормиториях был найден учёный с вспоротым брюхом, жуткое зрелище.<br>"
