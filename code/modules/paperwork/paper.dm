...
/obj/item/weapon/paper/wires
	name = "'Техническая документация к проводам шлюзов'"
...
/obj/item/weapon/paper/brig_arsenal
	name = " paper -'Опись оружейной'"
	info = {"<b>Опись дополнительного оборудования оружейной:</b><ul>6 развертываемых барьеров<br>4 Портативных флешера
	...
	<b>3 баллистических комплекта</b>:<small><ul><li> Баллистический шлем<li>3 Баллистическая броня
	</ul></small>
	<br><b>3 комплекта аблятивной брони </b>:<small><ul><li> Аблятивный шлем <li> Аблятивная броня
	</ui></small>
	...
	<b>Опись основного оборудования оружейной:</b>
...
/obj/item/weapon/paper/space_structures/proc/get_space_structures_info()
	var/paper_text = "<center><img src = bluentlogo.png /><br /><font size = 3>Показания датчиков <b>[station_name()]</b>:</font></center><br /><hr>"
	paper_text += "Результаты сканирования потенциально интересных мест:<br />"
...
/obj/item/weapon/paper/cloning_lab
	name = "Аппарат Клонирования H-11."
	info = {"<h4>Инструкция.</h4>
	Поздравляю, вы испытываете экспериментальное устройство клонирования H-11!<br>
	
Использование H-11 не сложнее операции на мозге! Просто вставьте целевого гуманоида в камеру для сканирования и выберите опцию клонирования, чтобы начать процесс.<br>
	<b>Это все.!</b><br>
	<i>Обратите внимание, система клонирования не может сканировать неорганические формы жизни или мелких приматов. Сканирование невозможно, если субъект получил серьезное повреждение головного мозга.</i><br>
	<p>Предоставленная система капсулы клонирования произведет желаемый клон. Стандартное время роста клона составляет примерно 90 секунд.
	Капсула клонирования может быть разблокирована до завершения роста клона.</p><br>
	<i>Пожалуйста, обратите внимание, что полученные клоны будут дефективными в результате генетического дрейфа.  Мы надеемся уменьшить этот негативный эффект с помощью проведения большего количества тестов.
<br>
	Клоны также могут потерять память и радикально изменить личность, в результате процесса клонирования.</i><br>
	<br>
	<font size=1>Данная технология произведена по лицензии компании КлонТЕК.</font>"}

/obj/item/weapon/paper/cloning_log
	name = "журнал экспериментов"
	info = {"<h4>День 1</h4>
	Мы очень рады быть частью первого экипажа НК Ирманда!!<br>
	Этот корабль создан для испытания инновационной сверхсветовой технологии. Сначала у меня были некоторые опасения, \
	но инженеры настаивают, что это безопасно и нет абсолютно никакого риска поломки внешнего крыла из-за ускорения.<br>
	Нам было поручено протестировать последнюю модель капсулы для клонирования от 'КлонТЕК'. Мы останемся в доке на неделю перед запуском, но начнем тесты прямо сейчас. \
	Если двигатель так быстр, как они говорят, то у нас может не хватить времени провести все рутинные тесты на клонированном субъекте!<br>
	<br>
	<h4>День 2</h4>
	Мы клонировали чей-то труп, который был передан нам медицинской бригадой. Генетическая репликация достаточно хороша, чтобы позволить субъекту выживать вне капсулы, \
	но повреждение клеток все еще проблема для его долгосрочной жизни. В целях безопасности мы будем держать его в карантине.<br>
	Мы оставили ему несколько книг, но были слишком оптимистичны в отношении его умственных способностей. Его мозг, похоже, страдает из-за того же генетического разложения, \
	которое вызывали
предыдущие модели. Мы проведем больше тестов, чтобы увидеть, есть ли улучшения.<br>
	<h4>День 4</h4>
	Похоже, запуск будет даже раньше, чем ожидалось! Пресса начинает терять интерес к проекту, поэтому мы вынуждены прервать предполетные проверки \
	и дать им что-нибудь, о чем можно написать. Надеюсь, это приведет к увеличению финансирования...<br>
	Весь экипаж был приглашен в главный зал, где у нас есть места для начального сверхсветового ускорения. К сожалению, клон не может покинуть комнату карантина \

	без риска заражения, поэтому мы пристегнем его к кровати и будем надеяться на лучшее. В любом случае, если что-то пойдет не так, мы сможем вырастить еще одного.
	<br>
	<i>Профессор Гален Линкович</i>"}
