...
####Циклы без типа
Например `for(var/obj/item/X in some_list)`
будет дороже чем
```
for(var/X in some_list)
    var/obj/item/I = X
```
Однако лист `some_list` должен содержать ТОЛЬКО данные типа `/obj/item/I`, если лист состоит из разных типов данных, тогда надо использовать стандартный метод написания цикла (например 1-й вариант).
