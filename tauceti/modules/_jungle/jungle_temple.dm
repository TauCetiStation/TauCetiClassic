...
/obj/effect/step_trigger/trap/Trigger(var/atom/A)
	...
	switch(trap_type)
		if("sawburst")
			...
			var/image/flicker = image('tauceti/modules/_jungle/jungle.dmi',"sawblade")
		...
		if("poison_dart")
			...
			var/atom/myloc = src.loc
			var/image/flicker = image('tauceti/modules/_jungle/jungle.dmi',"dart[rand(1,3)]")
			myloc.overlays += flicker
			spawn(8)
				myloc.overlays -= flicker
				qdel(flicker)
		...
		if("flame_burst")
			...
			var/atom/myloc = src.loc
			var/image/flicker = image('tauceti/modules/_jungle/jungle.dmi',"flameburst")
			myloc.overlays += flicker
			spawn(8)
				myloc.overlays -= flicker
				qdel(flicker)
		...
		if("thrower")
			...
			var/atom/myloc = src.loc
			var/image/flicker = image('tauceti/modules/_jungle/jungle.dmi',"throw[throw_dir]")
			myloc.overlays += flicker
