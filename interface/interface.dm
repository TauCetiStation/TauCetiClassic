...
/client/verb/rules()
	set name = "Rules"
	set desc = "Открыть правила сервера."
	set hidden = TRUE
...
/client/verb/reportissue()
	...
	var/message = "Это откроет GitHub в вашем браузере. Вы уверены?"
...
/client/verb/changes()
	set name = "Changelog"
	set desc = "Ознакомиться с чейнджлогом."
	set hidden = 1
...
/proc/link_with_alert(client/user, link_url)
	if(link_url)
		if(tgui_alert(usr, "Это откроет ваш браузер. Вы уверены?",, list("Да", "Нет")) == "Да")
			user << link(link_url)
	else
