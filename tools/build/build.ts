...
export const DME_NAME = 'taucetistation';
...
const gitHookExec = "tools/build/build.sh tgui"

export const HookInstallTarget = new Juke.Target({
  onlyWhen: async ({ get }) => {
    const res = await Juke.exec("git", [
      "config",
      "merge.merge-tgui-bundle.driver"
    ]);
    return res.stdout.trim() !== gitHookExec;
  },
  executes: ({ get }) => {
    return Juke.exec("git", [
      "config",
      "--replace-all",
      "merge.merge-tgui-bundle.driver",
      gitHookExec,
...
export const TguiTarget = new Juke.Target({
  dependsOn: [HookInstallTarget, BunTarget, BiomeInstallTarget],
  inputs: [
 ...
 */
export const TguiCleanAllTarget = new Juke.Target({
...
export const CleanAllTarget = new Juke.Target({
  dependsOn: [TguiCleanAllTarget, CleanTarget],
  executes: async () => {
